-- Restore Douyin official phone authorization for free-measurement leads.
-- Legacy 0.1.10 clients remain on SMS through the API bootstrap contract.
-- Rollback: add a follow-up migration that switches active merchant
-- installations to SMS and restores the guarded RPC from 20260919160000.
...
WHERE installation.installation_kind = 'merchant'
  ...
  AND (
    installation.runtime_config #>> '{features,douyin_phone}' IS DISTINCT FROM 'true'
    OR installation.runtime_config #>> '{features,phone_capture_mode}'
      IS DISTINCT FROM 'douyin_phone'
    OR installation.runtime_config #>> '{features,sms_lead}' IS DISTINCT FROM 'true'
  );
...
BEGIN
  ...
  v_next_features := v_features || pg_catalog.jsonb_build_object(
    'sms_lead', true,
    'douyin_phone', p_enabled,
    'phone_capture_mode',
      CASE WHEN p_enabled THEN 'douyin_phone' ELSE 'sms' END
  );
