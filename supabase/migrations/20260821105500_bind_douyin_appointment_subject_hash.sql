-- Forward repair only:
-- 1. The applied 105000 through 105400 migrations remain immutable.
-- 2. This migration changes stored create_request_hash semantics and therefore
--    requires the appointment table to be empty before replacement.
-- 3. The server-owned mini-program subject is bound to every new idempotency
--    identity without entering snapshots, responses, or logs.
-- Roll back before any appointment write by restoring the reviewed 105400
-- function through another forward migration. After writes begin, rollback
-- requires an explicit compatibility function for subject-bound hashes.
...
DO $block$
BEGIN
  LOCK TABLE public.douyin_measurement_appointments
    IN ACCESS EXCLUSIVE MODE;

  IF EXISTS (
    SELECT 1
    FROM public.douyin_measurement_appointments
  ) THEN
...
COMMENT ON FUNCTION public.submit_douyin_measurement_appointment(
  uuid, uuid, text, text, text, date, text, uuid, text, text, uuid,
  text, text, text, text, timestamptz, jsonb
) IS '提交抖音量房预约；幂等身份绑定安装、租户、会话主体和规范请求';
