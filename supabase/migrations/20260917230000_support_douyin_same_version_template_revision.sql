-- Support a newly confirmed template package that intentionally reuses the
-- same user-facing semantic version. The exact delivery identity is the
-- installation, template id, and version; provider_summary correlates that
-- identity with Douyin's version-list response.
--
-- Rollback: stop tenant release creation, prove no installation has multiple
-- template ids for one version, restore UNIQUE (installation_id,
-- template_version), restore both upload functions from 20260829012000, then
-- drop provider_summary. If same-version revisions exist, use a separately
-- reviewed forward migration instead of deleting or rewriting release rows.
BEGIN;
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '23505',
      MESSAGE = 'DOUYIN_RELEASE_EXACT_DELIVERY_DUPLICATES_EXIST';
  END IF;
END;
$$;
...
BEGIN
  ...
  WHERE release.installation_id = p_installation_id
    AND release.template_id = p_template_id
...
COMMENT ON FUNCTION public.get_or_create_and_claim_douyin_miniapp_release_upload(
  uuid, text, text, text, text, jsonb, uuid, timestamptz, uuid
) IS 'Atomically creates or claims an exact merchant template delivery and records its provider summary evidence.';

DROP FUNCTION public.get_or_create_and_claim_douyin_miniapp_release_upload_v2(
  uuid, text, text, text, text, jsonb, uuid, timestamptz, uuid
);
...
RETURNS TABLE(
  id uuid, installation_id uuid, template_id text, template_version text,
  description text, provider_summary text, channel text, ext_json jsonb,
  status text, douyin_log_id text, test_qr_url text,
  latest_test_qr_url text, audit_qr_url text, audit_host_names text[],
  audit_note text, audit_result jsonb, submitted_at timestamptz,
  audited_at timestamptz, released_at timestamptz,
  platform_operator_id uuid, created_at timestamptz, updated_at timestamptz,
  operation_name text, operation_claim_token uuid,
...
COMMENT ON FUNCTION public.get_or_create_and_claim_douyin_miniapp_release_upload_v2(
  uuid, text, text, text, text, jsonb, uuid, timestamptz, uuid
) IS 'Claims an exact Douyin template delivery and returns provider summary evidence.';
