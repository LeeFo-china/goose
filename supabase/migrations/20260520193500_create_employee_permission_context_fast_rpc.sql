...
AS $$
  WITH employee_row AS (
    SELECT
      ...
      tenant_department.legacy_department_id AS tenant_department_legacy_department_id,
  ...
  SELECT
    (
      SELECT jsonb_build_object(
        ...
        END,
        'department', CASE
          WHEN employee_row.department_id IS NULL THEN NULL
          ELSE jsonb_build_object(
            'name', employee_row.department_name,
            'code', employee_row.department_code
          )
        END,
        'tenant_department', CASE
          ...
          ELSE jsonb_build_object(
            ...
            'legacy_department_id', employee_row.tenant_department_legacy_department_id
          )
        END,
        'post', CASE
