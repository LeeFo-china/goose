...
BEFORE UPDATE OF
  ...
  received_at,
  created_at
ON public.wechat_virtual_payment_notifications
FOR EACH ROW
...
BEGIN
  IF p_order_id IS NULL
     ...
     OR (p_allow_late_closed_recovery AND p_source <> 'reconciliation')
     OR p_event_type NOT IN ('xpay_goods_deliver_notify', 'query_order')
     OR (p_source = 'notification' AND (
       p_notification_id IS NULL
       OR p_event_type <> 'xpay_goods_deliver_notify'
     ))
     OR (p_source <> 'notification' AND p_notification_id IS NOT NULL)
  ...
  IF p_notification_id IS NOT NULL THEN
    ...
    IF NOT FOUND
       ...
       OR v_notification.normalized_payload->>'openid_hash' IS DISTINCT FROM
          encode(digest(p_openid, 'sha256'), 'hex')
       OR v_notification.normalized_payload->>'provider_order_no'
          IS DISTINCT FROM p_provider_order_no
       OR v_notification.normalized_payload->>'transaction_id'
          IS DISTINCT FROM p_transaction_id
       OR (v_notification.normalized_payload->>'paid_at')::timestamptz
          IS DISTINCT FROM p_paid_at
       OR (v_notification.normalized_payload->>'quantity')::integer
          IS DISTINCT FROM p_quantity
       OR (v_notification.normalized_payload->>'orig_price_fen')::integer
          IS DISTINCT FROM p_orig_price_fen
       OR (v_notification.normalized_payload->>'actual_price_fen')::integer
          IS DISTINCT FROM p_actual_price_fen
       OR v_notification.normalized_payload->>'attach' IS DISTINCT FROM p_attach
    THEN
      RAISE EXCEPTION USING
...
REVOKE ALL ON FUNCTION public.branding_confirm_virtual_addon_purchase(
  uuid, uuid, text, boolean, text, boolean, text, text, text, text,
