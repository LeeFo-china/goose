...
BEGIN
  ...
  IF p_attribution - ARRAY[
    ...
    OR EXISTS (
      SELECT 1
      FROM jsonb_each(p_attribution - 'analysis_info') AS attribution(key, value)
    ...
    OR (
      p_attribution ? 'analysis_info'
      AND (
        CASE
          WHEN jsonb_typeof(p_attribution -> 'analysis_info') <> 'object'
            THEN false
          ELSE
            jsonb_typeof(p_attribution -> 'analysis_info' -> 'type') = 'number'
            AND p_attribution -> 'analysis_info' ->> 'type' IN ('1', '2', '3', '4')
            AND CASE p_attribution -> 'analysis_info' ->> 'type'
              WHEN '1' THEN
                (p_attribution -> 'analysis_info') - ARRAY[
                  'type', 'unique_id', 'author_open_id', 'video_item_id'
                ] = '{}'::jsonb
                AND jsonb_typeof(p_attribution -> 'analysis_info' -> 'video_item_id') = 'string'
              WHEN '2' THEN
                (p_attribution -> 'analysis_info') - ARRAY[
                  'type', 'unique_id', 'anchor_open_id', 'live_room_id'
                ] = '{}'::jsonb
                AND jsonb_typeof(p_attribution -> 'analysis_info' -> 'live_room_id') = 'string'
              WHEN '3' THEN
                (p_attribution -> 'analysis_info') - ARRAY['type', 'unique_id'] = '{}'::jsonb
                AND jsonb_typeof(p_attribution -> 'analysis_info' -> 'unique_id') = 'string'
              WHEN '4' THEN
                (p_attribution -> 'analysis_info') - ARRAY['type', 'unique_id'] = '{}'::jsonb
                AND jsonb_typeof(p_attribution -> 'analysis_info' -> 'unique_id') = 'string'
              ELSE false
            ...
            AND NOT EXISTS (
              SELECT 1
              FROM jsonb_each((p_attribution -> 'analysis_info') - 'type') AS official(key, value)
