...
-- 1. Disable new public-project publication writes and keep the compatibility route.
-- 2. Confirm no published client depends on this RPC.
-- 3. Only then use a forward migration to revoke EXECUTE and drop the function.
-- Existing profile rows are business data and must not be deleted during rollback.
-- This migration only replaces the function definition and never invokes it.
...
DECLARE
  ...
  v_image_reference record;
  v_selected_reference record;
...
BEGIN
  ...
  FOR v_image_reference IN
    SELECT reference.value
    FROM pg_catalog.unnest(v_image_references) AS reference(value)
  LOOP
    IF v_image_reference.value !~ '^https://'
      AND v_image_reference.value NOT LIKE v_scope_prefix || '%'
    THEN
      RETURN pg_catalog.jsonb_build_object(
        'error', pg_catalog.jsonb_build_object(
          'status_code', 400,
      ...
      THEN
        ...
        LOOP
          IF public.douyin_public_image_urls_are_valid(
              ARRAY[v_image_reference.value]
            )
            AND NOT (
              v_image_reference.value ~ '^https://'
              AND (
                pg_catalog.strpos(v_image_reference.value, '?') > 0
                OR pg_catalog.strpos(v_image_reference.value, '#') > 0
              )
            )
            AND (
              v_image_reference.value ~ '^https://'
              OR v_image_reference.value LIKE v_scope_prefix || '%'
            )
            AND pg_catalog.array_position(
              v_candidate_references,
              v_image_reference.value
            ) IS NULL
...
) IS 'Atomically validates tenant-owned project-log image references and saves a Douyin public project profile; PL/pgSQL loop records cannot shadow SQL aliases.';
