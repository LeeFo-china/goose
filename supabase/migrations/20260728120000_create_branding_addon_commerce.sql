-- Rollback: in a forward migration, disable the add-on product and purchase
-- entry points, revoke and drop branding_confirm_addon_purchase, remove the
-- four scoped role-permission mappings and permission rows, then preserve paid
-- order/event audit data until all in-flight orders are resolved before
-- dropping the notification, order, and product tables.
