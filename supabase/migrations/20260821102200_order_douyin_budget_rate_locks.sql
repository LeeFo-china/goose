-- Rollback: forward-only. Replacing this function with its prior definition
-- would restore a cross-request deadlock risk. Roll forward with another
-- same-signature CREATE OR REPLACE FUNCTION if the command needs adjustment.
...
CREATE OR REPLACE FUNCTION public.create_douyin_budget_estimate(
  p_tenant_id uuid,
...
BEGIN
  ...
  -- Order the final signed bigint identifiers, not their source text. This
  -- gives every caller the same lock order. A rare hash collision is acquired
  -- once and can only serialize unrelated callers; it cannot bypass a check.
  FOR v_lock_id IN
