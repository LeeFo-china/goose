-- Reset workflow replay data for the designated test project.
--
-- Project: 2d710a84-1045-4750-8dfd-51a0f463a4db
--
-- This complements the workflow runtime reset by clearing business artifacts
-- produced while testing workflow nodes, then recreating a fresh runtime at
-- the active construction workflow's initial node.
--
-- Scope:
-- - Deletes project_acceptances for this project; acceptance items, actions and
--   open tickets are removed through ON DELETE CASCADE.
-- - Deletes the stage_2 payment row created while testing the mid-stage
--   payment gate.
-- - Recreates one fresh workflow instance and subject projection.
--
-- Project logs are intentionally left untouched. This project currently has no
-- project_logs rows, and logs may be used later to test required node input.
DO $$
DECLARE
...
BEGIN
  ...
  IF v_tenant_id IS NULL THEN
    RAISE NOTICE 'Target test project not found, skip workflow replay data reset: %', v_project_id;
    RETURN;
  END IF;
  ...
  DELETE FROM public.project_acceptances
  WHERE tenant_id = v_tenant_id
    AND project_id = v_project_id::uuid;

  DELETE FROM public.payments
  WHERE project_id = v_project_id::uuid
    AND type = 'stage_2';
  ...
  SELECT public.start_workflow_instance(
    ...
    v_project_id,
    jsonb_build_object(
      'source', 'test_project_workflow_replay_data_reset',
      'reason', 'clear workflow replay artifacts and start from initial node',
      'reset_at', now()
    ),
    NULL
  )
  ...
  IF COALESCE((v_start_result->>'ok')::boolean, false) IS NOT TRUE THEN
    RAISE EXCEPTION 'Failed to reset project workflow replay data: %', v_start_result;
  END IF;
