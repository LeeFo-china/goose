-- Harden the tenant lead API without changing the applied 105000-105500 history.
BEGIN;

SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '30s';

CREATE FUNCTION public.list_tenant_douyin_lead_latest_appointments(
  p_tenant_id uuid,
  p_marketing_lead_ids uuid[]
)
RETURNS TABLE (
  id uuid,
  appointment_no text,
  ...
  budget_estimate_id uuid,
  preferred_visit_date date,
  preferred_visit_period text,
  community text,
  status text,
  confirmed_visit_at timestamptz,
...
BEGIN
  IF p_tenant_id IS NULL
    OR p_marketing_lead_ids IS NULL
    OR cardinality(p_marketing_lead_ids) < 1
    OR cardinality(p_marketing_lead_ids) > 100
    OR array_position(p_marketing_lead_ids, NULL) IS NOT NULL
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'tenant_douyin_lead_latest_appointments_invalid';
  END IF;

  SELECT count(*), count(DISTINCT requested.lead_id)
  INTO v_requested_count, v_unique_count
  FROM unnest(p_marketing_lead_ids) AS requested(lead_id);
  IF v_requested_count IS DISTINCT FROM v_unique_count THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'tenant_douyin_lead_latest_appointments_duplicate_ids';
  END IF;
  ...
  WITH ranked AS (
    SELECT
      ...
      appointment.budget_estimate_id,
      appointment.preferred_visit_date,
      appointment.preferred_visit_period,
      appointment.community,
      appointment.status,
      appointment.confirmed_visit_at,
      appointment.created_at,
      appointment.updated_at,
      appointment.version,
      row_number() OVER (
        PARTITION BY appointment.marketing_lead_id
        ORDER BY appointment.created_at DESC, appointment.id DESC
      ) AS appointment_rank
    FROM public.douyin_measurement_appointments AS appointment
    JOIN unnest(p_marketing_lead_ids) AS requested(lead_id)
      ON requested.lead_id = appointment.marketing_lead_id
    WHERE appointment.tenant_id = p_tenant_id
  )
  SELECT
    ranked.id,
    ranked.appointment_no,
    ranked.tenant_id,
    ranked.marketing_lead_id,
    ranked.customer_id,
    ranked.assigned_employee_id,
    ranked.budget_estimate_id,
    ranked.preferred_visit_date,
    ranked.preferred_visit_period,
    ranked.community,
    ranked.status,
    ranked.confirmed_visit_at,
    ranked.created_at,
    ranked.updated_at,
    ranked.version
  FROM ranked
  WHERE ranked.appointment_rank = 1
  ORDER BY ranked.marketing_lead_id;
END;
$function$;
...
REVOKE ALL ON FUNCTION public.list_tenant_douyin_lead_latest_appointments(
  uuid, uuid[]
) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.list_tenant_douyin_lead_latest_appointments(
  uuid, uuid[]
) TO service_role;
...
COMMENT ON FUNCTION public.list_tenant_douyin_lead_latest_appointments(
  uuid, uuid[]
) IS '批量返回租户抖音线索的最新一条量房预约摘要，最多100个唯一线索';
COMMENT ON FUNCTION public.convert_douyin_lead_to_customer(
  uuid, uuid, uuid, integer, uuid, uuid, boolean
) IS '按授权预检结果原子转换抖音线索，拒绝客户TOCTOU不一致';
