-- Ensure each project has only one authoritative running workflow instance.
--
-- Root cause:
--   The original runtime uniqueness constraint included definition_id, so
--   republishing/rebuilding a project workflow under a new definition could
--   leave an older definition's instance running for the same project. Reads
--   that ordered by updated_at could then select the late-touched legacy
--   instance and expose old constructing actions.

WITH ranked_running_project_instances AS (
  SELECT
    instance.*,
    row_number() OVER (
      PARTITION BY instance.tenant_id, instance.subject_type, instance.subject_id
      ORDER BY
...
instances_to_cancel AS (
  SELECT *
  FROM ranked_running_project_instances
  WHERE running_rank > 1
),
canceled_nodes AS (
  UPDATE public.workflow_instance_nodes node
  SET
    status = 'canceled',
    output = jsonb_build_object(
      'source', 'single_running_project_workflow_instance_migration',
      'reason', 'duplicate_running_project_workflow_instance'
    ),
    completed_at = now()
  FROM instances_to_cancel old_instance
  WHERE node.tenant_id = old_instance.tenant_id
    AND node.instance_id = old_instance.id
...
canceled_tasks AS (
  ...
  SET
    ...
    completed_at = now()
  FROM instances_to_cancel old_instance
  WHERE task.tenant_id = old_instance.tenant_id
    AND task.instance_id = old_instance.id
    AND task.status = 'pending'
  RETURNING task.id
),
canceled_instances AS (
  UPDATE public.workflow_instances instance
  SET
    status = 'canceled',
    completed_at = now()
  FROM instances_to_cancel old_instance
  WHERE instance.id = old_instance.id
  RETURNING instance.*
),
inserted_cancel_logs AS (
  ...
  SELECT
    ...
    NULL
  FROM canceled_instances instance
  RETURNING id
),
authoritative_instances AS (
  SELECT *
  FROM ranked_running_project_instances
  WHERE running_rank = 1
),
pending_task_counts AS (
  ...
  GROUP BY task.tenant_id, task.instance_id
)
...
SELECT
  ...
  instance.status,
  instance.current_node_key,
  nullif(btrim(instance.current_node_snapshot->>'title'), ''),
  nullif(btrim(instance.current_node_snapshot->>'business_kind'), ''),
  coalesce(task_counts.pending_task_count, 0)
FROM authoritative_instances instance
LEFT JOIN pending_task_counts task_counts
  ON task_counts.tenant_id = instance.tenant_id
 AND task_counts.instance_id = instance.id
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_workflow_instances_one_running_project_subject
ON public.workflow_instances(tenant_id, subject_type, subject_id)
WHERE status = 'running'
  AND subject_type = 'project';

DROP INDEX IF EXISTS idx_workflow_instances_project_subject_started;
CREATE INDEX idx_workflow_instances_project_subject_started
...
COMMENT ON INDEX public.idx_workflow_instances_one_running_project_subject
IS 'Guarantees one running workflow runtime per project subject across workflow definitions.';
...
COMMENT ON FUNCTION public.rebuild_workflow_subject_runtime(
  ...
  boolean,
  boolean
) IS 'Rebuilds a subject workflow runtime and cancels any existing running runtime for the same subject before starting the replacement.';
