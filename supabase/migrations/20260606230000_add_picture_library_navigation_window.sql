...
AS $$
  ...
  resolved_context AS (
    SELECT
      coalesce(p_category_id, (SELECT category_id FROM current_categories LIMIT 1)) AS category_id,
      least(greatest(p_limit, 1), 5) AS window_limit
  ),
  ordered_assets AS (
    SELECT
      pa.*,
  ...
  nav_positions AS (
    SELECT 'current'::text AS nav_position, cp.row_number, 0 AS nav_offset
    FROM current_position cp
    UNION ALL
    SELECT 'prev'::text AS nav_position, cp.row_number - offsets.nav_offset, offsets.nav_offset
    FROM current_position cp
    CROSS JOIN resolved_context rc
    CROSS JOIN generate_series(1, rc.window_limit) AS offsets(nav_offset)
    WHERE p_direction IN ('prev', 'both')
      AND cp.row_number - offsets.nav_offset >= 1
    UNION ALL
    SELECT 'next'::text AS nav_position, cp.row_number + offsets.nav_offset, offsets.nav_offset
    FROM current_position cp
    CROSS JOIN resolved_context rc
    CROSS JOIN generate_series(1, rc.window_limit) AS offsets(nav_offset)
  ...
  selected_assets AS (
    SELECT np.nav_position, np.nav_offset, oa.*
  ...
  bounds AS (
    SELECT
      ...
      (
        ...
        FROM selected_assets prev_asset
        WHERE prev_asset.nav_position = 'prev'
        ORDER BY prev_asset.nav_offset DESC
        LIMIT 1
      ) AS prev_cursor,
      (
        SELECT next_asset.id
        FROM selected_assets next_asset
        WHERE next_asset.nav_position = 'next'
        ORDER BY next_asset.nav_offset DESC
        LIMIT 1
      ) AS next_cursor
