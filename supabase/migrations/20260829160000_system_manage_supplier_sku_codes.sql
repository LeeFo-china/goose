...
CREATE FUNCTION public.command_supplier_sku_v3(
  p_action text,
...
BEGIN
  v_generated_code := CASE
    WHEN p_ownership_scope = 'tenant' THEN 'TS-'
    WHEN p_ownership_scope = 'platform' THEN 'PS-'
    ELSE ''
  END || upper(replace(p_sku_id::text, '-', ''));

  IF p_action = 'create' THEN
    v_effective_payload := (p_payload - 'sku_code') ||
      jsonb_build_object('sku_code', v_generated_code);
  ELSIF p_action = 'update' THEN
    v_effective_payload := p_payload - 'sku_code';
  ELSE
    v_effective_payload := p_payload;
  END IF;

  BEGIN
    RETURN public.command_supplier_sku_v2(
      p_action,
      p_ownership_scope,
      p_tenant_id,
  ...
  IF NOT FOUND
    OR v_event.tenant_id IS DISTINCT FROM p_tenant_id
  ...
  v_current_request := jsonb_build_object(
    'action', p_action,
  ...
  v_previous_request := v_event.from_state -> '_request';

  IF v_previous_request IS NULL
    OR jsonb_typeof(v_previous_request) <> 'object'
    OR jsonb_typeof(v_previous_request -> 'payload') <> 'object'
  THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001', MESSAGE = 'SUPPLIER_IDEMPOTENCY_CONFLICT';
  END IF;

  v_previous_payload := v_previous_request -> 'payload';
  IF p_action = 'create' THEN
    v_previous_payload := (v_previous_payload - 'sku_code') ||
      jsonb_build_object('sku_code', v_generated_code);
  ELSIF p_action = 'update' THEN
    v_previous_payload := v_previous_payload - 'sku_code';
  END IF;
  v_previous_request := jsonb_set(
    v_previous_request,
    '{payload}',
    v_previous_payload,
    false
  ...
  IF v_previous_request IS DISTINCT FROM v_current_request THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001', MESSAGE = 'SUPPLIER_IDEMPOTENCY_CONFLICT';
  END IF;
...
CREATE FUNCTION public.command_supplier_purchasable_product_v2(
  p_product_id uuid,
  p_sku_id uuid,
  p_tenant_id uuid,
  p_tenant_supplier_id uuid,
...
DECLARE
  ...
  v_previous_parent_request jsonb;
...
BEGIN
  ...
  v_effective_sku := (p_sku - 'sku_code') || jsonb_build_object(
    'sku_code', 'TS-' || upper(replace(p_sku_id::text, '-', ''))
  );
  ...
  IF NOT (v_effective_sku ? 'sku_code')
    OR NOT (v_effective_sku ? 'name')
    OR NOT (v_effective_sku ? 'purchase_unit_id')
    OR NOT (v_effective_sku ? 'spec_values')
    OR jsonb_typeof(v_effective_sku -> 'sku_code') <> 'string'
    OR jsonb_typeof(v_effective_sku -> 'name') <> 'string'
    OR jsonb_typeof(v_effective_sku -> 'purchase_unit_id') <> 'string'
    OR jsonb_typeof(v_effective_sku -> 'spec_values') <> 'object'
    OR NULLIF(btrim(v_effective_sku ->> 'name'), '') IS NULL
    OR btrim(v_effective_sku ->> 'name') IS DISTINCT FROM (v_effective_sku ->> 'name')
    OR char_length(v_effective_sku ->> 'name') > 160
    OR (v_effective_sku ->> 'sku_code') IS DISTINCT FROM
      'TS-' || upper(replace(p_sku_id::text, '-', ''))
    OR COALESCE(v_effective_sku ->> 'purchase_unit_id', '') !~*
      '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    OR (
      v_effective_sku ? 'specification'
      AND jsonb_typeof(v_effective_sku -> 'specification') NOT IN ('string', 'null')
    )
    OR (
      jsonb_typeof(v_effective_sku -> 'specification') = 'string'
      AND (
        NULLIF(btrim(v_effective_sku ->> 'specification'), '') IS NULL
        OR btrim(v_effective_sku ->> 'specification') IS DISTINCT FROM
          (v_effective_sku ->> 'specification')
      )
    )
    OR (
      v_effective_sku ? 'model'
      AND jsonb_typeof(v_effective_sku -> 'model') NOT IN ('string', 'null')
    )
    OR (
      jsonb_typeof(v_effective_sku -> 'model') = 'string'
      AND (
        NULLIF(btrim(v_effective_sku ->> 'model'), '') IS NULL
        OR btrim(v_effective_sku ->> 'model') IS DISTINCT FROM (v_effective_sku ->> 'model')
      )
    )
    OR (
      v_effective_sku ? 'batch_managed'
      AND jsonb_typeof(v_effective_sku -> 'batch_managed') <> 'boolean'
    )
    OR (
      v_effective_sku ? 'color_managed'
      AND jsonb_typeof(v_effective_sku -> 'color_managed') <> 'boolean'
    )
    OR (
      v_effective_sku ? 'serial_managed'
      AND jsonb_typeof(v_effective_sku -> 'serial_managed') <> 'boolean'
    )
    OR EXISTS (
      SELECT 1
      FROM jsonb_each(v_effective_sku -> 'spec_values') AS spec(key, value)
    ...
    OR EXISTS (
      SELECT 1
      FROM jsonb_object_keys(v_effective_sku) AS field(key)
      WHERE field.key NOT IN (
  ...
  v_purchase_unit_id := (v_effective_sku ->> 'purchase_unit_id')::uuid;
  ...
  IF FOUND THEN
    ...
    IF jsonb_typeof(v_previous_parent_request) = 'object'
      AND jsonb_typeof(v_previous_parent_request -> 'sku') = 'object'
    THEN
      v_previous_parent_request := jsonb_set(
        v_previous_parent_request,
        '{sku}',
        ((v_previous_parent_request -> 'sku') - 'sku_code') ||
          jsonb_build_object(
            'sku_code',
            'TS-' || upper(replace(p_sku_id::text, '-', ''))
          ),
    ...
    IF v_event.tenant_id IS DISTINCT FROM p_tenant_id
      ...
      OR v_event.command NOT IN (
        'supplier_purchasable_product_v1:create',
        'supplier_purchasable_product_v2:create'
      )
      OR v_previous_parent_request IS DISTINCT FROM v_parent_request
    THEN
      RAISE EXCEPTION USING
        ERRCODE = 'P0001',
  ...
  WHERE sku.id = p_sku_id
    OR (
      ...
      AND upper(btrim(sku.sku_code)) = upper(btrim(v_effective_sku ->> 'sku_code'))
    )
  ORDER BY sku.id
  FOR UPDATE;
  IF FOUND THEN
...
REVOKE ALL ON FUNCTION public.command_supplier_purchasable_product_v2(
  uuid, uuid, uuid, uuid, uuid, jsonb, jsonb, jsonb, uuid, uuid, text
) FROM PUBLIC, anon, authenticated, service_role;

GRANT EXECUTE ON FUNCTION public.command_supplier_purchasable_product_v2(
  uuid, uuid, uuid, uuid, uuid, jsonb, jsonb, jsonb, uuid, uuid, text
) TO service_role;

COMMENT ON FUNCTION public.command_supplier_sku_v3(
  text, text, uuid, uuid, uuid, uuid, uuid, integer, jsonb, uuid, uuid, text
) IS 'System-manages supplier SKU codes and preserves v2 idempotent replay compatibility.';

REVOKE ALL ON FUNCTION public.command_supplier_sku_v3(
  text, text, uuid, uuid, uuid, uuid, uuid, integer, jsonb, uuid, uuid, text
) FROM PUBLIC, anon, authenticated, service_role;

GRANT EXECUTE ON FUNCTION public.command_supplier_sku_v3(
  text, text, uuid, uuid, uuid, uuid, uuid, integer, jsonb, uuid, uuid, text
) TO service_role;

-- Keep v2 executable during rollout and rollback. Remove that grant only after
-- every deployed API version calls v3; revoking it here would break the old API
-- between migration application and the new container becoming healthy.
