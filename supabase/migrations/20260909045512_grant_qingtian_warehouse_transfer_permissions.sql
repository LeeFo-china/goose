...
-- Baseline: both grants absent. Rollback: close warehouse_transfers_enabled,
...
BEGIN
  ...
  IF v_tenant.name IS DISTINCT FROM '固始晴天装饰工程有限公司' OR v_tenant.status IS DISTINCT FROM 'active' THEN
    RAISE EXCEPTION 'QINGTIAN_TRANSFER_GRANT_TENANT_MISMATCH';
  END IF;
  ...
  IF NOT FOUND OR v_role.tenant_id IS DISTINCT FROM v_tenant_id
    ...
    RAISE EXCEPTION 'QINGTIAN_TRANSFER_GRANT_ROLE_MISMATCH';
  END IF;
  PERFORM id FROM public.permissions
    WHERE code IN('inventory.transfer.manage','inventory.transfer.approve') ORDER BY id FOR SHARE;
  IF (SELECT count(*) FROM public.permissions WHERE status='active' AND module='inventory' AND resource='transfer'
    AND ((code='inventory.transfer.manage' AND action='manage')
      OR (code='inventory.transfer.approve' AND action='approve')))<>2 THEN
    RAISE EXCEPTION 'QINGTIAN_TRANSFER_GRANT_PERMISSION_MISMATCH';
  END IF;
  ...
  IF (SELECT count(*) FROM public.role_permissions rp JOIN public.permissions p ON p.id=rp.permission_id
    WHERE rp.role_id=v_role_id AND p.code IN('inventory.transfer.manage','inventory.transfer.approve')
      AND rp.access_scope='all')<>2 THEN
    RAISE EXCEPTION 'QINGTIAN_TRANSFER_GRANT_SCOPE_MISMATCH';
  END IF;
END;
$$;
COMMIT;
