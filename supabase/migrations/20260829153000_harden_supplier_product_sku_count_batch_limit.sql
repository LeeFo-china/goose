-- Rollback: forward-only. Restore the previous function body in a follow-up migration.
BEGIN;

SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '5min';

CREATE OR REPLACE FUNCTION public.list_supplier_product_sku_counts(
  p_supplier_id uuid,
  p_product_ids uuid[],
...
BEGIN
  ...
  IF COALESCE(cardinality(p_product_ids), 0) > 100 THEN
