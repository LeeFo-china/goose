...
DECLARE
  ...
  v_subscription public.tenant_billing_subscriptions%ROWTYPE;
  v_paid_invoice public.tenant_subscription_invoices%ROWTYPE;
  v_plan public.tenant_billing_plans%ROWTYPE;
...
BEGIN
  ...
  WHERE tenant_id = v_tenant_id
    AND metadata->>'source' = 'mini_program_billing_lock_joint_test';
  ...
  v_next_period_start := v_paid_invoice.period_end;
  v_next_period_end := (v_paid_invoice.period_end + interval '1 month')::date;
  v_next_due_at := v_paid_invoice.period_end::timestamptz;

  UPDATE public.tenant_billing_subscriptions
  SET
    current_period_start = v_paid_invoice.period_start,
    current_period_end = v_paid_invoice.period_end,
    next_charge_at = v_next_due_at,
    metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
      'source', 'mini_program_billing_lock_joint_test',
      'repaired_by', '2026-07-14-gushi-qingtian-subscription-billing-repair',
      'repaired_at', now()
    )
  WHERE id = v_subscription.id;
  ...
  VALUES (
    ...
    v_next_period_end,
    v_next_due_at,
    v_plan.monthly_fee_credits,
    'upcoming',
    v_next_due_at -
      make_interval(days => greatest(coalesce(v_plan.reminder_days_before_due, 0), 0)),
    jsonb_build_object(
      'source', '2026-07-14-gushi-qingtian-subscription-billing-repair',
      'previous_paid_invoice_id', v_paid_invoice.id
    )
  )
  ON CONFLICT (tenant_id, period_start, period_end) DO NOTHING;
END;
