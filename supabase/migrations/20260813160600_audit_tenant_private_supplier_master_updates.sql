-- Rollback: forward-only. Replace the RPC in a new migration to change audit
-- behavior. Existing supplier_command_events remain immutable audit facts.
...
CREATE OR REPLACE FUNCTION public.update_tenant_private_supplier_master(
  p_tenant_id uuid,
...
BEGIN
  ...
  v_from_state := jsonb_build_object(
    ...
    'unified_social_credit_code', v_supplier.unified_social_credit_code,
  ...
  v_audit_key := 'private-master-update:' || gen_random_uuid()::text;
