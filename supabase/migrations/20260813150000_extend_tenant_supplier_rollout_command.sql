-- Rollback: forward-only. Disable the rollout API and all four flags through
-- the command in reverse order, then ship a new migration that revokes and
-- drops this function. Keep tenant data and command events for audit; never
-- repair or delete rollout state manually. If lock or statement timeout is
-- reached during the release window, this entire transaction rolls back.
