-- Rollback: revoke EXECUTE, then drop only
-- public.list_tenant_douyin_lead_assignee_filter_options(uuid, uuid[], integer, integer, text).
BEGIN;

SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '30s';

CREATE FUNCTION public.list_tenant_douyin_lead_assignee_filter_options(
  p_tenant_id uuid,
...
BEGIN
  ...
  v_keyword := pg_catalog.nullif(pg_catalog.btrim(p_keyword), '');
...
COMMENT ON FUNCTION public.list_tenant_douyin_lead_assignee_filter_options(
  uuid, uuid[], integer, integer, text
) IS '按精确可见员工集合分页查询抖音线索历史负责人筛选项';
