...
-- Rolling-compatible validation: existing releases may keep deployment_key only,
-- while newly uploaded releases pin the server-controlled deployment environment.
...
BEGIN
  IF p_installation_id IS NULL
    ...
    OR jsonb_object_length(p_ext_json -> 'ext') NOT IN (1, 2)
    OR (p_ext_json -> 'ext')
...
-- 20260719102000_create_douyin_miniapp_releases.sql. Existing rows remain valid.
