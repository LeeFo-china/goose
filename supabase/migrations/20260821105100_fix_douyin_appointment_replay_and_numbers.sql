-- Forward repair only:
-- 1. The applied 105000 migration remains immutable.
-- 2. This migration requires an empty follow-up table; it never invents replay
--    payloads for historical rows.
-- 3. Roll back by revoking the affected commands and shipping another reviewed
--    forward migration. Preserve appointment and lead history.
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM public.douyin_lead_follow_ups
  ) THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'DOUYIN_LEAD_FOLLOW_UP_REPLAY_REPAIR_REQUIRES_EMPTY_TABLE';
  END IF;
END;
$block$;
...
ALTER TABLE public.douyin_lead_follow_ups
ADD CONSTRAINT douyin_lead_follow_ups_result_payload_check
CHECK (
  (
    ...
    AND result_payload ->> 'appointment_id' =
      douyin_measurement_appointment_id::text
...
COMMENT ON COLUMN public.douyin_lead_follow_ups.result_payload IS
  '首发跟进命令的稳定结果；重放时仅覆盖 idempotent=true';
