...
- [ ] `project_logs` 增加或回填 `tenant_id`。
- [ ] 日志创建时从项目继承 `tenant_id`。
- [ ] 日志列表按 `tenant_id` 过滤。
- [ ] 日志详情校验所属租户。
- [ ] 日志评论按 `tenant_id` 或通过日志归属隔离。
- [ ] 客户侧日志接口校验客户与项目同租户。

### 2. 费用审批

- [ ] `expense_requests` 增加或回填 `tenant_id`。
- [ ] 创建费用申请时写入当前租户。
- [ ] 项目费用申请校验项目属于当前租户。
- [ ] 审批人只能来自当前租户员工。
- [ ] 费用列表、详情、审批操作全部加租户边界。
...
- [ ] `project_acceptances` 增加或回填 `tenant_id`。
- [ ] 验收项、操作记录、open ticket 关联租户上下文。
- [ ] 发起验收时从项目继承租户。
- [ ] 领导复核只能操作当前租户项目。
- [ ] 客户确认校验客户与验收单同租户。
- [ ] 短信 ticket 持久化 `tenant_id`。
- [ ] 异步短信发送带租户上下文。
...
- [ ] `system_settings` 支持平台级和租户级。
- [ ] 配置读取优先租户，缺失回退平台默认。
- [ ] 腾讯云 SIP、AI API Key、AI endpoint、默认模型、对象存储基础配置保持平台级。
- [ ] 短信签名、H5 品牌色、自定义 Logo、租户文案偏好属于租户级配置。
- [ ] 配置审计记录 `tenant_id`。
