...
- 员工首页 `scheduleDashboardSectionLoads()` 不要在 `/employee/bootstrap` resolve 后立刻请求客户/项目列表；建议延迟 `4s - 5s`，或在首屏统计、任务摘要渲染完成后的 idle/后台窗口触发。
- 如果用户在 `4s - 5s` 延迟窗口内主动进入完整客户列表或项目列表，允许立即请求完整列表页默认接口；不要为了首页 home 列表等待后台预热。
- 完整列表页、搜索页、筛选页不要传 `mode=home`，继续使用默认接口以获得完整字段和精确分页。
- 首页不要依赖 home 模式返回精确总数；如果需要“查看全部”，跳转完整列表页后重新拉默认接口。

小程序端验收口径：

- 首屏进入员工首页时，客户/项目 home 列表请求不得阻塞首页框架、统计、任务摘要渲染。
