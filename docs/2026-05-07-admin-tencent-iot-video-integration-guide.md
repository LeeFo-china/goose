...
`devices` 是腾讯云设备列表，即使设备刚创建、尚未完成 SIP 注册、还没有产生通道，也会出现在这里。`list` 是通道列表，只有设备注册并上报通道后才会有数据。Admin 页面应展示为“腾讯云设备与通道”的层级表：设备是主行，通道作为设备下一级子行；绑定动作仍发生在通道层。
