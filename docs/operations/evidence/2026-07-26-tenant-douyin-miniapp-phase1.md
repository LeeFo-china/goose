...
Chrome 与 Codex 插件运行时恢复后，使用开发库合成验收员工完成本地 Admin
登录，并打开真实 `/douyin-miniapp/workspace` 页面。手机号仅用于本地登录，
未写入证据文档、截图或项目文件。

真实页面确认：

- 页面、侧栏与权限导航均正常加载；
- 内部租户名称显示为“5H 验收租户 AAA”；
- 小程序公开品牌独立显示为“5H 验收租户 A”；
- 授权状态为“已授权”；
- 最近版本状态为“体验测试中”；
- 精选案例、在建工地、有效服务区域均为 1；
- 页面控制台为 0 error、0 warning；
- 未展示平台密钥、令牌或其他敏感字段。
...
```json
{
  "innerWidth": 1800,
  "innerHeight": 907,
  "documentClientWidth": 1800,
  "documentScrollWidth": 1785,
...
`2026-07-26-tenant-douyin-miniapp-phase1-wide.jpg`
...
`2026-07-26-tenant-douyin-miniapp-phase1-375.jpg`

手机视口下状态、按钮、公开资料和内容统计均按纵向排列，未出现横向滚动。
验收后已清除临时视口覆盖并恢复默认 1800px 视口。

### 状态覆盖边界

开发库当前只有一条真实活跃商家安装，且公开资料已发布、最近版本处于体验测试。
为避免仅为截图修改开发库业务状态，以下状态通过
`TenantDouyinMiniappWorkspace` 的 React SSR 契约测试验证：

- 无 `douyin_miniapp.read` 权限；
- 未授权；
- 公开资料待审核；
- 最近版本审核驳回；
- 超长公开品牌名；
...
真实 Chrome 覆盖活跃安装宽屏、375px 窄屏、真实权限导航、真实租户数据与实际
CSS 布局；SSR 契约覆盖不适合在开发库临时构造的离散状态。两类证据共同满足
Phase 1 浏览器 smoke 的状态与布局边界。

## Phase 1 退出门禁结论

通过。只读租户工作台具备租户安全响应、正确权限、内部名称与公开品牌分离、
服务区域可见性和单租户一个活跃商家安装保护；真实 Chrome 宽屏及 375px
窄屏均无横向溢出。
