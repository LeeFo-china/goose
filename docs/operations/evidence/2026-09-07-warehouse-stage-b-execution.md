...
**四步尚未全部完成，禁止合并 main、开放补货开关或生产发布。**
用户后续确认开发 API/Admin 候选部署，不等于最终验收放行；开发迁移、类型同步和
候选流水线的最新证据见[开发执行记录](./2026-09-06-warehouse-procurement-inventory-stage-b-dev.md)。
下文按实施阶段保留历史检查结果，“未应用/未部署”需结合该最新记录判断。
...
- [ ] 3. Admin 库存余额、流水、菜单、采购单目的地、批次补货及财务目的地均已实现并完成下述本地 UI 验收；真实联调和整体最终审查仍待完成，不开放补货。
- [ ] 4. 获准开发迁移已应用，593 项 Local/Remote 对齐，31 个生成类型块已同步；完整真实接口/隔离/性能/原项目回归、最终审查、合并和安全清理仍待完成。严格数据库写入审计仍有 17 个上游既存候选，不能记为整体通过。
...
- 沿用应付和付款申请页面、API 与精确金额工具，接入项目/仓库目的地筛选、同仓应付建单、
  提交、独立审批、分次付款、关闭尾款和取消占用；原项目完整流程保留。
- 跨仓、项目/仓库混选和跨项目混选均拒绝；目的地从应付派生并冻结。HTTP 参数使用
  `destination_type`/`warehouse_id`，仓库选项可分页查询停用仓库，保留 20 条之后的付款记录。
- 仓库操作叠加仓库管理权限；审批与付款不额外要求财务管理。无应付查看权限的申请管理角色
  仍走专用批量事实深链；不查询无权 settings/筛选接口。停用仓库、关闭补货不阻断既有结算。
- 保留凭证、冻结发票门禁、金额及版本规则；仓库付款不发项目财务刷新，项目行为不变。
- 未确认请求保留原 UUID、payload、version、key，按租户/用户隔离 sessionStorage；
  原金额和目的地只作本地冻结元数据，不进入 HTTP/指纹。截断 200、错回执、断连以及
  未确认请求重试遭拒都不能释放原请求；已确认成功后的读失败仅重新读取。
- 规格审查真实复现异常申请金额/付款增量回执被错误接受，修复后按冻结金额基线及真实 SQL
  状态约束验证。旧 pending 缺少新增金额基线时保留 key、停止自动写入，不凭最新读取补造原意图。
- 浏览器 RED/GREEN 补齐两种恢复边界：reload 后先查看同单/另一单可安全关闭并恢复原请求；
  A 重试响应延迟期间打开 B，A 回执不能覆盖 B 标题/分配/原因，也不再启动旧 A 详情读取。
- 质量审查复现卸载/生命周期换代后，迟到的旧 A 回执会清除或覆盖新 B 的未确认存储。
  以生命周期代次隔离旧回执，过时代次返回 null，不更新 storage、内存或调用方。
  卸载/槽位往返 × 迟到成功/不确定的四格真实 hook/sender 测试 RED→GREEN，B 重挂载恢复不变。
- 根代理在最终冻结候选上独立逐文件运行两个财务目录 14 个 Bun 文件：93 tests、442 assertions；
  `bun run check` 通过（1,449 个 TS/TSX 文件大小门禁及类型检查），完整 17 个 Playwright
  场景通过（1.8 分钟），`bun run build` 通过（95 页），总执行 exit 0。
- 已实际查看根代理生成的付款申请桌面/375px、应付桌面与恢复另一单详情截图。桌面仓库页码
  不再竖排，375px 页脚可见，表格保留内部横滚，未确认操作说明与安全关闭入口可见。
- 完整规格审查及两项 Important 的定向复核已通过；质量生命周期 Important 定向复核也通过
  （独立 3 文件、29 tests / 148 assertions），无剩余发现。本财务 Admin 单元按精确文件提交。
- 以上是独立 loopback HTTP fixture（3026/3995）上的 Admin 契约验证，非真实开发 API 或
  数据库业务联调。未开启仓库采购、未改既有应付、未发布或合并 main；开发状态见
  [开发库记录](./2026-09-06-warehouse-procurement-inventory-stage-b-dev.md)。
...
- [目录外仓与补充 ACL](./2026-09-07-warehouse-stage-b-catalog-scope-acl.md)收紧旧目录夹具：双方采购门禁开启且正向可读后，双向外仓只接受 `WAREHOUSE_NOT_FOUND`；补仓库创建/修改、履约确认、正式收货及默认仓库 helper 的实际权限检查。双阶段审查通过，根代理在该候选下全 21 个夹具通过；仍非完整 HTTP 权限验收。
