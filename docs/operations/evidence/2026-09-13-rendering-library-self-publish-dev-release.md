...
- 下方追加验证覆盖保存的最终公开 URL，但尚未由双端已认证 API 响应核对 `image_url` 字段；CDN 缓存与隐藏后的旧 URL 可访问性没有验收。隐藏只保证新 API 请求不返回该记录，不撤销已缓存图片。
- 合成样本的素材记录已隐藏，私有源与公开副本对象仍保留以供审计；本次没有执行对象删除或回滚演练。
- 微信 `orange` 页面、抖音小程序页面、客户照片上传/AI 生成、方舟计费调用及装修建议均不在本次发布范围。

## 2026-09-13 双端验收追加检查

在固定开发 VM、project ref 和数据库 host 守卫下，只读查询上述已隐藏合成样本的公开文件 URL、校验值和平台公开基地址；URL、对象路径及身份值只在进程内使用，未输出。对**数据库保存的最终公开 URL**直接 HTTPS GET 为 200，响应为 WebP，SHA-256 与公开文件记录一致，URL origin 与平台配置一致。该结果补充了上文 COS 原站测试，但不证明真实客户端 DTO 或 CDN 缓存刷新。

同租户只读计数：有效抖音安装为 1；当前有效、已选择该装修公司的微信 visitor 上下文为 0。开发 Admin 会话访问微信目录返回 401 `UNAUTHORIZED`，访问抖音目录返回 401 `TOKEN_INVALID`；无 token 的双端目录仍返回 401。这些是渠道隔离负向检查，不是客户端正向登录验收。

正向测试仍需由正常小程序登录取得一次性微信 `wx.login` code 与抖音 `tt.login` code，并选择同一测试租户；不得用 Admin token、伪造 JWT、手工补数据库上下文或把真实 token 写入工单。取得有效会话后再建立非客户已发布样本，核对两端列表/详情、分页、跨租户、隐藏后列表消失与详情 404。本次追加检查没有新增或修改远端数据，也没有改动 `orange`。
