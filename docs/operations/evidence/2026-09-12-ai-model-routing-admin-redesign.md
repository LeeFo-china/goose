...
**结论：** 计划规定的本地 API、Admin 和仓库卫生检查均通过；Task 10 已完成开发数据库 migration、API 后 Admin 的开发发布及无付费配置 smoke。真实业务端到端生图仍不属于本记录。

**证据级别：** 本地可复跑摘要 + 开发环境不可变发布证据。验证批次于 2026-09-12 完成，首个证据提交时间为 `2026-09-12T15:26:06+08:00`；浏览器截图文件时间为 `15:22:04` 至 `15:22:45 +08:00`，环境指纹采集于 `15:28:40 +08:00`。Task 10 的 workflow run ID、开发镜像摘要、部署 revision、健康 gate 和真实 API smoke 见第 7、8 节。
...
第 2 节的本地证据单独不能证明开发数据库或线上运行状态；第 7、8 节已经补充 Task 10 的开发 migration、发布和无付费 smoke。整份证据仍**不证明**真实火山方舟密钥有效、真实 Seedream 请求可成功、Admin 配置已被微信/抖音小程序业务链消费，或“上传房型图 -> 生图 -> 装修建议”端到端流程已经打通。Admin 配置验证不等于业务端到端生图验证，剩余边界需要后续小程序联调和经授权的计费 smoke 继续确认。

## 7. Task 10 开发 migration 与发布证据

### 7.1 冻结提交与安全发布分支

功能分支证据提交前的源提交为 `00666aa70b31c1f5e7cba964d3fa88f042455919`，tree 为 `470f67197502b366a8e1e4881eb01a10847d2f2b`。发布分支 `release/ai-model-routing-admin-dev-20260912` 冻结在 `155e17f4fee9c287f88328acb056e1400d01703e`，其 tree 同样为 `470f67197502b366a8e1e4881eb01a10847d2f2b`，因此发布内容与已验证源内容逐文件一致。

首次直接推送原功能分支时，GitHub push protection 因较早历史提交中的测试 fixture 命中疑似密钥而拒绝。当前 tree 不含该值，本轮没有使用 GitHub 提供的绕过入口，也没有改写本地功能分支的细粒度历史；发布分支使用以 `origin/main` 基线 `b1d468a1d5268c968e65a747e90ca0294d1cd1de` 为父提交、tree 完全相同的干净 snapshot commit。远端 `refs/heads/release/ai-model-routing-admin-dev-20260912` 已核对为上述 release SHA。

### 7.2 Migration 运行结果

所有 migration workflow 的 `headSha` 均为同一个 release SHA `155e17f4fee9c287f88328acb056e1400d01703e`，结论均为 `success`：

| 阶段 | GitHub Actions run | 关键结果 |
|---|---:|---|
| 首次 plan | `34681267187` | Remote/Local 迁移数均为 616；唯一 pending 为 `20260912100000` |
| apply | `34681297833` | applied 仅 `20260912100000`；迁移数由 616 增至 617，最新版本为 `20260912100000` |
| apply 后 plan | `34681332068` | Remote/Local 均为 617，最新版本 `20260912100000`，pending 为 0 |

以上结果证明开发项目 `fclnkyatvfvmzgzdqlba` 的 migration 历史已对齐。本次数据库变更只通过仓库 migration 和受控 workflow 应用，没有远端手工 DDL/DML。若后续发现数据库问题，继续采用前向 migration 修复，不直接删除本次新增对象或数据。

### 7.3 开发发布运行结果

开发发布 run `34681359729` 的 `headSha` 为 `155e17f4fee9c287f88328acb056e1400d01703e`，最终结论为 `success`。可信编排中的 build、migration-history gate、API deploy/readiness、Admin deploy/readiness 均为 `success`，发布顺序为 API 成功并通过 barrier 后再部署 Admin。

不可变镜像证据：

- API：`sha256:20b7a32a2a8f3cbca24d0e002736d58c76a61f655674f98a49625fdb1354420b`
- Admin：`sha256:22e14466d88337d7dcd7238fcaf7f00f561ec9061184014076b767dbe9854ffc`

两个 image manifest 均记录 target `development`、commit `155e17f4fee9c287f88328acb056e1400d01703e` 和 build run `34681359729`。部署 gate 已校验容器配置的 digest、OCI revision、run ID、`running` 状态和 `healthy` 健康状态；Admin 发布阶段的登录及 project-health 分页 smoke 同样成功。

release workflow 附带的自动 migration evidence 使用工作流内部目标版本 `20260711120000`，只能证明该最低门槛存在且历史对齐；本记录以独立 apply 后 plan run `34681332068` 的 `latest=20260912100000`、`pending=0` 作为本次 migration 的最终证据。

## 8. 开发环境无付费 smoke

Smoke 通过开发 Admin fixture 账号完成，只访问 Gooes Admin/API；没有读取或输出真实密钥值，没有调用供应商验证、文本推理或 `/images/generations`。

### 8.1 只读检查

- 登录、当前用户、密钥元数据、供应商分页、场景分页和路由分页接口均返回 HTTP 200。
- `ARK_API_KEY` 返回 `source=database`、`status=configured` 和 `can_manage=true`，响应不含 `value` 字段。
- 火山方舟供应商为 active，协议 `openai_compatible`，Endpoint 为 `https://ark.cn-beijing.volces.com/api/v3`，密钥引用存在且未标记 invalid。
- 装修生图场景 `decoration_raw_drawing` 为 image 模态，要求 `text + image` 输入、至少 2 张参考图，并允许在 `runtime_status=not_connected` 时预配置。

### 8.2 配置保存与刷新

开发库中既有 Seedream 模型 `doubao-seedream-5-0-pro-260628` 原本被错误登记为 text 模态。通过 Admin API 将其改为 image 模态、输入能力改为 `text + image`，版本由 1 更新为 2；随后把装修生图主模型绑定到该模型，备用模型留空，路由版本由 2 更新为 3。

重新读取路由后，`primary_model_id`、供应商关系、image 模态、`timeout_ms=60000` 和 `response_format=null` 均保持；图片模型候选接口分页返回该 Seedream 模型。旧模型内部编码 `manual.openai_compatible_2.doubao_seedream_5_0_pro_260628` 被保留，没有为了符合新 `mdl_` 前缀破坏既有身份。

### 8.3 自定义场景与删除保护

- 创建停用路由 `DEV 模型路由验收 20260912-1555`，未绑定主/备模型；服务端返回场景编码 `scene_43ba8d94059146a1a564e04b4a811ce7` 和 route version 1。重新读取场景分页与路由分页后，该编码、名称、text 模态和停用路由均可见。场景注册表本身保持 active，路由为 inactive，因此不会进入运行时选路。
- 对存在模型引用的火山方舟供应商只发起一次删除请求，HTTP 409、错误码 `AI_PROVIDER_IN_USE`、提示改用停用；供应商和关联数据没有被删除，也没有重试。

### 8.4 验证边界

本轮尝试使用浏览器自动化查看已发布 Admin，但本地浏览器控制扩展连续超时，因此不声明已完成开发环境的视觉回归；视觉与交互布局由第 4 节的本地 31 项 Playwright 回归和截图覆盖，开发环境则由上述真实 API 合同 smoke 覆盖。

本轮仍未验证真实火山方舟鉴权、Seedream 计费生图、小程序调用或装修建议生成。此前第 3、5、6 节中“Task 10 未执行/待执行”的描述仅记录发布前证据边界；本节是其完成后的追加证据，并取代其中关于 migration 和开发发布尚未执行的结论。
