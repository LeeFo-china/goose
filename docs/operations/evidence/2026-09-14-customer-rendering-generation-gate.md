...
| 环节 | 本地证据 | 结论 |
| --- | --- | --- |
| 数据库迁移 | 隔离 Supabase 项目 `gooes_rendering_verify` 使用独立端口，从现有迁移链应用到 `20260914014930` 和 `20260914022533`；`supabase migration list --local` 的 Local/Remote 版本对齐，验证后已停止并移除该项目容器。两份新迁移与仓库原件 SHA-256 相同。旧迁移中 6 处 `CREATE/DROP INDEX CONCURRENTLY` 及 1 处要求特定生产租户的数据修复只在临时验证副本中调整，仓库原件未改。 | 证明新迁移可在完整基线后的隔离库执行；不证明生产项目已应用。生产链接尚未建立，`supabase migration list --linked` 不能取得远端状态。 |
...
1. 读取生产 `supabase migration list`，确认待执行版本、备份与回滚方案，按序由迁移流水线应用后再次核对 Local/Remote。不要手工执行 DDL/DML。破坏性回滚只能通过新的补偿 migration；关停入口与 Worker 不应删除历史任务或预占。
2. 核对生产 COS CI 开通状态、桶及对象的私有读策略、Worker CAM 的私有 GET/PUT/HEAD 与审核权限；用非客户测试图验证房间图签名读取、结果私有写入、HEAD/校验和、CI 批准与拒绝、匿名读取失败及签名到期。
3. 核对实际 Ark 模型、单次最高计费与日预算。当前无法从可信提供方回执取得任务级实际费用：提交后的任务按预占金额保守占用日预算，提交前明确失败记 0。必须先确定结算与超额处置规则，并完成付费测试任务的账本核对。
4. 先部署 API 和迁移且保持准入开关、`CUSTOMER_RENDERING_JOB_WORKER_ENABLED` 关闭；验证 401/404、幂等、跨主体、审核和失败恢复。之后只对一个测试租户开放 Worker 与准入，完成至少一次通过、一次拒绝、一次结果不明的人工对账演练，再考虑扩大范围。
