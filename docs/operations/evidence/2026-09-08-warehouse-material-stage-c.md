...
- [x] 最终相关静态检查、构建和独立审查；首次验收的路由清单缺口已在后续发布准备中解除，见下文。
...
首次 C 验收时的基线门禁问题：`tenant-service-route-inventory.test.ts` 的全量路由清单对 11 条现有抖音素材路由缺少登记，失败中没有 Stage C 路由。当时未越界修改抖音模块或宣称全量测试通过。用户随后授权发布准备，提交 `5a83ed54` 在核实专用 JWT／租户／主体鉴权后补齐测试登记及 44 次真实路由拒绝回归；完整路由清单现已通过，生产认证逻辑没有变更。最新状态和迁移边界见 [发布准备记录](../2026-09-08-warehouse-stage-c-release-readiness.md)。
