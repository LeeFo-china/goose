...
最新检查点（2026-09-09）：指定租户的真实员工 Chrome 领料、分次退料及库存／成本／应付对账通过，库存恢复 1 箱／88 元，项目成本净变动 0，应付与付款未变。合格收货摘要修复已应用，601/601 migration 对齐；本轮 API、隔离 SQL（含并发与权限）、25 次真实 HTTP→SQL、44 项桌面／移动页面回归通过，进入阶段 D1 请求契约批次。详见[阶段 C 开发实际验收](evidence/2026-09-09-warehouse-stage-c-dev-acceptance.md)。这不代表阶段 D 已上线，也不批准生产发布。

前序检查点（2026-09-08 16:27 后）：用户确认“执行 apply，开发发布”后，固定候选 `983b775e` 的原 4 条 C migration 已成功应用开发库，CLI Local / Remote 599 条完整对齐。刷新备份、兼容性及应用后权限检查通过；API / Admin 同候选开发发布成功，实际镜像 / revision / run ID 和健康状态核对通过，其他服务未发布。租户 C 开关仍关闭，没有写领退料业务单据，真实员工验收尚未执行。main 未合并，生产及 Orange 未改动。详见[开发 apply 与发布证据](evidence/2026-09-08-warehouse-stage-c-dev-release.md)。下方各检查点保留历史范围，不代表仍等待 apply 或部署。

前序检查点（2026-09-08 15:43 后）：用户确认下一步第 1、2 项并指定“固始晴天装饰工程有限公司”，已只读选择有现成权限的员工“风清扬”。固定候选 `983b775e` 已推送，开发 migration plan 成功，Local 599 / Remote 595，待执行仍仅原 4 条 C migration，applied=0。相关静态、构建、API / Admin 单测和隔离数据库回归重新通过。main 合入会触发全服务发布，故保留待合并；该检查点 C apply / 部署 / 开关均未执行。详见[固定候选与开发 plan 验收](evidence/2026-09-08-warehouse-stage-c-candidate-plan.md)。
