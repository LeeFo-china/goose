...
| Task 10 Step 3   | Partially complete            | OCR credentials, per-environment result keys and the Tencent-issued encrypted-ID PKCS#1 public key are configured. After the operator replaced the broad OCR permission with a custom policy, `bun run ocr:cam:readiness` returned `ready=true`: the three Phase 1 actions reached provider validation and `GeneralBasicOCR` was denied. The runtime behavior gate therefore passes. `ListAttachedUserAllPolicies` remains unavailable to the runtime child account, so a main account or independent CAM administrator must still read back direct, group, inherited and permissions-boundary grants before policy binding is considered audited. |
| Task 10 Step 4-5 | Complete                      | Tencent official demo business-license and bank-card samples passed end-to-end recognition, encrypted storage/readback, idempotent replay and successful-result cache checks. A blurred derivative was rejected safely. Authorized encrypted ID front/back samples also passed the tenant API path, including provider request encryption, response decryption, encrypted persistence/readback, idempotent replay and successful-result cache. Admin upload, field review, warning display and selected-field fill passed without saving or submitting an applyment.                                                                               |
| Task 10 Step 6   | In progress                   | Static, migration, production deployment, negative-boundary, official demo document, authorized encrypted-ID tenant API smoke, masked Admin UI, a successful least-privilege runtime CAM probe, development cleanup with 410 readback, production cleanup dry-run/apply, and the first enabled production schedule success are recorded. CAM control-plane policy-binding readback remains absent.                                                                                                                                                                                                                                                 |
| Task 10 Step 7   | Pending                       | Cannot declare Phase 1 production-ready before CAM control-plane policy-binding review. Development COS CORS also needs an approved exact-origin update if direct upload without proxy fallback is a release requirement.                                                                                                                                                                                                                                                                                                                                                                                                                          |
