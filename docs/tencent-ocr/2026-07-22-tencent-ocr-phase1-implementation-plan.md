...
| Task 10 Step 3   | Partially complete            | OCR credentials, per-environment result keys and the Tencent-issued encrypted-ID PKCS#1 public key are configured. The public key has passed backend format validation and provider request encryption. `bun run ocr:cam:readiness` proves the current OCR credential is still over-privileged because `GeneralBasicOCR` is reachable. A read-only CAM audit can identify the current child account, but `ListAttachedUserAllPolicies` returns `AuthFailure.UnauthorizedOperation`; a main account or independent CAM administrator must therefore create the replacement child user, attach only `deploy/tencent-ocr-phase1-cam-policy.json`, and replace the runtime credential before readiness can return `ready=true`. |
...
| Task 10 Step 6   | In progress                   | Static, migration, deployment, negative-boundary, official demo document, authorized encrypted-ID tenant API smoke, masked Admin UI, and a real development expired-record cleanup with 410 readback are recorded in the smoke record. Least-privilege CAM replacement and production cleanup scheduler evidence remain absent.                                                                                                                                                                                                                                                                                                                                                                                             |
| Task 10 Step 7   | Pending                       | Cannot declare Phase 1 production-ready before least-privilege CAM readiness and production cleanup dry-run/apply/scheduled-run evidence. Development COS CORS also needs an approved exact-origin update if direct upload without proxy fallback is a release requirement.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
