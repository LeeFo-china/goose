...
平台超管增加设备资产运维视图，用于排查设备通道的租户归属、密码和同步状态。
...
后端仍保留第三方通道列表接口，作为运维排查或后续轻量弹窗纳入资产的能力，不作为租户页面主入口。
...
1. 用户进入工地监控页。
2. 进入“设备接入”tab，查看“设备资产池”。
3. 如需新增腾讯云设备，在资产池头部点击“新增设备”。
4. 如果腾讯云设备刚创建但通道尚未补齐，点击“同步资产”。
5. 点击“绑定摄像头”。
6. 选择项目。
7. 从未绑定资产中选择设备通道。
8. 提交绑定。
...
- `同步`：对当前设备触发资产同步。
- `接入信息`：查看腾讯云设备 ID、编码、SIP 用户名和 SIP 服务器配置。
- `查密码`：查看腾讯云设备当前 SIP 密码。
- `重置密码`：生成新的腾讯云设备 SIP 密码。
...
所有平台设备运维操作都写入 `platform_audit_logs`。
