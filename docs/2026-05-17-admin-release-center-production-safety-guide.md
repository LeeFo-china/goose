...
- 点击「作为来源」只会把 Commit 填入创建 Tag 表单，不会自动提交。
- 点击「回滚 Tag」会让后端自动生成当天的下一个 `vYYYY.MM.DD.N` Tag，并指向该成功 Commit。
- 回滚 Tag 创建成功后，Admin 自动切换到生产发布表单，并填入刚创建的 Tag。
...
- 不自动提交生产发布，创建回滚 Tag 后仍需要人工确认。
- 如果历史 Commit 依赖已经被删除的外部配置或镜像，GitHub Actions 仍可能失败，需要查看 run 日志。

## 当前边界

第一版不做一键直接回滚。回滚先创建不可变 Tag，再走生产发布确认，避免绕过生产发布规则。
