...
## Admin 发布后只读 smoke 回填

回填时间：2026-06-18

Admin 侧已完成发布后只读 smoke，未在 Admin 端执行任何 workflow 推进、
模板发布、节点调整、手工补状态或数据库修改。

核验结果：

| 检查项 | 结果 |
| --- | --- |
| 员工后台登录 | 通过：账号 `18800000001`，员工 `风清扬`，employee ID `d8ecc522-e6a1-49d6-b7b7-aaa0f3084826`，租户 `固始晴天装饰工程有限公司` |
| 项目详情 | 通过：`/projects/54f11aa5-09a8-4410-a9c5-604a7fe9e09c` 可见项目 `张学友·固始县蓼都廉租房 4单元201`，状态 `施工中` |
| 财务台账 | 通过：`/finance/ledger?page=1` 可见 `项目收款入账`，金额 `¥10,000.00` |
| workflow 模板页 | 通过：`/workflows` 仅查看流程列表，未发布新模板，未调整节点 |
