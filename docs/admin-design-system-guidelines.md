...
- `.codex/skills/admin-design/SKILL.md`。本文件供人工查阅；若页面类型或容器规则与该技能冲突，以该技能的当前规则为准。
...
先判断页面主任务，再选择对应结构。运营工作台不套用列表页的“概览 + 列表 Card”或仪表盘 KPI 网格。

### 状态与行动工作台

当页面围绕一个租户、项目或流程的当前状态、阻断和下一步有效操作时，采用 [admin-design 状态与行动工作台规范](../.codex/skills/admin-design/references/operational-workspace.md)。状态与动作置顶；阻断紧邻动作；公开资料、简单计数、局部设置和版本记录在一张无边框页面底板中用分隔线平铺。此规范优先于本文件较早的通用 Card 建议，抖音小程序工作台是仓库实例。
...
此结构用于以 Tab 切换为主任务的管理页面；状态与行动工作台若有附属 Tab，首屏状态和动作仍留在 Tab 上方。
...
`loading.tsx` 必须与真实页面结构一致。以下约束仅针对列表页：
...
- 是否使用统一页面框架。
- 是否按主任务选择状态与行动、列表、Tab、详情或仪表盘结构。
- 状态与行动工作台是否先显示状态、当前动作和阻断，且没有卡片嵌套或简单计数 KPI Card。
- 列表页的筛选、表格、分页是否处于同一个列表 Card。
