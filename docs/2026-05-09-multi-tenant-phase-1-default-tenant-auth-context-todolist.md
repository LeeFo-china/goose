...
- [ ] 插入默认租户：
  ...
  - `status = active`
- [ ] 优先使用 `DEFAULT_TENANT_SLUG` 查找或创建默认租户。
...
- [ ] 给核心表增加 `tenant_id`：
  ...
  - `properties`
- [ ] 回填现有数据到默认租户。
- [ ] 确认默认租户只用于初始化和历史数据回填，不作为新业务请求兜底。
- [ ] 给核心表增加基础索引：
  - `(tenant_id, created_at)`
  - `(tenant_id, status)`
  - `(tenant_id, phone)`，客户表适用
- [ ] 对新增字段先允许兼容旧数据，确认回填完成后再考虑 `not null`。

### 2. 类型与 domain

- [ ] 更新 Supabase database types。
- [ ] 更新 `@gooes/domain` 中涉及 tenant 的基础类型。
- [ ] 定义 `TenantStatus`。
- [ ] 定义租户基础响应类型。
...
- [ ] `AuthContext` 增加：
  ...
  - `tenantStatus`
  - `isPlatformAdmin`
- [ ] 员工登录时通过 `employees.tenant_id` 关联 `tenants`。
- [ ] 员工无租户时返回明确错误。
- [ ] 租户状态非 `active` 时，禁止进入普通业务接口。
- [ ] 缓存 key 设计检查，避免多租户身份缓存混乱。

### 4. Admin 登录态

- [ ] `/admin/auth/me` 返回 tenant 信息。
- [ ] admin session 类型增加 tenant。
- [ ] admin 顶部或用户信息区域预留公司名称展示。
- [ ] 租户停用时 admin 显示明确提示并退出。

### 5. 小程序登录态

- [ ] 后端客户/员工登录响应预留 tenant 信息。
- [ ] 小程序端暂不强制改 UI。
- [ ] 文档说明小程序后续可在“我的”页展示所属公司。
