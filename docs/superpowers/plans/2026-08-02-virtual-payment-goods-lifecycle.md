...
- [x] **Step 4: 更新 repository/service 数据类型和序列化**
...
- [x] **Step 5: 运行 migration 契约和现有管理测试**
...
- [x] **Step 4: 让 management service 传入服务端拥有的字段**
...
实现时由 controller 直接调用独立 lifecycle service，避免让已接近文件大小上限的 settings facade 继续承担无关职责；仍保持 controller → service 分层。

新增 `getGoodsStatus`、`startGoodsUpload`、`startGoodsPublish`，只负责调用 lifecycle service；保留 `validate` 的原路径和语义。

- [x] **Step 5: 写 schema/controller 失败测试并实现路由**
...
- [x] **Step 6: 运行 service/schema/controller 测试**
