...
**Architecture:** Keep public project base rows cached and attach a public-safe live display label after every cache path. A bounded repository method reads compact workflow subject states for all project and tenant IDs in the current page using one query; a focused service maps those rows to labels without exposing workflow internals.
...
`project`, filters both tenant and subject IDs, de-duplicates and caps each ID
set at 100, and applies `.limit(100)`.
...
- [ ] **Step 1: Write a failing cache regression test**
...
- [ ] **Step 2: Verify the cache regression test fails**
...
- [ ] **Step 3: Attach live labels after cache resolution**
...
- [ ] **Step 4: Verify all targeted tests pass**
...
- [ ] **Step 2: Run API typecheck and separate baseline failures**
