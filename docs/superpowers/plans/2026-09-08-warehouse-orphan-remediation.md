...
- [ ] 取得固定文件与 hash 的真实开发 apply 最终确认。

## Task 3：隔离发布修复，重做备份恢复门禁

- [ ] 从已应用开发基线准备只包含清理缺陷修复＋修复 migration 的候选，不包含 4 条 C migration。核对真实完整历史；本轮未 fetch／修改 main，不把当前本地引用当最终发布事实。
- [ ] 在获准目标生成新备份，保留原失败归档；明确删除 4 条及日志审计方案并取得 apply 授权。开发 workflow 无自动备份，不跳过此步。
- [ ] 使用固定修复候选的完整 migration list／plan，确认只待执行修复文件；若还有其他版本则停止。使用 migration 工具应用，不把上述 DO 块粘贴到远端手工执行。
- [ ] 应用后 migration list 全量对齐；核对孤儿=0、正常仓库不变、9 表数据不变、审计=4。
- [ ] 基于纠正后的开发库重新生成全 schema、角色及 Vault 根密钥备份，使用已有隔离恢复流程完整 pg_restore，保持所有约束／ownership／ACL；325 张表须依据新的同快照清单逐表对账，不能复用旧数量或省略 post-data。
- [ ] 将修复提交合入 C 候选后重新求完整差集。修复 migration 的新时间戳会晚于 C 原 4 条：可能需要获审查的 `--include-all` 才能列出／应用较早未执行的 C 文件，或者使用既有按完整差集执行的工作流；不能改已应用编号、repair 历史或把 C 和纠正混作一次未获授权的升级。
...
- [ ] 真实 apply、修复后备份／完整恢复门禁、发布候选合并尚未执行。

后续可按 executing-plans 在当前会话逐项实施，先完成代码与 migration 的本地验证；不因用户批准方案就跳过真实删除／apply 的清单确认。
