...
3. Verify non-overlapping source-owned files are patch-equivalent. For overlapping files, record
   ...
   **approved integration exceptions**, not silent drift. Every source-owned path outside this
   ...
   | `apps/api/src/services/wechat-pay-gateway-close-timeout.test.ts` | Source close timeout contract | Integration-only split keeps the source-owned close test below 500 lines and proves one timer spans fetch, body read, signature verification, and body validation, with cleanup after every terminal path. |
