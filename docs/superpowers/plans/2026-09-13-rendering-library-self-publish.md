...
发布弹窗在一次同版本操作中保存 UUID；网络中断、502/503、发布进行中等未知结果用原键重试。明确版本冲突后加载最新资料，版本推进时为新版本生成新 UUID 并清空责任确认；版本和资料均未变化则保留原键。用户关闭后重新发起也生成新 UUID。发布弹窗必须有未选中的责任 checkbox，按钮 disabled 直到确认；展示当前预览和 metadata 摘要。
...
发布成功刷新列表；仅版本冲突 409 显示加载最新资料按钮，发布进行中 409 提示稍后同键重试。处理中阻止关闭和重复提交。页面头部改为“整理并发布公司装修风格素材”，不再声称仅内部草稿。
