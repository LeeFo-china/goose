...
- 成本分类按 SKU 所属商品解析租户规则：商品规则优先，其次目录分类及上级分类规则，冻结在领料明细；配置缺失或不属于本租户时阻断提交，不新增 SKU 级分类规则。
...
- [x] 先编写隔离 SQL 验收，证明新单据/命令尚不存在；记录 RED。
- [x] 新建 `warehouse_issue_orders/items`、`warehouse_return_orders/items`、命令回执和独立开关；tenant 复合外键、索引、强制 RLS、service-role-only RPC、私有 helper ACL。
- [x] 实现有界读取及草稿、提交、完成、取消命令，遵循上述规则；返回明确的单据、版本和可追溯成本字段。将真实 RPC 参数/结果契约交给后续 API 任务。
- [x] 改造成本事实以兼容仓库来源，补齐数据库聚合的净成本语义。
- [x] 真实隔离 PostgreSQL 验证收货→领料→分次退料、尾差、余额流水一致、项目净成本、无新增应付、跨租户/项目权限、停用仓库/关闭开关、版本/重放、失败回滚及并发超领/超退。
- [x] 运行现有 receipt-accounting 回归，确保项目直采和仓库入库兼容；独立规格与质量审查后提交。
...
- [x] 编写失败测试覆盖 strict schema、成本字段拒绝、分页上下限、权限交集、项目越权、重复命令、错误映射以及净成本。
- [x] 读取 API：`GET /warehouse-issues[/:id[/items]]`、`GET /warehouse-returns[/:id[/items]]`；新增项目选项使用现有项目数据范围/分页机制；退料明细可读原领料数量及剩余可退数量。
- [x] 写 API：`POST /warehouse-issues/:id/{save-draft,submit,complete,cancel}` 和 `POST /warehouse-returns/:id/{save-draft,complete,cancel}`；使用期望版本及幂等键，HTTP 只处理身份、校验和响应。
- [x] Repository 严格解析 RPC 返回、金额字符串、来源/单据关系；Service 编排真实权限，错误统一 `Errors`，不吞异常或使用裸 Error。
- [x] 将 `warehouse_materials_enabled` 贯通平台读取/写入、租户 supplier-settings 及领域类型；历史客户端省略时保留值。
- [x] 集成补充：现有 `/supplier-settings` 要求 `supplier.view`，因此新增仅返回有效 C 开关的 `GET /warehouse-issues/settings`；仓库查看/管理/确认任一权限与 `project.read` 交集，不扩大供应商配置读取权限。
- [x] 净成本汇总 `increase=+amount, decrease=-amount`，验证旧采购、领料及退料混合数据；查清所有成本事实消费者的语义。
- [x] Domain build、API typecheck、定向测试、路由分类、权限/文件门禁；独立规格和质量审查后提交。全量路由清单仅余基线 11 条抖音白名单缺口，见证据文档；未宣称该全量测试通过。
...
- [x] 读取 admin-design 与 shadcn 技能、现有库存和采购单实现。保留当前库存筛选简化成果，复用现有业务组件和命令未知结果恢复模式。
- [x] “仓库库存”提供项目领料/退料入口，支持分页搜索、详情和状态过滤；默认仓库、分页 SKU/项目选择、数量编辑、成本只读。
- [x] 领料草稿/提交/确认/取消，已完成领料发起退料，展示每行剩余可退数量及关联原单。权限与服务开关控制操作；后端仍最终校验。
- [x] 版本冲突刷新后重新确认；网络未知结果保留冻结载荷/幂等键供重试；成功刷新失败只重读，不重复提交。补充严格回执校验和大写 UUID 恢复。
- [x] 库存来源展示领料/退料单号并按真实权限跳转；平台新增独立开关，遵守依赖/审计和历史读取。
- [x] 最小静态检查先行，再以真实页面+确定性 HTTP fixture 测桌面/375px 完整创建、确认、退料、权限、失败/重试、分页和竞态；截图人工核验。领退料 28 项、平台开关 16 项全部通过。
- [x] 独立规格与质量审查后提交。
...
- [x] 在隔离数据环境连接真实 API 执行领料/退料链路，核对库存、成本、应付、原项目采购兼容；不得拿纯 mock 替代数据库过账证据。
- [x] 大数据分页/项目范围/来源读取 EXPLAIN 验证索引及扫描边界；并发连接验证最终出库和退料限额。
- [x] 验证 Domain build、API check、Admin check/build、相关定向测试及浏览器 smoke。React mock 测试按既有隔离策略逐文件执行。
- [x] 对接文档写明路径、权限、状态、开关、幂等/版本、金额精度、错误码和可执行验收示例；不改 orange。
- [x] 汇总准确证据与发布待办，完成最终独立审查。只在上述本地交付证据完整时声明阶段 C 实现完成；远端发布/开关操作另按明确授权执行。
