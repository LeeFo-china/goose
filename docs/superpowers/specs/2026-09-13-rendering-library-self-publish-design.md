...
同版本的网络中断、502/503 或发布进行中等未知结果，客户端只用原幂等键重试；
明确收到版本冲突并加载最新资料后，若版本已变化，新版本发布使用新键并要求重新
确认责任。版本和资料均未变化时保留原键。幂等键内容冲突需关闭弹窗重新发起。
...
也不能保证立即清除客户端或 CDN 已缓存的副本。请求执行期间禁用重复提交；
仅版本冲突时加载服务器最新版本，不用本地数据覆盖。发布进行中允许同键稍后
重试，不把它误判为版本冲突。
