...
- [ ] `marketing_campaigns` 增加或回填 `tenant_id`。
- [ ] 活动列表按租户过滤。
- [ ] 活动创建自动写入当前租户。
- [ ] 活动包含项目只允许选择当前租户项目。
- [ ] 活动统计按租户统计。
...
- [ ] `marketing_h5_pages` 增加或回填 `tenant_id`。
- [ ] H5 页面创建写入当前租户。
- [ ] H5 页面编辑、发布、预览校验租户边界。
- [ ] `/p/:slug` 查询页面时反查 `tenant_id`。
- [ ] slug MVP 阶段继续全局唯一。
- [ ] 后续预留 `/t/:tenantSlug/p/:slug`。

### 3. H5 表单线索

- [ ] `marketing_leads` 增加或回填 `tenant_id`。
- [ ] H5 提交时不信任前端传 `tenant_id`。
- [ ] 根据 page slug 得到 `tenant_id`。
- [ ] 根据页面租户创建线索。
- [ ] 线索去重改为租户内去重。
- [ ] 同手机号允许在不同租户分别存在。
