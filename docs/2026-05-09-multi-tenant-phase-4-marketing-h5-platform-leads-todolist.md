...
- [x] slug MVP 阶段继续全局唯一。
- [x] 增加 `/t/:tenantSlug/p/:slug` 租户化访问路径。
- [x] 增加租户化公开页面列表、详情、线索、埋点接口。
- [x] H5 session token 写入 `tenant_id`，提交时校验 token 租户边界。
...
- [x] 线索去重改为租户内去重。
- [x] 同手机号允许在不同租户分别存在。
- [x] H5 前端按 `/t/:tenantSlug/p/:slug` 自动调用租户化公开接口。
...
- [x] 客户手机号命中多个租户时返回：
  ...
  - `tenants`
- [x] 小程序展示公司选择页对接文档。
- [x] 新增 `POST /customer/auth/select-tenant`。
- [x] 后端校验客户归属。
- [x] 签发带 `tenant_id/customer_id` 的客户会话。
- [ ] “我的”页支持切换装修公司。

### 6. 员工拓客直绑定

- [x] 设计员工分享小程序码参数。
- [x] 优先使用 `share_token` 反查租户和分享员工。
- [x] 设计 H5 分享链接参数：
  - `share_token`
  - 或 `tenant + share_employee_id`
- [x] 新增 `tenant_share_links`。
- [x] `tenant_share_links` 预留 `expires_at`。
- [x] MVP 可先不强制二维码过期。
- [x] 后续活动页、报价表单、临时推广码可启用过期时间。
- [x] 客户扫码或打开链接后缓存分享上下文。
- [x] 客户登录/注册后校验分享上下文。
- [x] 在目标租户下按手机号查找客户。
- [x] 客户已存在：
...
- [x] 客户不存在：
  ...
  - 写入首次来源。
  - 关联分享员工 ID。
- [x] 同手机号已存在其他租户时，不影响本次绑定。
- [x] 本链路不进入 `platform_leads`。
- [ ] 通知租户管理员和分享员工。
...
- [x] 客户详情增加“线索来源时间线”。
- [x] `customers.source` 保留首次来源。
- [x] `customer_sources` 展示后续所有触达来源。
- [x] 线索/客户列表显示：
  - 老客户新线索
  - 平台新线索
- [ ] 支持按标记筛选。
