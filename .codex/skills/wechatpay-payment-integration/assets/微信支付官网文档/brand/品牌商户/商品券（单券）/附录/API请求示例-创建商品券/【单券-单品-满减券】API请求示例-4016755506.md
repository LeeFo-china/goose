---
url: "https://pay.weixin.qq.com/doc/brand/4016755506.md"
...
## 4. 单券-单品-满减券 (SINGLE_SINGLE_NORMAL)
...
curl -X POST \
  ...
  -d '{
    "out_request_no": "SINGLE_SINGLE_NORMAL_20250120_004", # 必填，创建请求单号
    "scope": "SINGLE", # 必填，优惠范围
    "type": "NORMAL", # 必填，券类型
    "usage_mode": "SINGLE", # 必填，使用模式

    "display_info": { # 必填，展示信息
      "name": "特定商品满80减20", # 必填，商品券名称
      ...
      "original_price": 12000, # 当scope=SINGLE时必填，商品原价，单位：分
    ...
    "stock": { # 必填，批次信息
      ...
      "single_usage_rule": { # 当usage_mode=SINGLE时必填，单券使用规则
        ...
        "normal_coupon": { # 当type=NORMAL且scope=SINGLE时必填，满减券规则（单品券在single_usage_rule中设置）
          "threshold": 8000, # 当type=NORMAL且scope=SINGLE时必填，门槛金额，单位：分
          "discount_amount": 2000 # 当type=NORMAL且scope=SINGLE时必填，减免金额，单位：分
        }
      },
...
public class SingleNormalJava {
  ...
  public static void main(String[] args) {
    ...
    SingleNormalJava client = new SingleNormalJava(
    ...
    request.displayInfo.name = "特定商品满80减20"; // 必填，商品券名称，最多12个字符
    ...
    request.displayInfo.originalPrice = 12000L; // 必填(单品券)，商品原价(单位：分)
    request.displayInfo.comboPackageList = new ArrayList<>(); // 必填(单品券)，套餐组合列表
    {
      ...
      choice.price = 600L; // 必填，商品价格(单位：分)
    ...
    // 条件必填，满减券使用规则(单品券的优惠规则在批次中配置，当type=NORMAL且scope=SINGLE时必填)
    request.stock.singleUsageRule.normalCoupon = new NormalCouponUsageRule();
    request.stock.singleUsageRule.normalCoupon.threshold = 8000L; // 必填，门槛金额(单位：分)，满80元
    request.stock.singleUsageRule.normalCoupon.discountAmount = 2000L; // 必填，固定减免金额(单位：分)，减20元
    // 必填，使用规则展示信息
    ...
    try {
      CreateProductCouponResponse response = client.run(request);
        // TODO: 请求成功，继续业务逻辑
        System.out.println("单品满减券创建成功: " + response.productCouponId);
  ...
  public SingleNormalJava(String brand_id, String certificateSerialNo, String privateKeyFilePath, String wechatPayPublicKeyId, String wechatPayPublicKeyFilePath) {
...
func main() {
	...
	request := &CreateProductCouponRequest{
		...
		DisplayInfo: &ProductCouponDisplayInfo{
			Name:          wxpay_brand_utility.String("特定商品满80减20"),                             // 必填，商品券名称，最多12个字符
			...
			OriginalPrice: wxpay_brand_utility.Int64(12000), // 必填(单品券)，商品原价(单位：分)
			// 必填(单品券)，套餐组合列表
					...
					ChoiceList: []ComboPackageChoice{
						{
							...
							Price:            wxpay_brand_utility.Int64(600),                                          // 必填，商品价格(单位：分)
		...
		Stock: &StockForCreate{
			...
			SingleUsageRule: &SingleUsageRule{
				...
				NormalCoupon: &NormalCouponUsageRule{
					Threshold:      wxpay_brand_utility.Int64(8000), // 必填，门槛金额(单位：分)，满80元
					DiscountAmount: wxpay_brand_utility.Int64(2000), // 必填，固定减免金额(单位：分)，减20元
				},
			},
			// 必填，使用规则展示信息
	...
	if err != nil {
		fmt.Printf("单品满减券创建失败: %+v\n", err)
		// TODO: 请求失败，根据状态码执行不同的处理
		return
	}

	// TODO: 请求成功，继续业务逻辑
	fmt.Printf("单品满减券创建成功: %+v\n", response)
}
