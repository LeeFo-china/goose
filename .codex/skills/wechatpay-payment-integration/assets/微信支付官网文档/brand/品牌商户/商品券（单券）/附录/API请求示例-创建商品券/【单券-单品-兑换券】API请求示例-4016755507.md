---
url: "https://pay.weixin.qq.com/doc/brand/4016755507.md"
title: "【单券-单品-兑换券】API请求示例"
doc_id: 4016755507
---
...
## 5. 单券-单品-兑换券 (SINGLE_SINGLE_EXCHANGE)
...
curl -X POST \
  ...
  -d '{
    "out_request_no": "SINGLE_SINGLE_EXCHANGE_20250120_005", # 必填，创建请求单号
    "scope": "SINGLE", # 必填，优惠范围
    "type": "EXCHANGE", # 必填，券类型
    "usage_mode": "SINGLE", # 必填，使用模式

    "display_info": { # 必填，展示信息
      "name": "特定商品10元兑换券", # 必填，商品券名称
      ...
      "original_price": 9900, # 当scope=SINGLE时必填，商品原价，单位：分
    ...
    "stock": { # 必填，批次信息
      ...
      "single_usage_rule": { # 当usage_mode=SINGLE时必填，单券使用规则
        ...
        "exchange_coupon": { # 当type=EXCHANGE且scope=SINGLE时必填，兑换券规则（单品券在single_usage_rule中设置）
          "threshold": 0, # 当type=EXCHANGE且scope=SINGLE时必填，门槛金额，单位：分
          "exchange_price": 1000 # 当type=EXCHANGE且scope=SINGLE时必填，兑换价格，单位：分
        }
      },
 ...
 * 创建商品券 - 单券-单品-兑换券
 */
public class SingleExchangeJava {
  ...
  public static void main(String[] args) {
    ...
    SingleExchangeJava client = new SingleExchangeJava(
    ...
    request.displayInfo.name = "特定商品兑换券"; // 必填，商品券名称，最多12个字符
    ...
    request.displayInfo.originalPrice = 9900L; // 必填(单品券)，商品原价(单位：分)
    request.displayInfo.comboPackageList = new ArrayList<>(); // 必填(单品券)，套餐组合列表
    {
      ...
      choice.price = 1500L; // 必填，商品价格(单位：分)
    ...
    // 条件必填，兑换券使用规则(单品券的优惠规则在批次中配置，当type=EXCHANGE且scope=SINGLE时必填)
    request.stock.singleUsageRule.exchangeCoupon = new ExchangeCouponUsageRule();
    request.stock.singleUsageRule.exchangeCoupon.threshold = 0L; // 必填，门槛金额(单位：分)，0表示无门槛
    request.stock.singleUsageRule.exchangeCoupon.exchangePrice = 1000L; // 必填，兑换价格(单位：分)，10元兑换
    // 必填，使用规则展示信息
    ...
    request.stock.couponDisplayInfo.entranceFinder.finderId = "gh_12345678"; // 必填，视频号ID
    request.stock.couponDisplayInfo.entranceFinder.finderVideoId = "UDFsdf24df34dD456Hdf34"; // 选填，视频号视频ID
    request.stock.couponDisplayInfo.entranceFinder.finderVideoCoverImageUrl = "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/xxx"; // 选填，视频封面图URL
    request.stock.notifyConfig = new NotifyConfig(); // 选填，事件通知配置
    request.stock.notifyConfig.notifyAppid = "wx4fd12345678"; // 必填，通知AppID
    request.stock.storeScope = StockStoreScope.NONE; // 必填，门店适用范围：NONE-不限制/ALL-全部门店/SPECIFIC-指定门店
    try {
      CreateProductCouponResponse response = client.run(request);
        // TODO: 请求成功，继续业务逻辑
        System.out.println("单品兑换券创建成功: " + response.productCouponId);
  ...
  public SingleExchangeJava(String brand_id, String certificateSerialNo, String privateKeyFilePath, String wechatPayPublicKeyId, String wechatPayPublicKeyFilePath) {
...
func main() {
	...
	request := &CreateProductCouponRequest{
		...
		DisplayInfo: &ProductCouponDisplayInfo{
			Name:          wxpay_brand_utility.String("特定商品10元兑换券"),                            // 必填，商品券名称，最多12个字符
			...
			OriginalPrice: wxpay_brand_utility.Int64(9900), // 必填(单品券)，商品原价(单位：分)
			// 必填(单品券)，套餐组合列表
					...
					ChoiceList: []ComboPackageChoice{
						{
							...
							Price:            wxpay_brand_utility.Int64(1500),                                         // 必填，商品价格(单位：分)
		...
		Stock: &StockForCreate{
			...
			SingleUsageRule: &SingleUsageRule{
				...
				ExchangeCoupon: &ExchangeCouponUsageRule{
					...
					ExchangePrice: wxpay_brand_utility.Int64(1000), // 必填，兑换价格(单位：分)，10元兑换
				},
			},
			// 必填，使用规则展示信息
	...
	if err != nil {
		fmt.Printf("单品兑换券创建失败: %+v\n", err)
		// TODO: 请求失败，根据状态码执行不同的处理
		return
	}

	// TODO: 请求成功，继续业务逻辑
	fmt.Printf("单品兑换券创建成功: %+v\n", response)
}
