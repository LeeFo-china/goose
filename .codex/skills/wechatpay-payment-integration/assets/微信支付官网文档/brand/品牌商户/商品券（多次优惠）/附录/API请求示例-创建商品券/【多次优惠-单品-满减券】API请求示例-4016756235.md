---
url: "https://pay.weixin.qq.com/doc/brand/4016756235.md"
title: "【多次优惠-单品-满减券】API请求示例"
doc_id: 4016756235
---
...
## 4. 多次优惠-单品-满减券 (usage_mode=PROGRESSIVE_BUNDLE, scope=SINGLE, type=NORMAL)
...
curl -X POST \
  ...
  -d '{
    "out_request_no": "PROGRESSIVE_BUNDLE_SINGLE_NORMAL_20250101_004", # 必填，创建请求单号，品牌侧需保持唯一性，可使用数字、大小写字母、下划线_、短横线-组成，长度6-40字符
    ...
    "display_info": { # 必填，展示信息
      ...
      "original_price": 800, # 条件必填，当scope=SINGLE时必填，商品原价，单位：分
    ...
    "out_product_no": "Product_ProgressiveBundle_004", # 选填，外部商品ID

    "stock_bundle": { # 当usage_mode=PROGRESSIVE_BUNDLE时必填，批次组信息
      "remark": "单品满减券批次", # 选填，备注
      ...
      "progressive_bundle_usage_rule": { # 当usage_mode=PROGRESSIVE_BUNDLE时必填，多次优惠使用规则
        ...
        "normal_coupon_list": [ # 当type=NORMAL时必填，满减券规则列表，数量必须等于count（最少3个，最多15个）
          { "threshold": 0, "discount_amount": 100 }, # 第1次：减1元（threshold=0表示无门槛，discount_amount最小1）
          { "threshold": 0, "discount_amount": 150 }, # 第2次：减1.5元
          { "threshold": 0, "discount_amount": 200 }, # 第3次：减2元
          { "threshold": 0, "discount_amount": 250 }, # 第4次：减2.5元
          { "threshold": 0, "discount_amount": 300 }  # 第5次：减3元
        ]
      },
      ...
      "usage_rule_display_info": { # 必填，使用规则展示信息
        ...
        "usage_description": "指定商品可用，多次递增满减", # 必填，使用说明
...
/**
 * 创建商品券 - 多次优惠-单品-满减券
 * usage_mode=PROGRESSIVE_BUNDLE, scope=SINGLE, type=NORMAL
 */
public class SingleNormalSequentialJava {
  ...
  public static void main(String[] args) {
    ...
    SingleNormalSequentialJava client = new SingleNormalSequentialJava(
    ...
    request.outRequestNo = "PROGRESSIVE_BUNDLE_SINGLE_NORMAL_20250101_004"; // 必填，创建请求单号，6-40个字符
    ...
    request.displayInfo.name = "单品满减券-薯条5次优惠"; // 必填，商品券名称，最多12个字符
    ...
    request.displayInfo.originalPrice = 800L; // 必填(单品券)，商品原价(单位：分)
    request.displayInfo.comboPackageList = new ArrayList<>(); // 必填(单品券)，套餐组合列表
    {
      ...
      choice.name = "薯条"; // 必填，商品名称
      choice.price = 800L; // 必填，商品价格(单位：分)
    ...
    request.outProductNo = "Product_ProgressiveBundle_004"; // 选填，商户侧商品券唯一标识
    // 必填，批次信息
    request.stockBundle = new StockForCreate();
    request.stockBundle.remark = "单品满减券批次"; // 选填，批次备注，最多60个字符
    ...
    request.stockBundle.progressiveBundleUsageRule.normalCouponList = new ArrayList<>();
    {
      ...
      rule1.discountAmount = 100L; // 必填，固定减免金额(单位：分)
    ...
    request.stockBundle.usageRuleDisplayInfo.usageDescription = "指定商品可用，多次递增满减"; // 选填，使用说明
  ...
  public SingleNormalSequentialJava(String brand_id, String certificateSerialNo, String privateKeyFilePath, String wechatPayPublicKeyId, String wechatPayPublicKeyFilePath) {
...
func main() {
	...
	request := &CreateProductCouponRequest{
		OutRequestNo: wxpay_brand_utility.String("PROGRESSIVE_BUNDLE_SINGLE_NORMAL_20250101_004"), // 必填，创建请求单号，6-40个字符
		...
		DisplayInfo: &ProductCouponDisplayInfo{
			Name:          wxpay_brand_utility.String("单品满减券-薯条5次优惠"),                       // 必填，商品券名称，最多12个字符
			...
			OriginalPrice: wxpay_brand_utility.Int64(800), // 必填(单品券)，商品原价(单位：分)
			// 必填(单品券)，套餐组合列表
			ComboPackageList: []ComboPackage{
				{
					...
					ChoiceList: []ComboPackageChoice{
						{
							Name:             wxpay_brand_utility.String("薯条"),                                       // 必填，商品名称
							Price:            wxpay_brand_utility.Int64(800),                                          // 必填，商品价格(单位：分)
		...
		OutProductNo: wxpay_brand_utility.String("Product_ProgressiveBundle_004"), // 选填，商户侧商品券唯一标识
		// 必填，批次信息
		StockBundle: &StockForCreate{
			Remark:         wxpay_brand_utility.String("单品满减券批次"), // 选填，批次备注，最多60个字符
			...
			ProgressiveBundleUsageRule: &ProgressiveBundleUsageRule{
				...
				NormalCouponList: []NormalCouponUsageRule{
					{Threshold: wxpay_brand_utility.Int64(0), DiscountAmount: wxpay_brand_utility.Int64(100)}, // 必填: Threshold-门槛金额, DiscountAmount-固定减免金额
					{Threshold: wxpay_brand_utility.Int64(0), DiscountAmount: wxpay_brand_utility.Int64(150)},
					{Threshold: wxpay_brand_utility.Int64(0), DiscountAmount: wxpay_brand_utility.Int64(200)},
					{Threshold: wxpay_brand_utility.Int64(0), DiscountAmount: wxpay_brand_utility.Int64(250)},
					{Threshold: wxpay_brand_utility.Int64(0), DiscountAmount: wxpay_brand_utility.Int64(300)},
				},
			},
			// 必填，使用规则展示信息
			UsageRuleDisplayInfo: &UsageRuleDisplayInfo{
				...
				UsageDescription: wxpay_brand_utility.String("指定商品可用，多次递增满减"),       // 选填，使用说明
				// 选填，可用门店信息
