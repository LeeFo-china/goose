---
url: "https://pay.weixin.qq.com/doc/brand/4016756232.md"
title: "【多次优惠-全场-折扣券】API请求示例"
doc_id: 4016756232
---
...
## 1. 多次优惠-全场-折扣券 (usage_mode=PROGRESSIVE_BUNDLE, scope=ALL, type=DISCOUNT)
...
curl -X POST \
  ...
  -d '{
    "out_request_no": "PROGRESSIVE_BUNDLE_ALL_DISCOUNT_20250101_001", # 必填，创建请求单号，品牌侧需保持唯一性，可使用数字、大小写字母、下划线_、短横线-组成，长度6-40字符
    "scope": "ALL", # 必填，优惠范围：ALL=全场券（券类型仅可配置NORMAL或DISCOUNT），SINGLE=单品券（券类型可配置NORMAL、DISCOUNT或EXCHANGE）
    "type": "DISCOUNT", # 必填，商品券类型：NORMAL=满减券，DISCOUNT=折扣券，EXCHANGE=兑换券（仅scope=SINGLE时可用）
    ...
    "out_product_no": "Product_ProgressiveBundle_001", # 选填，外部商品ID

    "stock_bundle": { # 当usage_mode=PROGRESSIVE_BUNDLE时必填，批次组信息
      "remark": "全场折扣券批次", # 选填，备注
      "coupon_code_mode": "WECHATPAY", # 必填，券Code分配模式：WECHATPAY=微信支付生成，UPLOAD=品牌上传，API_ASSIGN=API动态分配
      ...
      "progressive_bundle_usage_rule": { # 当usage_mode=PROGRESSIVE_BUNDLE时必填，多次优惠使用规则
        "coupon_available_period": { # 必填，券可核销时间
          "available_begin_time": "2025-08-01T00:00:00+08:00", # 必填，开始时间，RFC3339格式，如2025-08-01T00:00:00+08:00表示北京时间2025年8月1日0点
          ...
          "weekly_available_period": { # 选填，每周可用时间（如需全周可用则不配置此字段）
            "day_list": ["SATURDAY", "SUNDAY"], # 条件必填，可用星期，可选值：MONDAY/TUESDAY/WEDNESDAY/THURSDAY/FRIDAY/SATURDAY/SUNDAY（此示例仅周末可用）
            "day_period_list": [ # 选填，当天可用时间段
              {
                "begin_time": "09:00:00", # 当配置day_period_list时必填，开始时间，格式HH:mm:ss
                "end_time": "21:00:00" # 当配置day_period_list时必填，结束时间，格式HH:mm:ss
              }
            ]
          },
          "irregular_available_period_list": [ # 选填，无规律可用时间段，最少1个，最多2个区间
            {
              "begin_time": "2025-10-01T00:00:00+08:00", # 当配置irregular_available_period_list时必填，开始时间，RFC3339格式
              "end_time": "2025-10-07T23:59:59+08:00" # 当配置irregular_available_period_list时必填，结束时间，RFC3339格式
            }
          ]
        },
        "special_first": false, # 选填，是否首笔特惠，可选值：true/false，默认false
        "discount_coupon_list": [ # 当type=DISCOUNT时必填，折扣券规则列表，数量必须等于count（最少3个，最多15个）
          { "threshold": 10000, "percent_off": 10 }, # 第1次：满100打9折（threshold=门槛金额分，0表示无门槛；percent_off=折扣百分比，10表示减10%即9折，最小1，最大99）
          { "threshold": 10000, "percent_off": 15 }, # 第2次：满100打8.5折
          { "threshold": 10000, "percent_off": 20 }, # 第3次：满100打8折
          { "threshold": 10000, "percent_off": 25 }, # 第4次：满100打7.5折
          { "threshold": 10000, "percent_off": 30 }  # 第5次：满100打7折
        ]
      },
      ...
      "usage_rule_display_info": { # 必填，使用规则展示信息
        ...
        "mini_program_appid": "wx1234567890", # 条件必填，当coupon_usage_method_list包含MINI_PROGRAM时必填，小程序AppID
        "mini_program_path": "/pages/index/product", # 条件必填，当coupon_usage_method_list包含MINI_PROGRAM时必填，小程序路径
        "app_path": "pages/index/product", # 条件必填，当coupon_usage_method_list包含APP时必填，APP路径
        "usage_description": "全场可用，多次递增优惠", # 必填，使用说明
        "coupon_available_store_info": { # 选填，可用门店信息
          "description": "所有门店可用", # 当配置coupon_available_store_info时必填，门店描述
      ...
      "coupon_display_info": { # 必填，用户券展示信息
        "code_display_mode": "QRCODE", # 必填，Code展示模式：INVISIBLE=不显示，BARCODE=条形码，QRCODE=二维码
        ...
        "entrance_mini_program": { # 选填，小程序入口
          ...
          "entrance_wording": "立即使用", # 当配置entrance_mini_program时必填，入口文案
          "guidance_wording": "多次优惠等你来" # 当配置entrance_mini_program时必填，引导文案
        },
      ...
      "store_scope": "NONE", # 必填，可用门店范围：NONE=不限制，ALL=全部门店，SPECIFIC=指定门店
...
public class AllDiscountSequentialJava {
  ...
  public AllDiscountSequentialJava(String brand_id, String certificateSerialNo, String privateKeyFilePath, String wechatPayPublicKeyId, String wechatPayPublicKeyFilePath) {
...
func main() {
	...
	request := &CreateProductCouponRequest{
		OutRequestNo: wxpay_brand_utility.String("PROGRESSIVE_BUNDLE_ALL_DISCOUNT_20250101_001"), // 必填，创建请求单号，6-40个字符
		...
		DisplayInfo: &ProductCouponDisplayInfo{
			Name:          wxpay_brand_utility.String("全场折扣券-5次递增优惠"),                        // 必填，商品券名称，最多12个字符
		...
		OutProductNo: wxpay_brand_utility.String("Product_ProgressiveBundle_001"), // 选填，商户侧商品券唯一标识
		// 必填，批次信息
		StockBundle: &StockForCreate{
			Remark:         wxpay_brand_utility.String("全场折扣券批次"), // 选填，批次备注，最多60个字符
			...
			ProgressiveBundleUsageRule: &ProgressiveBundleUsageRule{
				...
				CouponAvailablePeriod: &CouponAvailablePeriod{
					...
					DayList: []WeekEnum{ // 条件必填，每周可用星期数（此示例仅周末可用）
						WEEKENUM_SATURDAY,
						WEEKENUM_SUNDAY,
					},
					},
				...
				DiscountCouponList: []DiscountCouponUsageRule{
					{Threshold: wxpay_brand_utility.Int64(10000), PercentOff: wxpay_brand_utility.Int64(10)}, // 必填: Threshold-门槛金额, PercentOff-折扣百分比
					{Threshold: wxpay_brand_utility.Int64(10000), PercentOff: wxpay_brand_utility.Int64(15)},
					{Threshold: wxpay_brand_utility.Int64(10000), PercentOff: wxpay_brand_utility.Int64(20)},
					{Threshold: wxpay_brand_utility.Int64(10000), PercentOff: wxpay_brand_utility.Int64(25)},
					{Threshold: wxpay_brand_utility.Int64(10000), PercentOff: wxpay_brand_utility.Int64(30)},
				},
			},
			// 必填，使用规则展示信息
			UsageRuleDisplayInfo: &UsageRuleDisplayInfo{
				...
				UsageDescription: wxpay_brand_utility.String("全场可用，多次递增优惠"),         // 选填，使用说明
				// 选填，可用门店信息
