---
url: "https://pay.weixin.qq.com/doc/brand/4016756234.md"
title: "【多次优惠-单品-兑换券】API请求示例"
doc_id: 4016756234
---
...
## 5. 多次优惠-单品-兑换券 (usage_mode=PROGRESSIVE_BUNDLE, scope=SINGLE, type=EXCHANGE)
...
curl -X POST \
  ...
  -d '{
    "out_request_no": "PROGRESSIVE_BUNDLE_SINGLE_EXCHANGE_20250101_005", # 必填，创建请求单号，品牌侧需保持唯一性，可使用数字、大小写字母、下划线_、短横线-组成，长度6-40字符
    "scope": "SINGLE", # 必填，优惠范围：ALL=全场券（券类型仅可配置NORMAL或DISCOUNT），SINGLE=单品券（券类型可配置NORMAL、DISCOUNT或EXCHANGE）
    "type": "EXCHANGE", # 必填，商品券类型：NORMAL=满减券，DISCOUNT=折扣券，EXCHANGE=兑换券（仅scope=SINGLE时可用）
    ...
    "display_info": { # 必填，展示信息
      ...
      "original_price": 2000, # 条件必填，当scope=SINGLE时必填，商品原价，单位：分（20元）
      "combo_package_list": [ # 条件必填，当scope=SINGLE时必填，套餐组合
        {
          "name": "超值套餐", # 必填，套餐名
          "pick_count": 1, # 必填，可选单品数量
          "choice_list": [ # 必填，单品列表
            {
              "name": "汉堡", # 必填，单品名称
              "price": 2000, # 必填，单品价格，单位：分
              "count": 1, # 必填，单品数量
    ...
    "out_product_no": "Product_ProgressiveBundle_005", # 选填，外部商品ID

    "stock_bundle": { # 当usage_mode=PROGRESSIVE_BUNDLE时必填，批次组信息
      "remark": "单品兑换券批次", # 选填，备注
      "coupon_code_mode": "WECHATPAY", # 必填，券Code分配模式：WECHATPAY/UPLOAD/API_ASSIGN
      ...
      "progressive_bundle_usage_rule": { # 当usage_mode=PROGRESSIVE_BUNDLE时必填，多次优惠使用规则
        ...
        "exchange_coupon_list": [ # 当type=EXCHANGE时必填，兑换券规则列表，数量必须等于count（最少3个，最多15个）
          { "threshold": 0, "exchange_price": 1500 }, # 第1次：15元兑换（threshold=0表示无门槛，exchange_price=兑换价格分，最小0）
          { "threshold": 0, "exchange_price": 1200 }, # 第2次：12元兑换
          { "threshold": 0, "exchange_price": 1000 }, # 第3次：10元兑换
          { "threshold": 0, "exchange_price": 800 },  # 第4次：8元兑换
          { "threshold": 0, "exchange_price": 500 }   # 第5次：5元兑换
        ]
      },
      ...
      "usage_rule_display_info": { # 必填，使用规则展示信息
        ...
        "usage_description": "指定商品可用，多次递减价格兑换", # 必填，使用说明
      ...
      "coupon_display_info": { # 必填，用户券展示信息
        ...
        "entrance_mini_program": { # 选填，小程序入口
          ...
          "entrance_wording": "立即兑换", # 当配置时必填
          "guidance_wording": "多次兑换等你来" # 当配置时必填
        },
      ...
      "store_scope": "NONE", # 必填，可用门店范围：NONE/ALL/SPECIFIC
...
/**
 * 创建商品券 - 多次优惠-单品-兑换券
 * usage_mode=PROGRESSIVE_BUNDLE, scope=SINGLE, type=EXCHANGE
 */
public class SingleExchangeSequentialJava {
  ...
  public static void main(String[] args) {
    ...
    SingleExchangeSequentialJava client = new SingleExchangeSequentialJava(
    ...
    request.outRequestNo = "PROGRESSIVE_BUNDLE_SINGLE_EXCHANGE_20250101_005"; // 必填，创建请求单号，6-40个字符
    request.scope = ProductCouponScope.SINGLE; // 必填，优惠范围：SINGLE-单品券
    request.type = ProductCouponType.EXCHANGE; // 必填，商品券类型：EXCHANGE-兑换券(仅scope=SINGLE时可用)
    ...
    request.displayInfo.name = "单品兑换券-汉堡5次优惠"; // 必填，商品券名称，最多12个字符
    ...
    request.displayInfo.originalPrice = 2000L; // 必填(单品券)，商品原价(单位：分)
    request.displayInfo.comboPackageList = new ArrayList<>(); // 必填(单品券)，套餐组合列表
    {
      ...
      combo.choiceList.add(choice);
      request.displayInfo.comboPackageList.add(combo);
    };
    request.outProductNo = "Product_ProgressiveBundle_005"; // 选填，商户侧商品券唯一标识
    // 必填，批次信息
    request.stockBundle = new StockForCreate();
    request.stockBundle.remark = "单品兑换券批次"; // 选填，批次备注，最多60个字符
    ...
    // 条件必填，兑换券使用规则列表(当type=EXCHANGE时必填，数量需与count一致)
    request.stockBundle.progressiveBundleUsageRule.exchangeCouponList = new ArrayList<>();
    {
      ExchangeCouponUsageRule rule1 = new ExchangeCouponUsageRule();
      rule1.threshold = 0L; // 必填，门槛金额(单位：分)，0表示无门槛
      rule1.exchangePrice = 1500L; // 必填，兑换价格(单位：分)
      request.stockBundle.progressiveBundleUsageRule.exchangeCouponList.add(rule1);
      ExchangeCouponUsageRule rule2 = new ExchangeCouponUsageRule();
      rule2.threshold = 0L;
      rule2.exchangePrice = 1200L;
      request.stockBundle.progressiveBundleUsageRule.exchangeCouponList.add(rule2);
      ExchangeCouponUsageRule rule3 = new ExchangeCouponUsageRule();
      rule3.threshold = 0L;
      rule3.exchangePrice = 1000L;
      request.stockBundle.progressiveBundleUsageRule.exchangeCouponList.add(rule3);
      ExchangeCouponUsageRule rule4 = new ExchangeCouponUsageRule();
      rule4.threshold = 0L;
      rule4.exchangePrice = 800L;
      request.stockBundle.progressiveBundleUsageRule.exchangeCouponList.add(rule4);
      ExchangeCouponUsageRule rule5 = new ExchangeCouponUsageRule();
      rule5.threshold = 0L;
      rule5.exchangePrice = 500L;
      request.stockBundle.progressiveBundleUsageRule.exchangeCouponList.add(rule5);
    };
    // 必填，使用规则展示信息
    ...
    request.stockBundle.usageRuleDisplayInfo.usageDescription = "指定商品可用，多次递减价格兑换"; // 选填，使用说明
    ...
    request.stockBundle.couponDisplayInfo.entranceMiniProgram.entranceWording = "立即兑换"; // 必填，入口文案
    request.stockBundle.couponDisplayInfo.entranceMiniProgram.guidanceWording = "多次兑换等你来"; // 选填，引导文案
  ...
  public SingleExchangeSequentialJava(String brand_id, String certificateSerialNo, String privateKeyFilePath, String wechatPayPublicKeyId, String wechatPayPublicKeyFilePath) {
...
func main() {
	...
	request := &CreateProductCouponRequest{
		OutRequestNo: wxpay_brand_utility.String("PROGRESSIVE_BUNDLE_SINGLE_EXCHANGE_20250101_005"), // 必填，创建请求单号，6-40个字符
		Scope:        PRODUCTCOUPONSCOPE_SINGLE.Ptr(),                                       // 必填，优惠范围：SINGLE-单品券
		Type:         PRODUCTCOUPONTYPE_EXCHANGE.Ptr(),                                      // 必填，商品券类型：EXCHANGE-兑换券(仅scope=SINGLE时可用)
		...
		DisplayInfo: &ProductCouponDisplayInfo{
			Name:          wxpay_brand_utility.String("单品兑换券-汉堡5次优惠"),                       // 必填，商品券名称，最多12个字符
			...
			OriginalPrice: wxpay_brand_utility.Int64(2000), // 必填(单品券)，商品原价(单位：分)
			// 必填(单品券)，套餐组合列表
			ComboPackageList: []ComboPackage{
				{
					...
					ChoiceList: []ComboPackageChoice{
						{
							Name:             wxpay_brand_utility.String("汉堡"),                                       // 必填，商品名称
							Price:            wxpay_brand_utility.Int64(2000),                                         // 必填，商品价格(单位：分)
		...
		OutProductNo: wxpay_brand_utility.String("Product_ProgressiveBundle_005"), // 选填，商户侧商品券唯一标识
		// 必填，批次信息
		StockBundle: &StockForCreate{
			Remark:         wxpay_brand_utility.String("单品兑换券批次"), // 选填，批次备注，最多60个字符
			...
			ProgressiveBundleUsageRule: &ProgressiveBundleUsageRule{
				...
				// 条件必填，兑换券使用规则列表(当type=EXCHANGE时必填，数量需与count一致)
				ExchangeCouponList: []ExchangeCouponUsageRule{
					{Threshold: wxpay_brand_utility.Int64(0), ExchangePrice: wxpay_brand_utility.Int64(1500)}, // 必填: Threshold-门槛金额, ExchangePrice-兑换价格
					{Threshold: wxpay_brand_utility.Int64(0), ExchangePrice: wxpay_brand_utility.Int64(1200)},
					{Threshold: wxpay_brand_utility.Int64(0), ExchangePrice: wxpay_brand_utility.Int64(1000)},
					{Threshold: wxpay_brand_utility.Int64(0), ExchangePrice: wxpay_brand_utility.Int64(800)},
					{Threshold: wxpay_brand_utility.Int64(0), ExchangePrice: wxpay_brand_utility.Int64(500)},
				},
			},
			// 必填，使用规则展示信息
			UsageRuleDisplayInfo: &UsageRuleDisplayInfo{
				...
				UsageDescription: wxpay_brand_utility.String("指定商品可用，多次递减价格兑换"),     // 选填，使用说明
				// 选填，可用门店信息
			...
			CouponDisplayInfo: &CouponDisplayInfo{
				...
				EntranceMiniProgram: &EntranceMiniProgram{
					...
					EntranceWording: wxpay_brand_utility.String("立即兑换"),                 // 必填，入口文案
					GuidanceWording: wxpay_brand_utility.String("多次兑换等你来"),               // 选填，引导文案
				},
				// 选填，公众号入口
...
func CreateProductCoupon(config *wxpay_brand_utility.BrandConfig, request *CreateProductCouponRequest) (response *CreateProductCouponResponse, err error) {
	...
	if httpResponse.StatusCode >= 200 && httpResponse.StatusCode < 300 {
		err = wxpay_brand_utility.ValidateResponse(config.WechatPayPublicKeyId(), config.WechatPayPublicKey(), &httpResponse.Header, respBody)
		if err != nil {
			return nil, err
		}
		...
		return nil, wxpay_brand_utility.NewApiException(httpResponse.StatusCode, httpResponse.Header, respBody)
	}
}
