---
url: "https://pay.weixin.qq.com/doc/brand/4015562404.md"
title: "开发指引"
doc_id: 4015562404
---
...
商家可通过在微信支付下单接口的attach字段传入会员卡code（ `user_card_code`）来导入未开通会员的用户。下单用户若此前未开通会员卡，通过此方式可实时添加会员并自动生效一张会员卡，详细指引请参考：[实时动态传入的方式](https://pay.weixin.qq.com/doc/brand/4015562429.md#2.3%E3%80%81%E5%AE%9E%E6%97%B6%E5%8A%A8%E6%80%81%E4%BC%A0%E5%85%A5%E7%9A%84%E6%96%B9%E5%BC%8F)
...
sequenceDiagram
    rect rgb(255,255,255)
        ...
        participant P2 as 商家系统
        participant WxPay as 微信支付

        User->>P2: 购物
        P2->>WxPay: 创建用户动态信息
        User->>WxPay: 展示会员卡动态信息
    end
```
...
stateDiagram-v2
    direction LR
    state "state：UNACTIVATED（用户已领卡未激活）" as S1
    state "state: EFFECTIVE（已生效 ）" as S2
    state "state: INVALID（已失效）" as S3
    state "state: EXPIRED（已过期）" as S4

    [*] --> S1: 用户在商家名片、小程序组件开卡
    S1 --> S2: 商家系统同步微信支付系统用户会员卡开卡成功
    S1 --> S3: 商家作废会员卡、商家拒绝会员卡
    S2 --> S3: 商家作废会员卡、用户主动删除会员卡
    S2 --> S4: 非永久有效的会员卡到期

    [*] --> S2: 商家通过根据OPENID导入用户会员卡接口或根据手机号导入用户会员卡接口API导入用户会员卡
    S3 --> [*]
    S4 --> [*]
```
...
| 会员卡模版ID | 配置到哪张会员卡 | ![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/0a1cdf070010821a6d1d97707a5bd5e8.png) |
| 消耗积分 | 兑换这一张券，需要消耗多少积分 | ![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/0a1cdf070010821a6d1d97707a5bd5e8.png) |
| 总库存 | 在架有多少份这个「x积分兑换xxx券」的福利 | ![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/0a1cdf070010821a6d1d97707a5bd5e8.png) |
| 用户兑换单日限额 | 用户每一天可以兑换多少次<br>每一次用户请求，微信支付都会问询商家是否可兑换。该字段仅作为一个前置拦截规则。 | ![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/0a1cdf070010821a6d1d97707a5bd5e8.png) |
| 排序ID | 在积分货架上，当前积分兑券模版排序第几顺位。 | ![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/0a1cdf070010821a6d1d97707a5bd5e8.png) |
| 商品券ID | 同品牌ID商家下，已创建好的商品券ID | ![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/0a1cdf070010821a6d1d97707a5bd5e8.png) |
...
| 按需接入 | H5/小程序拉起品牌会员入会组件 | 商户可基于自身需求选择H5（公众号端）或小程序的方式引导用户完成入会流程<br>- 通过调用「[H5拉起品牌会员入会组件](https://pay.weixin.qq.com/doc/brand/4015582734.md)」接口可使用微信支付侧提供的 JS 调起品牌会员入会组件，引导用户完成入会流程<br>  <br>- 通过调用「[小程序拉起品牌会员入会组件](https://pay.weixin.qq.com/doc/brand/4015582737.md)」接口可在小程序内使用微信支付提供的小程序方法调起品牌会员入会组件，引导用户完成入会流程 |
...
| 按需接入 | 作废用户会员卡 | 通过调用「[作废用户会员卡](https://pay.weixin.qq.com/doc/brand/4015582726.md)」接口可将用户的会员卡设置为作废状态。<br>注意：<br>- 设置卡券作废的操作不可逆，即无法将设置为作废的卡调回有效状态，商家须慎重调用该接口。<br>  <br>- 商家调用作废接口前须与顾客事先告知并取得同意，否则因此带来的顾客投诉，微信将会按照《微信运营处罚规则》进行处罚。 |
| 按需接入 | 根据OPENID导入用户会员卡 | 对于经营会员生意的商家来说，存量会员的注册渠道广泛，包括但不限于线下渠道、微信渠道、App及其他线上渠道。因而，在商家名片经营会员时会遇到两类问题：<br>（1）存量会员的身份难统一识别；<br>（2）在应用入会活动能力时，容易对存量（但未同步身份至商家名片侧的）老会员重复补贴。<br>因而，针对存量会员，商家可通过用户在微信公众号/小程序内的OpenID，调用「[根据OPENID导入用户会员卡](https://pay.weixin.qq.com/doc/brand/4015582744.md)」接口将存量会员通过接口导入至商家名片。<br>注意：<br>用户OpenID对应的AppID，需要和会员卡所属的品牌号绑定。 |
...
| --- | --- | --- | --- | --- |
| 1 | 必接 | 微信支付通知商户，用户在C端点了兑换按钮 | [用户积分兑券事件通知](https://pay.weixin.qq.com/doc/brand/4015879057.md) | 建议在3秒内回传该通知，否则用户界面可能不再等待响应，导致用户无法及时获知兑换结果，影响体验。<br>![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/e12c9f5b9afc87d6a97acd0f63beca65.png) |
| 2 | 必接 | 用户进入有积分余额展示的页面时，微信支付会向商家查询用户的当前积分余额，<br>以便用户清晰了解自己可兑换的权益。 | [用户积分同步事件通知](https://pay.weixin.qq.com/doc/brand/4016096803.md) | ![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/671af91885e8d85c74a9ff70aac9dd2f.png) |
| 3 | 必接 | 商家回调告知微信支付，是否可以兑券 | [同步积分兑券结果](https://pay.weixin.qq.com/doc/brand/4015897310.md) | ![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/82245a70c72ec2f2d927bde42fcc498b.png) |
| 4 | 看商家需求<br>接了用户体验更好 | 商家异步通知微信支付，当前用户积分值 | [同步积分余额](https://pay.weixin.qq.com/doc/brand/4015897319.md) |  |
| 5 | 看商家需求<br>接了积分兑券效果更好 | 如果想要在支付后的链路将用户可兑换的内容告知用户，<br>需要接入下单同步用户实时动态，在微信支付下单接口的attach字段内<br>按照下单同步用户实时动态的内容传入 | [下单同步用户实时动态](https://pay.weixin.qq.com/doc/brand/4015562429.md) | ![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/f54bf074ea7f65a87da9700ac73c1536.png)<br>![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/0072b806b9930d7e0e239464444563cd.png) |