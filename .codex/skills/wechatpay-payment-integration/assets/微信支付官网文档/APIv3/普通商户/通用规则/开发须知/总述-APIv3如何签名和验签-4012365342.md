---
url: "https://pay.weixin.qq.com/doc/v3/merchant/4012365342.md"
title: "总述-APIv3如何签名和验签"
doc_id: 4012365342
---
...
sequenceDiagram
    ...
    note right of 商户: 发起请求需要构造请求签名
    商户->>商户: 使用商户API证书私钥生成签名
    商户->>微信支付: 发起业务请求
    微信支付->>微信支付: 使用商户API证书公钥验证签名
    微信支付->>微信支付: 使用微信支付私钥或者平台证书私钥生成签名
    微信支付->>商户: 返回应答信息
    商户->>商户: 使用微信支付公钥或者平台证书公钥验证签名
    end
```
...
| --- | --- |
| ```mermaid<br>sequenceDiagram<br>	rect rgb(255,255,255)<br>    participant Merchant as 商户<br>    participant WeChatPay as 微信支付<br>    <br>    Merchant ->> Merchant: 使用商户API证书私钥生成签名<br>    Merchant ->> WeChatPay: 发起业务请求<br>    WeChatPay ->> WeChatPay: 使用商户API证书公钥验证签名<br>    <br>    note right of Merchant: 微信支付应答商户请求时，商户需要验签<br>   <br>    WeChatPay ->> WeChatPay: 使用微信支付私钥或者平台证书私钥生成签名<br>    WeChatPay ->> Merchant: 返回应答信息<br>    Merchant ->> Merchant: 使用微信支付公钥或者平台证书公钥验证签名<br>    end<br>``` | ```mermaid<br>sequenceDiagram<br>	rect rgb(255,255,255)<br>    participant 商户 as 商户<br>    participant 微信支付 as 微信支付<br>    微信支付->>微信支付: 使用APIv3密钥 加密回调信息<br>    <br>    note right of 商户: 接收微信支付回调时，需要验签<br>    <br>    微信支付->>微信支付: 使用微信支付私钥或者平台证书私钥生成签名<br>    微信支付->>商户: 发送回调信息<br>    商户->>商户: 使用微信支付公钥或者平台证书公钥验证签名<br>    商户->>商户: 使用APIv3密钥 解密回调信息<br>    商户->>微信支付: 返回处理结果<br>    end<br>``` |
