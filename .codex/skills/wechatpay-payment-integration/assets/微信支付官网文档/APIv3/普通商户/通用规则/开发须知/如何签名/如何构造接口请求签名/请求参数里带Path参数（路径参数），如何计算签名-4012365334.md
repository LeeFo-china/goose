---
url: "https://pay.weixin.qq.com/doc/v3/merchant/4012365334.md"
...
你可以点击查看一个API接口说明文档，如[查询单笔退款（通过商户退款单号）](https://pay.weixin.qq.com/doc/v3/merchant/4012791863.md)，如下图，可以看到该接口请求参数部分带有out\_refund\_no这个path参数，因为不同的参数类型构造请求签名略有不同，

为了方便大家理解，我们将使用一对测试的商户API证书公私钥，通过命令行形式调用"查询单笔退款（通过商户退款单号）"接口，一步一步介绍请求参数里带Path参数（路径参数），如何计算签名。强烈建议你使用测试的商户API证书公私钥，按照本文实际操作一下，如果算出来的签名和文档的一致则说明计算无误。实际业务中请替换为你的真实的参数。

![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/328106c749fd4c4121d6d9e1e9c7146a.png)
...
第二步，获取请求的绝对URL，查询单笔退款（通过商户退款单号）接口的URL为/v3/refund/domestic/refunds/{out\_refund\_no}，path里面带了参数out\_refund\_no，这里需要填上实际的商户退款单号，例如123123123123
...
对于查询单笔退款（通过商户退款单号）接口的接口来说，请求报文主体是一个空串，只需要附加一个\\n即可。
...
![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/8c6ecfc88a8b8216bbb8938548c3889b.png)

最终我们可以组一个包含了签名的HTTP请求了。

```
curl https://api.mch.weixin.qq.com/v3/refund/domestic/refunds/123123123123 \
-H 'Authorization: WECHATPAY2-SHA256-RSA2048 mchid="1900007291",nonce_str="593BEC0C930BF1AFEB40B4A08C8FB242",signature="",timestamp="1554208460",serial_no="408B07E79B8269FEC3D5D3E6AB8ED163A6A380DB"' -H 'Accept: application/json' \
```
