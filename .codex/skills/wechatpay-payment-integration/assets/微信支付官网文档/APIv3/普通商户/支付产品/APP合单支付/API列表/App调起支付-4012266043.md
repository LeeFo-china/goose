---
url: "https://pay.weixin.qq.com/doc/v3/merchant/4012266043.md"
title: "App调起支付"
doc_id: 4012266043
---
...
| 参数名 | 类型 | 必填 | 描述 |
| --- |  --- | --- | --- |
| appId | string(32) | true | 【合单商户应用ID】请填写下单时传入 `combine_appid` |
| partnerId | string(32) | true | 【合单商户号】请填写下单时传入的 `combine_mchid` |
| prepayId | string(64) | true | 【预支付交易会话标识】[APP合单下单](https://pay.weixin.qq.com/doc/v3/merchant/4012556944.md)接口返回的prepay\_id，该值有效期为2小时，超过有效期需要重新请求[APP合单下单](https://pay.weixin.qq.com/doc/v3/merchant/4012556944.md)接口以获取新的prepay\_id。 |
...
| timeStamp | string(10) | true | 【时间戳】Unix时间戳，是从1970年1月1日（UTC/GMT的午夜）开始所经过的秒数。<br>注意：常见时间戳为秒级或毫秒级，该处必需传秒级时间戳。 |
| sign | string(512) | true | 【签名值】使用字段appId、timeStamp、nonceStr、prepayId以及[商户API证书](https://kf.qq.com/faq/161222NneAJf161222U7fARv.html)私钥生成的RSA签名值，详细参考[APP调起支付签名](https://pay.weixin.qq.com/doc/v3/merchant/4012365340.md)。 |
...
注：前端回调并不保证它绝对可靠，不可只依赖前端回调判断订单支付状态，订单状态需以后端[查询合单订单](https://pay.weixin.qq.com/doc/v3/merchant/4012557006.md)和[合单订单支付成功回调通知](https://pay.weixin.qq.com/doc/v3/merchant/4012158598.md)为准。
