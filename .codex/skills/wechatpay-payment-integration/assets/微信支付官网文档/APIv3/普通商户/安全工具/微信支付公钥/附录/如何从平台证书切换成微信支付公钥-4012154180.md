---
url: "https://pay.weixin.qq.com/doc/v3/merchant/4012154180.md"
title: "如何从平台证书切换成微信支付公钥"
doc_id: 4012154180
---
...
| （1）请求应答场景 | （2）回调场景 |
| --- | --- |
| ```mermaid<br>sequenceDiagram<br>    rect rgb(255,255,255)<br>    participant 商户<br>    participant 微信支付<br>    %% 商户自操作：生成签名<br>    商户->>商户: 使用商户API证书私钥生成签名<br>    %% 发起业务请求<br>    商户->>微信支付: 发起业务请求<br>    %% 微信支付自操作：验签<br>    微信支付->>微信支付: 使用商户API证书公钥验证签名<br>    %% 红框内的应答流程，用无底色 rect 框选<br>    rect rgb(255,255,255)<br>        微信支付->>微信支付: 使用平台证书私钥生成签名<br>        微信支付->>商户: 返回应答信息<br>        商户->>商户: 使用平台证书公钥验证签名<br>    end<br>    end<br>``` | iwiki-document:main |
...
| （1）请求应答场景 | （2）回调场景 |
| --- | --- |
| ```mermaid<br>sequenceDiagram<br>    rect rgb(255,255,255)<br>    participant 商户<br>    participant 微信支付<br>    %% 商户自操作：生成签名<br>    商户->>商户: 使用商户API证书私钥生成签名<br>    %% 发起业务请求（携带Wechatpay-Serial）<br>    商户->>微信支付: 发起业务请求<br/>(Wechatpay-Serial填入微信支付公钥ID)<br>    %% 微信支付验签<br>    微信支付->>微信支付: 使用商户API证书公钥验证签名<br>    %% 微信支付生成应答签名<br>    微信支付->>微信支付: 根据请求的Wechatpay-Serial<br/>选择使用微信支付私钥生成签名<br>    %% 返回应答<br>    微信支付->>商户: 返回应答<br>    %% 商户验证应答签名<br>    商户->>商户: 使用Wechatpay-Serial对应的微信支付公钥验证签名<br>    end<br>``` | ```mermaid<br>sequenceDiagram<br>    rect rgb(255,255,255)<br>    participant 商户<br>    participant 微信支付<br>    %% 微信支付自操作：加密回调信息<br>    微信支付->>微信支付: 使用APIv3密钥加密回调信息<br>    %% 红框内的回调验签流程，用无底色 rect 框选<br>    rect rgb(255,255,255)<br>        微信支付->>微信支付: 使用微信支付私钥或者平台证书私钥生成签名<br>        微信支付->>商户: 发送回调信息<br>        商户->>商户: 使用微信支付公钥或者平台证书公钥验证签名<br>    end<br>    %% 后续流程<br>    商户->>商户: 使用APIv3密钥解密回调信息<br>    商户->>微信支付: 返回处理结果<br>    end<br>``` |
...
sequenceDiagram
    ...
    %% 商户加密敏感信息
    商户->>商户: 使用微信支付公钥<br/>加密敏感信息（如姓名）
    %% 发起业务请求，携带公钥ID
    商户->>微信支付: 发起业务请求<br/>(Wechatpay-Serial填微信支付公钥ID)
    %% 微信支付解密请求信息
    微信支付->>微信支付: 使用微信支付私钥<br/>解密敏感信息（如姓名）
    %% 微信支付加密应答信息
    微信支付->>微信支付: 使用商户API证书公钥<br/>加密敏感信息（如手机号）
    %% 返回应答
    微信支付->>商户: 返回应答
    %% 商户解密应答信息
    商户->>商户: 使用商户API证书私钥<br/>解密敏感信息（如手机号）
    end
```
