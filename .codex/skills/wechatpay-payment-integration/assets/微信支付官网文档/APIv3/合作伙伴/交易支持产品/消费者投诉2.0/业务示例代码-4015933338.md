...
本文档Demo中使用的 `WXPayUtility` 工具类可通过[Java](https://pay.weixin.qq.com/doc/v3/partner/4014985777.md)文档获得。
...
// 使用合作伙伴平台通用投诉接口：https://pay.weixin.qq.com/doc/v3/partner/4012458106
...
public class CreateComplaintNotificationsExample {
    public static void main(String[] args) {
        // 请准备商户开发必要参数，参考：https://pay.weixin.qq.com/doc/v3/partner/4013080340
        ...
        try {
            ...
            if (e.getErrorCode().equals("PARAM_ERROR")) {
                ...
                // 解决方式：请参阅
                // https://pay.weixin.qq.com/doc/v3/partner/4012081726，如果可以修改参数达到符合业务规则的修改请求符合业务规则之后再原单重试
...
// 使用合作伙伴平台通用投诉接口：https://pay.weixin.qq.com/doc/v3/partner/4012459065
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (e.getErrorCode().equals("PARAM_ERROR")) {
                ...
                // 错误：签名错误
                // 解决方式：检查服务商证书序列号，证书私钥文件，同时确认下签名过程是不是按照微信支付的签名方式
            } else if (e.getErrorCode().equals("SYSTEM_ERROR")) {
                // 错误：系统错误
                // 解决方式：稍后原单重试
            } else if (e.getErrorCode().equals("NO_AUTH")) {
...
// 使用合作伙伴平台通用投诉接口：https://pay.weixin.qq.com/doc/v3/partner/4012459287
...
// 使用合作伙伴平台通用投诉接口：https://pay.weixin.qq.com/doc/v3/partner/4012460474
...
public class Callback {
    ...
    public Callback() {
        this.wechatPayPublicKeyId = "PUB_KEY_ID_xxxxxxxxxxxxx"; // 微信支付公钥ID，如何获取请参考
                                                                // https://pay.weixin.qq.com/doc/v3/partner/4013038589
        ...
        this.apiv3AesKey = "CF3xxxxxxxxxxxxxxxxxxxxxxxxxxxxx"; // APIv3密钥，如何获取请参考
                                                               // https://pay.weixin.qq.com/doc/v3/partner/4013059095
    }
...
// 使用合作伙伴平台通用投诉接口：https://pay.weixin.qq.com/doc/v3/partner/4012691648
...
public class QueryComplaintListExample {
    public static void main(String[] args) {
        ...
        // 分页大小最大为50
        // 对于服务商、渠道商，可通过调用此接口，查询指定特约商户号下的投诉信息，若不指定，则查询的是名下所有特约商户投诉信息。
        request.limit = 10L;
        request.offset = 0L;
...
// 使用合作伙伴平台通用投诉接口：https://pay.weixin.qq.com/doc/v3/partner/4012691802
...
public class QueryComplaintExample {
    ...
    QueryComplaintExample() {
        ...
        this.certificateSerialNo = "1DDE55AD98Exxxxxxxxxx"; // 商户API证书序列号，如何获取请参考
                                                            // https://pay.weixin.qq.com/doc/v3/partner/4013058924
            ...
            if (complaintMediaList != null && complaintMediaList.size() > 0) {
                ...
                for (ComplaintMedia complaintMedia : complaintMediaList) {
                    ...
                    for (String media_url : media_urls) {
                        ...
                        // 请求图片数据，请参考https://pay.weixin.qq.com/doc/v3/partner/4012467223
                    }
                }
            }
...
// 使用合作伙伴平台通用投诉接口：https://pay.weixin.qq.com/doc/v3/partner/4012467213
...
public class ResponseComplaintExample {
        ...
        {
            request.responseImages.add("aabbccdd"); // 在传入图片前应当先上传图片，请参考：https://pay.weixin.qq.com/doc/v3/partner/4012467222
        }
        ...
        try {
            ...
            if (e.getErrorCode().equals("PARAM_ERROR")) {
                ...
                // 错误：请求非法，请求参数正确，但是不符合业务规则
                // 解决方式：请参阅 https://pay.weixin.qq.com/doc/v3/partner/4012081726，如果可以修改参数达到符合业务规则的修改请求符合业务规则之后再原单重试
            } else if (e.getErrorCode().equals("SIGN_ERROR")) {
                // 错误：签名错误
                // 解决方式：检查平台商户公钥ID，公钥文件，同时确认下签名过程是不是按照微信支付的签名方式
            } else if (e.getErrorCode().equals("SYSTEM_ERROR")) {
...
// 使用合作伙伴平台通用投诉接口：https://pay.weixin.qq.com/doc/v3/partner/4012467217
...
// 使用合作伙伴平台通用投诉接口：https://pay.weixin.qq.com/doc/v3/partner/4012467218
