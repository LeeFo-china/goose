...
## 场景1: 单券-全场-折扣券 (usage_mode=SINGLE, scope=ALL, type=DISCOUNT)
...
curl -X POST \
  ...
  -d '{
    "out_request_no": "SINGLE_ALL_DISCOUNT_20250101_001", # 必填，创建请求单号，品牌侧需保持唯一性，6-40个字符
    "brand_id": "120344", # 必填，品牌ID
    "scope": "ALL", # 必填，优惠范围：ALL=全场券，SINGLE=单品券
    "type": "DISCOUNT", # 必填，商品券类型：NORMAL=满减券，DISCOUNT=折扣券，EXCHANGE=兑换券(仅单品)
    "usage_mode": "SINGLE", # 必填，使用模式：SINGLE=单券模式，PROGRESSIVE_BUNDLE=多次优惠

    "single_usage_info": { # 当usage_mode=SINGLE且scope=ALL时，在此配置优惠规则
      "discount_coupon": { # 当type=DISCOUNT且scope=ALL时必填，折扣券使用规则
        "threshold": 10000, # 必填，门槛金额，单位为分，0表示无门槛。满100元可用填10000
        "percent_off": 20 # 必填，固定减免百分比，20表示减免20%即打8折
      }
    },

    "display_info": { # 必填，商品券展示信息
      "name": "全场满100立打8折", # 必填，商品券名称，最多20个字符
      "image_url": "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/xxx", # 必填，商品图片URL，需通过图片上传接口获取
    ...
    "out_product_no": "Product_SINGLE_001", # 选填，外部商品ID
    ...
    "stock": { # 必填，批次信息
      ...
      "single_usage_rule": { # 当usage_mode=SINGLE时必填，单券使用规则
        "coupon_available_period": { # 必填，券可核销时间
          ...
          "available_end_time": "2025-08-31T23:59:59+08:00", # 必填，结束时间，RFC3339格式
          "available_days": 30, # 选填，生效后N天有效，最多365天
          "wait_days_after_receive": 0, # 选填，领取后N天生效，最多30天，0表示立即生效
          "weekly_available_period": { # 选填，每周可用时间
            "day_list": [ # 当配置weekly_available_period时必填，可用星期列表
              "MONDAY",
              "TUESDAY",
              "WEDNESDAY",
              "THURSDAY",
          ...
          "irregular_available_period_list": [ # 选填，无规律可用时间段，最多2个区间
            {
              "begin_time": "2025-08-15T00:00:00+08:00", # 当配置此列表时必填，开始时间
              "end_time": "2025-08-15T23:59:59+08:00" # 当配置此列表时必填，结束时间
            }
          ]
        }
        # 注意：scope=ALL时，优惠规则在single_usage_info中配置，此处不配置discount_coupon
      },

      "usage_rule_display_info": { # 必填，使用规则展示信息
        "coupon_usage_method_list": [ # 必填，券使用方式列表
          "OFFLINE", # 线下滴码核销
          "MINI_PROGRAM", # 线上小程序核销
          "PAYMENT_CODE" # 微信支付付款码核销
        ],
        "mini_program_appid": "wx1234567890", # 当coupon_usage_method_list包含MINI_PROGRAM时必填，核销小程序AppID
        "mini_program_path": "/pages/index/product", # 当coupon_usage_method_list包含MINI_PROGRAM时必填，核销小程序路径
        "usage_description": "工作日可用", # 必填，使用说明
        "coupon_available_store_info": { # 选填，可用门店信息
          "description": "所有门店可用，可使用小程序查看门店列表", # 当配置此对象时必填，门店描述
      ...
      "coupon_display_info": { # 必填，用户券展示信息
        ...
        "entrance_mini_program": { # 选填，小程序入口
          "appid": "wx1234567890", # 当配置此对象时必填，小程序AppID
          "path": "/pages/index/product", # 当配置此对象时必填，跳转路径
          "entrance_wording": "欢迎选购", # 当配置此对象时必填，入口文案
          "guidance_wording": "获取更多优惠" # 当配置此对象时必填，引导文案
        },
        "entrance_official_account": { # 选填，公众号入口
          "appid": "wx1234567890" # 当配置此对象时必填，公众号AppID
        },
        "entrance_finder": { # 选填，视频号入口
          "finder_id": "gh_12345678", # 当配置此对象时必填，视频号ID
          "finder_video_id": "UDFsdf24df34dD456Hdf34", # 当配置此对象时必填，视频号视频ID
          "finder_video_cover_image_url": "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/xxx" # 当配置此对象时必填，视频封面图URL
        }
      },

      "notify_config": { # 必填，事件通知配置
        "notify_appid": "wx4fd12345678" # 必填，事件通知AppID
      },
...
public class AllDiscountSingleJava {
  ...
  public AllDiscountSingleJava(String mchid, String certificateSerialNo, String privateKeyFilePath, String wechatPayPublicKeyId, String wechatPayPublicKeyFilePath) {
    this.mchid = mchid;
...
func main() {
	...
	request := &CreateProductCouponRequest{
		...
		DisplayInfo: &ProductCouponDisplayInfo{
			// 必填：商品券名称，最多20个字符
			Name: wxpay_utility.String("全场满100立打8折"),
...
// 单券使用规则（scope=ALL时只配置CouponAvailablePeriod，scope=SINGLE时还需配置优惠规则）
