...
curl -X POST \
  ...
  -d '{
    "out_request_no" : "MANY_SINGLE_EXCHANGE_20250101_005",  # 必填，创建请求单号，品牌侧需保持唯一性，长度6-40个字符
    "brand_id" : "120344",  # 必填，品牌ID
    "out_product_no" : "Product_1234567890",  # 选填，外部商品ID
    "scope" : "SINGLE",  # 必填，优惠范围：SINGLE=单品券。兑换券仅支持单品券
    "type" : "EXCHANGE",  # 必填，商品券类型：EXCHANGE=兑换券（用户支付固定金额兑换指定商品），仅scope=SINGLE时可用
    ...
    "progressive_bundle_usage_info" : {  # 条件必填：当usage_mode="PROGRESSIVE_BUNDLE"时必填，多次优惠模式配置信息
      ...
      "interval_days" : 7  # 选填，多次优惠使用间隔天数，7=每周可兑换一次（使用后需等7天才能使用下一次）
    },
    "display_info" : {  # 必填，商品券展示信息
      "name" : "9.9元兑换原价99商品(可用3次)",  # 必填，商品券名称
      ...
      "original_price" : 9900,  # 条件必填：当scope="SINGLE"时，与combo_package_list二选一必填，商品原价，单位为分（99元=9900分）
      "combo_package_list" : [  # 条件必填：当scope="SINGLE"时，与original_price二选一必填，套餐组合列表
        {
          "name" : "超值兑换套餐",  # 条件必填：当配置combo_package_list数组时必填，套餐名称
          "pick_count" : 3,  # 条件必填：当配置combo_package_list数组时必填，可选单品数量
          "choice_list" : [  # 条件必填：当配置combo_package_list数组时必填，单品列表
            { "name" : "汉堡", "price" : 1500, "count" : 1, "image_url" : "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/burger" },  # name/price/count必填
            { "name" : "鸡翅", "price" : 1200, "count" : 1, "image_url" : "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/wings" },
            { "name" : "可乐", "price" : 500, "count" : 1, "image_url" : "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/cola" }
          ]
        }
      ]
    },
    "stock_bundle" : {  # 条件必填：当usage_mode="PROGRESSIVE_BUNDLE"时必填，批次组信息
        "remark" : "9.9元兑换批次",  # 选填，批次备注
        ...
        "progressive_bundle_usage_rule" : {  # 条件必填：当usage_mode="PROGRESSIVE_BUNDLE"时必填，多次优惠使用规则
          "coupon_available_period" : {  # 必填，券可核销时间
            ...
            "weekly_available_period" : {  # 选填，每周可用时间
              "day_list" : ["MONDAY","TUESDAY","WEDNESDAY","THURSDAY","FRIDAY","SATURDAY","SUNDAY"]  # 条件必填：当配置weekly_available_period对象时必填，全周可用
            }
          },
          "exchange_coupon" : {  # 条件必填：当type="EXCHANGE"时必填，兑换券使用规则，定义兑换优惠的具体内容
            "threshold" : 0,  # 必填，门槛金额，单位为分，兑换券通常为0表示无门槛
            "exchange_price" : 990  # 必填，兑换价格，单位为分，用户需支付的金额。9.9元兑换填990
          }
        },
        "usage_rule_display_info" : {  # 必填，使用规则展示信息
          "coupon_usage_method_list" : ["OFFLINE","MINI_PROGRAM","PAYMENT_CODE"],  # 必填，券使用方式列表
          "mini_program_appid" : "wx1234567890",  # 条件必填：当coupon_usage_method_list包含"MINI_PROGRAM"时必填
          "mini_program_path" : "/pages/index/product",  # 条件必填：当coupon_usage_method_list包含"MINI_PROGRAM"时必填
          "usage_description" : "用9.9元兑换原价99元商品",  # 必填，使用说明
          "coupon_available_store_info" : {  # 选填，可用门店信息
            "description" : "所有门店可用，可使用小程序查看门店列表",  # 条件必填：当配置coupon_available_store_info对象时必填
        ...
        "coupon_display_info" : {  # 必填，用户券展示信息
          ...
          "entrance_mini_program" : {  # 选填，小程序入口
            ...
            "entrance_wording" : "立即兑换",  # 条件必填：当配置entrance_mini_program对象时必填，入口文案
            "guidance_wording" : "获取更多优惠"  # 条件必填：当配置entrance_mini_program对象时必填
          },
          "entrance_official_account" : { "appid" : "wx1234567890" },  # 选填
 ...
 */
public class SingleExchangeManyJava {
  ...
  public static void main(String[] args) {
    ...
    SingleExchangeManyJava client = new SingleExchangeManyJava(
      "19xxxxxxxx",                    // 商户号
    ...
    request.outRequestNo = "MANY_SINGLE_EXCHANGE_20250101_005";
    // 必填，品牌ID，由微信支付分配
    ...
    // 选填，多次优惠使用间隔天数(7表示每周可兑换一次)，最高7天，默认0
    request.progressiveBundleUsageInfo.intervalDays = 7L;  // 每周可兑换一次

    // 必填，商品券展示信息
    request.displayInfo = new ProductCouponDisplayInfo();
    // 必填，商品券名称，最多12个字符
    request.displayInfo.name = "9.9元兑换原价99商品(可用3次)";
    // 必填，商品券图片URL
    ...
    combo.name = "超值兑换套餐";  // 必填，套餐名称
    combo.pickCount = 3L;        // 必填，可选商品数量
    ...
    choice2.name = "鸡翅";    // 必填，商品名称
    choice2.price = 1200L;    // 必填，商品价格(单位：分)
    choice2.count = 1L;       // 必填，商品数量
    choice2.imageUrl = "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/wings";  // 必填，商品图片URL
    combo.choiceList.add(choice2);
    ComboPackageChoice choice3 = new ComboPackageChoice();
    choice3.name = "可乐";    // 必填，商品名称
    choice3.price = 500L;     // 必填，商品价格(单位：分)
    choice3.count = 1L;       // 必填，商品数量
    choice3.imageUrl = "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/cola";  // 必填，商品图片URL
    combo.choiceList.add(choice3);
    request.displayInfo.comboPackageList.add(combo);

    // 条件必填，批次信息(当usage_mode=PROGRESSIVE_BUNDLE时必填)
    request.stockBundle = createStock("9.9元兑换批次", 0, 990);
  ...
  private static StockForProgressiveBundle createStock(String remark, long threshold, long exchangePrice) {
    ...
    stock.progressiveBundleUsageRule.couponAvailablePeriod.weeklyAvailablePeriod.dayList.add(WeekEnum.SATURDAY);
    stock.progressiveBundleUsageRule.couponAvailablePeriod.weeklyAvailablePeriod.dayList.add(WeekEnum.SUNDAY);

    // 兑换券规则
    stock.progressiveBundleUsageRule.exchangeCoupon = new ExchangeCouponUsageRule();
    stock.progressiveBundleUsageRule.exchangeCoupon.threshold = threshold;
    stock.progressiveBundleUsageRule.exchangeCoupon.exchangePrice = exchangePrice;
    ...
    stock.usageRuleDisplayInfo.usageDescription = "用9.9元兑换原价99元商品";
    ...
    stock.couponDisplayInfo.entranceMiniProgram.entranceWording = "立即兑换";
  ...
  public SingleExchangeManyJava(String mchid, String certificateSerialNo, String privateKeyFilePath, String wechatPayPublicKeyId, String wechatPayPublicKeyFilePath) {
    this.mchid = mchid;
