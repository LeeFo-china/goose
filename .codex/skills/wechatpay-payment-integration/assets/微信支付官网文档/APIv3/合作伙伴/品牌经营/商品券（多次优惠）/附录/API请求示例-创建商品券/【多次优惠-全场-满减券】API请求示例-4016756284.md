...
curl -X POST \
  ...
  -d '{
    "out_request_no" : "MANY_ALL_NORMAL_20250101_002",  # 必填，创建请求单号，品牌侧需保持唯一性，可使用数字、大小写字母、下划线_、短横线-组成，长度6-40个字符
    "brand_id" : "120344",  # 必填，品牌ID，服务商为品牌创建商品券时使用
    "out_product_no" : "Product_1234567890",  # 选填，外部商品ID，品牌侧商品唯一标识
    "scope" : "ALL",  # 必填，优惠范围：ALL=全场券（适用于全部商品）
    "type" : "NORMAL",  # 必填，商品券类型：NORMAL=满减券（满足门槛金额后减免固定金额）
    ...
    "progressive_bundle_usage_info" : {  # 条件必填：当usage_mode="PROGRESSIVE_BUNDLE"时必填，多次优惠模式配置信息
      ...
      "interval_days" : 1  # 选填，多次优惠使用间隔天数，1=使用后次日00:00:00才能使用下一次优惠
    },
    "display_info" : {  # 必填，商品券展示信息
      "name" : "全场满100减15(可用3次)",  # 必填，商品券名称，最多20个字符
    ...
    "stock_bundle" : {  # 条件必填：当usage_mode="PROGRESSIVE_BUNDLE"时必填，批次组信息
        "remark" : "满100减15批次",  # 选填，批次备注
        "coupon_code_mode" : "WECHATPAY",  # 必填，券Code分配模式：WECHATPAY=微信支付自动生成券码
        ...
        "progressive_bundle_usage_rule" : {  # 条件必填：当usage_mode="PROGRESSIVE_BUNDLE"时必填，多次优惠使用规则
          ...
          "normal_coupon" : {  # 条件必填：当type="NORMAL"时必填，满减券使用规则，定义满减优惠的具体内容
            "threshold" : 10000,  # 必填，门槛金额，单位为分，达到该金额后可使用满减券。满100元可用填10000，无门槛填0
            "discount_amount" : 1500  # 必填，固定减免金额，单位为分，满足门槛后减免的金额。减15元填1500
          }
        },
        ...
        "coupon_display_info" : {  # 必填，用户券展示信息
          "code_display_mode" : "QRCODE",  # 必填，Code展示模式：QRCODE=显示二维码
          ...
          "entrance_mini_program" : {  # 选填，小程序入口
            ...
            "path" : "/pages/index/product",  # 条件必填：当配置entrance_mini_program对象时必填，跳转路径
            "entrance_wording" : "欢迎选购",  # 条件必填：当配置entrance_mini_program对象时必填，入口文案
            "guidance_wording" : "获取更多优惠"  # 条件必填：当配置entrance_mini_program对象时必填，引导文案
          },
          ...
          "entrance_finder" : {  # 选填，视频号入口
            ...
            "finder_video_id" : "UDFsdf24df34dD456Hdf34",  # 条件必填：当配置entrance_finder对象时必填，视频号视频ID
            "finder_video_cover_image_url" : "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/xxx"  # 条件必填：当配置entrance_finder对象时必填，视频封面图URL
          }
        },
        "store_scope" : "NONE",  # 必填，可用门店范围：NONE=不限制门店
...
public class AllNormalManyJava {
  ...
  public AllNormalManyJava(String mchid, String certificateSerialNo, String privateKeyFilePath, String wechatPayPublicKeyId, String wechatPayPublicKeyFilePath) {
    this.mchid = mchid;
