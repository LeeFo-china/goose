...
## 场景3: 单券-单品-折扣券 (usage_mode=SINGLE, scope=SINGLE, type=DISCOUNT)
...
curl -X POST \
  ...
  -d '{
    "out_request_no": "SINGLE_SINGLE_DISCOUNT_20250101_003", # 必填，创建请求单号
    ...
    # 注意：scope=SINGLE时，不在single_usage_info中配置优惠规则，优惠规则在stock.single_usage_rule中配置

    "display_info": { # 必填，商品券展示信息
      "name": "指定商品8折", # 必填，商品券名称
      ...
      "original_price": 9900, # 当scope=SINGLE时必填，商品原价，单位为分（99元）
      "combo_package_list": [ # 当scope=SINGLE时选填，套餐组合列表（与original_price二选一）
        {
          "name": "超值套餐", # 必填，套餐名称
          "pick_count": 2, # 必填，可选单品数量
          "choice_list": [ # 必填，单品列表
            {
              "name": "汉堡", # 必填，单品名称
              "price": 1500, # 必填，单品价格，单位为分（15元）
              "count": 1, # 必填，单品数量
              "image_url": "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/xxx", # 选填，单品图片URL
              "mini_program_appid": "wx1234567890", # 选填，跳转小程序AppID
              "mini_program_path": "/pages/index/product" # 选填，跳转小程序路径
            },
            {
              "name": "可乐", # 必填，单品名称
              "price": 500, # 必填，单品价格，单位为分
              "count": 1, # 必填，单品数量
              "image_url": "https://wxpaylogo.qpic.cn/wxpaylogo/xxxxx/xxx", # 选填
              "mini_program_appid": "wx1234567890", # 选填
              "mini_program_path": "/pages/index/product" # 选填
            }
          ]
        }
      ]
    ...
    "out_product_no": "Product_SINGLE_003", # 选填，外部商品ID

    "stock": { # 必填，批次信息
      "remark": "8月单品折扣批次", # 选填，备注
      ...
      "single_usage_rule": { # 当usage_mode=SINGLE时必填，单券使用规则
        "coupon_available_period": { # 必填，券可核销时间
          ...
          "weekly_available_period": { # 选填，每周可用时间
            "day_list": ["MONDAY", "TUESDAY", "WEDNESDAY", "THURSDAY", "FRIDAY"] # 当配置此对象时必填
          },
        ...
        "discount_coupon": { # 当type=DISCOUNT且scope=SINGLE时必填，折扣券使用规则
          "threshold": 0, # 必填，门槛金额，单位为分，0表示无门槛
          "percent_off": 20 # 必填，固定减免百分比，20表示打8折
        }
      },
 ...
 */
public class SingleDiscountSingleJava {
  ...
  public static void main(String[] args) {
    ...
    SingleDiscountSingleJava client = new SingleDiscountSingleJava(
    ...
    request.outRequestNo = "SINGLE_SINGLE_DISCOUNT_20250101_003";
    // 必填，品牌ID，由微信支付分配
    ...
    // 选填，商户侧商品券唯一标识
    request.outProductNo = "Product_SINGLE_003";

    // 必填，商品券展示信息
    request.displayInfo = new ProductCouponDisplayInfo();
    // 必填，商品券名称，最多12个字符
    request.displayInfo.name = "指定商品8折";
    // 必填，商品券图片URL
    ...
    // 必填，门槛金额(单位：分)，0表示无门槛
    request.stock.singleUsageRule.discountCoupon.threshold = 0L;
    // 必填，折扣百分比，20表示减免20%即打8折
    request.stock.singleUsageRule.discountCoupon.percentOff = 20L;
    ...
    request.stock.usageRuleDisplayInfo.usageDescription = "指定商品可用，享8折优惠";
  ...
  public SingleDiscountSingleJava(String mchid, String certificateSerialNo, String privateKeyFilePath, String wechatPayPublicKeyId, String wechatPayPublicKeyFilePath) {
    this.mchid = mchid;
...
// - type: DISCOUNT（折扣券）
//
// ⚠️ 重要：单品券的优惠规则配置在 stock.single_usage_rule.discount_coupon 中（不是single_usage_info）
//
...
func main() {
	...
	request := &CreateProductCouponRequest{
		...
		OutRequestNo: wxpay_utility.String("SINGLE_SINGLE_DISCOUNT_20250101_003"),
		// 必填：品牌ID，由微信支付分配
		...
		// 选填：商户侧商品券唯一标识
		OutProductNo: wxpay_utility.String("Product_SINGLE_003"),
		...
		DisplayInfo: &ProductCouponDisplayInfo{
			// 必填：商品券名称，最多20个字符
			Name: wxpay_utility.String("指定商品8折"),
		...
		Stock: &StockForCreate{
			// 选填：备注
			Remark: wxpay_utility.String("8月单品折扣批次"),
			...
			SingleUsageRule: &SingleUsageRule{
				...
				// ⚠️【关键】当type=DISCOUNT且scope=SINGLE时，在此配置折扣券优惠规则
