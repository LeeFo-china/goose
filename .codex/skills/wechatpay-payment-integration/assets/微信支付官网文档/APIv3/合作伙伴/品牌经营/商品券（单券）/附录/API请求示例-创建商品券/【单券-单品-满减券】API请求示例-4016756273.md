...
## 场景4: 单券-单品-满减券 (usage_mode=SINGLE, scope=SINGLE, type=NORMAL)
...
curl -X POST \
  ...
  -d '{
    "out_request_no": "SINGLE_SINGLE_NORMAL_20250101_004", # 必填，创建请求单号
    ...
    # 注意：scope=SINGLE时，不在single_usage_info中配置优惠规则

    "display_info": { # 必填，商品券展示信息
      "name": "指定商品立减15元", # 必填，商品券名称
      ...
      "combo_package_list": [ # 当scope=SINGLE时选填，套餐组合列表
        {
          "name": "超值套餐", # 必填，套餐名称
    ...
    "out_product_no": "Product_SINGLE_004", # 选填，外部商品ID

    "stock": { # 必填，批次信息
      "remark": "8月单品满减批次", # 选填，备注
      ...
      "single_usage_rule": { # 当usage_mode=SINGLE时必填，单券使用规则
        ...
        "normal_coupon": { # 当type=NORMAL且scope=SINGLE时必填，满减券使用规则
 ...
 */
public class SingleNormalSingleJava {
  ...
  public static void main(String[] args) {
    ...
    SingleNormalSingleJava client = new SingleNormalSingleJava(
    ...
    request.outRequestNo = "SINGLE_SINGLE_NORMAL_20250101_004";
    // 必填，品牌ID，由微信支付分配
    ...
    // 选填，商户侧商品券唯一标识
    request.outProductNo = "Product_SINGLE_004";

    // 必填，商品券展示信息
    request.displayInfo = new ProductCouponDisplayInfo();
    // 必填，商品券名称，最多12个字符
    request.displayInfo.name = "指定商品立减15元";
    // 必填，商品券图片URL
    ...
    request.stock.singleUsageRule.normalCoupon = new NormalCouponUsageRule();
    // 必填，门槛金额(单位：分)，0表示无门槛
    request.stock.singleUsageRule.normalCoupon.threshold = 0L;
    // 必填，减免金额(单位：分)
    request.stock.singleUsageRule.normalCoupon.discountAmount = 1500L;
    ...
    request.stock.usageRuleDisplayInfo.usageDescription = "指定商品可用，立减15元";
  ...
  public SingleNormalSingleJava(String mchid, String certificateSerialNo, String privateKeyFilePath, String wechatPayPublicKeyId, String wechatPayPublicKeyFilePath) {
    this.mchid = mchid;
...
// - type: NORMAL（满减券）
//
// ⚠️ 重要：单品券的优惠规则配置在 stock.single_usage_rule.normal_coupon 中（不是single_usage_info）
//
