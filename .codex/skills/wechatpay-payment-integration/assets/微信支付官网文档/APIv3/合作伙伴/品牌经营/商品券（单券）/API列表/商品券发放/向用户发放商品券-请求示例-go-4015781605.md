---
url: "https://pay.weixin.qq.com/doc/v3/partner/4015781605-go.md"
title: "向用户发放商品券-请求示例-go"
doc_id: 4015781605
example_lang: "go"
---
...
func main() {
	...
	request := &SendUserProductCouponRequest{
		...
		SendRequestNo:   wxpay_utility.String("MCHSEND202003101234"),
		Attach:          wxpay_utility.String("any attach content"),
		BrandId:         wxpay_utility.String("120344"),
		CouponTagInfo: &CouponTagInfo{
			CouponTagList: []UserProductCouponTag{
				USERPRODUCTCOUPONTAG_MEMBER,
			},
			MemberTagInfo: &MemberTagInfo{
				MemberCardId: wxpay_utility.String("MemberCardId_1234567890"),
			},
		},
	}

	response, err := SendUserProductCoupon(config, request)
	if err != nil {
		fmt.Printf("请求失败: %+v\n", err)
...
func SendUserProductCoupon(config *wxpay_utility.MchConfig, request *SendUserProductCouponRequest) (response *UserProductCouponEntity, err error) {
	const (
...
func (o *SendUserProductCouponRequest) MarshalJSON() ([]byte, error) {
	type Alias SendUserProductCouponRequest
	a := &struct {
		Openid *string `json:"openid,omitempty"`
		*Alias
	}{
