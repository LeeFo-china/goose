...
import type { navigateToMaterialDetail, navigateToPage } from "../../platform/navigation";
import {
  applyMaterialKeyword, beginMaterialListLoad, createMaterialListPageState,
  failMaterialListLoad, MaterialExperienceLifecycle, resolveMaterialListLoad,
  updateMaterialKeyword, type MaterialListPageState, type MaterialListRequest,
  type MaterialOperationAuthority,
} from "./page-model";

export type MaterialsPageDependencies = {
...
export function createMaterialsPageDefinition(dependencies: MaterialsPageDependencies) {
  return definePage({
    ...
    data: {
      items: [] as MaterialListPageState["pagination"]["items"], keyword: "",
    ...
    onShow() {
      ...
      this.syncState();
      if (!this.featureReady) { void this.initialize(); return; }
      const keyword = applyMaterialKeyword(this.pageState, this.pageState.debounceSequence);
      if (keyword.request) {
    ...
    async load(mode: "loadMore" | "refresh" | "retry") {
      ...
      await this.executeLoad(pending.state, pending.request, mode === "refresh", authority);
    },
    async executeLoad(
    ...
    syncState() {
      ...
      this.setData({
        items: this.pageState.pagination.items, keyword: this.pageState.keyword,
    ...
    onKeywordInput(event: { detail: { value?: string } }) {
      ...
      this.debounceTimer = setTimeout(() => {
        ...
        this.syncState();
        if (pending.request) void this.executeLoad(pending.state, pending.request, false, authority);
      }, changed.debounce.delayMs);
    },
    onRetry() { void this.load("retry"); },
    onLoadMore() { void this.load("loadMore"); },
    onOpenMine() { this.navigateWithFeedback(dependencies.navigateToPage("pages/my-materials/index")); },
