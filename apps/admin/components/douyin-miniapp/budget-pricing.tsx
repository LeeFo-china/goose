...
import {
  ...
  calculatePricingPreview, createBudgetPricingRequestAuthority,
...
export function BudgetPricing({ initialData, initialError }: {
  ...
  const listAuthority = useRef(createBudgetPricingRequestAuthority()).current;
  ...
  const loadPage = useCallback(async (page: number) => {
    ...
    try {
      ...
      if (isBudgetPricingAbortError(loadError) || !listAuthority.isCurrent(request)) return;
  ...
  const runMutation = useCallback(async (
    path: string, body: unknown, successMessage: string,
  ): Promise<BudgetPricingVersion | null> => {
    setBusy(true);
    try {
      const raw = await requestBackendJson<unknown>(path, {
        method: "POST", body: JSON.stringify(body), fallbackMessage: "报价配置操作失败",
      });
      const version = normalizePricingVersion(raw);
      if (!version) throw new Error("报价版本响应无效，请刷新后重试");
      setData((current) => ({
        ...current,
        list: current.list.some((item) => item.id === version.id)
          ? current.list.map((item) => item.id === version.id ? version : item)
  ...
  async function handleSaveItems() {
    ...
    setBusy(true);
    try {
      const raw = await requestBackendJson<unknown>(`${API_PATH}/${selectedVersion.id}/items`, {
        method: "PUT", body: JSON.stringify(buildPricingItemsPayload(selectedVersion.updated_at, items)),
        fallbackMessage: "报价项目保存失败",
      });
      const version = normalizePricingVersion(raw);
      if (!version) throw new Error("报价版本响应无效，请刷新后重试");
      setData((current) => ({ ...current, list: current.list.map((item) => item.id === version.id ? version : item) }));
      setDirty(false);
      toast.success("报价项目已保存");
    } catch (saveError) {
      toast.error(saveError instanceof Error ? saveError.message : "报价项目保存失败");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function handleStateChange(action: "activate" | "archive") {
    ...
    const version = await runMutation(`${API_PATH}/${selectedVersion.id}/${action}`, {
      expected_updated_at: selectedVersion.updated_at,
    }, action === "activate" ? "报价版本已启用" : "报价版本已归档");
    if (version) void loadPage(data.pagination.page);
  }
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(18rem,0.7fr)_minmax(28rem,1.3fr)]">
        <div className="flex min-w-0 flex-col gap-4">
          <Card className="shadow-none">
            ...
            <CardContent>{activeVersion ? <VersionSummary version={activeVersion} /> : <StatusAlert tone="warning">暂无生效报价，预算初算不会开放。</StatusAlert>}</CardContent>
          </Card>
          ...
          <Card className="shadow-none">
            ...
            <CardContent className="flex flex-col gap-2">
              ...
              {viewState === "error" ? <StatusAlert>{error}</StatusAlert> : null}
              ...
              {viewState === "ready" ? data.list.map((version) => <VersionButton key={version.id} version={version} selected={version.id === selectedId} disabled={dirty && version.id !== selectedId} onSelect={() => setSelectedId(version.id)} />) : null}
            </CardContent>
            <CardFooter className="flex flex-wrap justify-between gap-2 border-t">
              ...
              <div className="flex gap-2"><Button variant="outline" disabled={loading || dirty || data.pagination.page <= 1} onClick={() => void loadPage(data.pagination.page - 1)}><ChevronLeft data-icon="inline-start" />上一页</Button><Button variant="outline" disabled={loading || dirty || data.pagination.page >= data.pagination.totalPages} onClick={() => void loadPage(data.pagination.page + 1)}>下一页<ChevronRight data-icon="inline-end" /></Button></div>
            </CardFooter>
          </Card>
        </div>
        ...
        <Card className="min-w-0 shadow-none">
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            {selectedVersion?.status === "draft" ? <>
              <ValidationSummary warnings={saveWarnings} />
          ...
          {selectedVersion?.status === "draft" ? <CardFooter className="flex flex-wrap items-center justify-between gap-3 border-t">
            ...
            <div className="flex flex-wrap gap-2"><AlertDialog><AlertDialogTrigger asChild><Button variant="outline" disabled={busy || activationWarnings.length > 0}>启用报价版本</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>启用报价版本 {selectedVersion.version_no}？</AlertDialogTitle><AlertDialogDescription>启用后将归档当前生效版本，且本版本价格和有效期不可原地修改。请确认已录入公司审定价格。</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={busy}>返回检查</AlertDialogCancel><AlertDialogAction disabled={busy} onClick={() => void handleStateChange("activate")}>确认启用报价</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog><ValidationSummary warnings={activationWarnings} compact /></div>
...
function ValidationSummary({ warnings, compact = false }: { warnings: string[]; compact?: boolean }) { if (warnings.length === 0) return null; return <div id="budget-pricing-validation-summary" className={compact ? "max-w-md" : undefined}><StatusAlert tone="warning" title="请先完成以下检查"><ul className="mt-1 list-disc pl-5">{warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul></StatusAlert></div>; }
