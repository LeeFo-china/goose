...
import {
  ...
  BUDGET_LAYOUT_FACTOR_LABELS, BUDGET_PRICING_PAGE_SIZE,
  BUDGET_STYLE_FACTOR_LABELS, addPricingEditorItem, bpsToPercentInput,
  buildPricingDraftPayload, buildPricingFactorsPayload, buildPricingItemsPayload,
  ...
  getPricingAmountFieldErrors, getPricingCoefficientFieldError,
  getPricingDraftWarnings, getPricingFactorWarnings, getPricingItemWarnings,
  isBudgetPricingAbortError,
  normalizePricingEditorItemOrder, normalizePricingVersion,
  normalizePricingVersionPage, pricingItemToEditor, pricingStatusDisplay,
  removePricingEditorItem, toggleCanonicalCondition, type BudgetPricingDraftInput,
  type BudgetPricingEditorItem, type BudgetPricingPage,
  type BudgetPricingFactorPayload, type BudgetPricingVersion,
...
export function BudgetPricing({ initialData, initialError }: {
  ...
  const [factorPayload, setFactorPayload] = useState<BudgetPricingFactorPayload | null>(null);
  const [dirty, setDirty] = useState(false);
  const [factorDirty, setFactorDirty] = useState(false);
  ...
  useEffect(() => {
    setItems(normalizePricingEditorItemOrder(
      (selectedVersion?.items ?? []).map(pricingItemToEditor),
    ));
    setFactorPayload(selectedVersion?.factor_payload ?? null);
    setDirty(false);
    setFactorDirty(false);
  ...
  async function handleCreateDraft() {
    if (draftWarnings.length > 0 || dirty || factorDirty) return;
  ...
  async function handleSaveFactors() {
    if (!selectedVersion || !factorPayload || factorWarnings.length > 0) return;
    const version = await runMutation(`${API_PATH}/${selectedVersion.id}/factors`,
      buildPricingFactorsPayload(selectedVersion.updated_at, factorPayload), "户型风格系数已保存", "PUT");
    if (version) setFactorDirty(false);
  }
  ...
  function updateFactorPayload(patch: BudgetPricingFactorPayload) {
    setFactorPayload(patch);
    setFactorDirty(true);
  }
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(18rem,0.7fr)_minmax(28rem,1.3fr)]">
        <div className="flex min-w-0 flex-col gap-4">
          ...
          <Card className="shadow-none">
            ...
            <CardFooter><Button title={dirty || factorDirty ? "请先保存或撤销当前报价修改" : undefined} disabled={busy || dirty || factorDirty || draftWarnings.length > 0} onClick={() => void handleCreateDraft()}>{busy ? <Loader2 className="animate-spin" data-icon="inline-start" /> : <Plus data-icon="inline-start" />}创建报价草稿</Button></CardFooter>
          </Card>
          ...
          <Card className="shadow-none">
            ...
            <CardContent className="flex flex-col gap-2">
              ...
              {viewState === "ready" ? data.list.map((version) => <VersionButton key={version.id} version={version} isCurrent={version.id === activeVersion?.id} selected={version.id === selectedId} disabled={busy || ((dirty || factorDirty) && version.id !== selectedId)} onSelect={() => setSelectedId(version.id)} />) : null}
            </CardContent>
            <CardFooter className="flex flex-wrap justify-between gap-2 border-t">
              ...
              <div className="flex gap-2"><Button variant="outline" disabled={busy || loading || dirty || factorDirty || data.pagination.page <= 1} onClick={() => loadPage(data.pagination.page - 1)}><ChevronLeft data-icon="inline-start" />上一页</Button><Button variant="outline" disabled={busy || loading || dirty || factorDirty || data.pagination.page >= data.pagination.totalPages} onClick={() => loadPage(data.pagination.page + 1)}>下一页<ChevronRight data-icon="inline-end" /></Button></div>
            </CardFooter>
          </Card>
        </div>
...
export function PricingItemEditor({ item, index, onChange, onRemove }: { item: BudgetPricingEditorItem; index: number; onChange: (patch: Partial<BudgetPricingEditorItem>) => void; onRemove: () => void }) {
  const errorId = useId();
  const amountErrors = getPricingAmountFieldErrors(item);
  const minimumErrorId = `${errorId}-minimum-error`;
  const maximumErrorId = `${errorId}-maximum-error`;
  return <section className="flex flex-col gap-4 rounded-md border p-4" aria-labelledby={`budget-item-${index}`}><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><Badge variant="outline">{BUDGET_ITEM_LABELS[item.item_code]}</Badge><Badge variant="secondary">{BUDGET_CATEGORY_LABELS[item.category_code]}</Badge></div><Button type="button" variant="ghost" size="sm" onClick={onRemove}><Trash2 data-icon="inline-start" />移除项目</Button></div><FieldGroup className="grid gap-4 md:grid-cols-2 xl:grid-cols-3"><Field><FieldLabel id={`budget-item-${index}`} htmlFor={`budget-label-${index}`}>展示名称</FieldLabel><Input id={`budget-label-${index}`} value={item.label} maxLength={40} onChange={(event) => onChange({ label: event.target.value })} /></Field><Field data-invalid={Boolean(amountErrors.minimum)}><FieldLabel htmlFor={`budget-min-${index}`}>最低价（元/{item.unit === "sqm" ? "㎡" : "项"}）</FieldLabel><Input id={`budget-min-${index}`} inputMode="decimal" value={item.minimum_amount_yuan} aria-invalid={Boolean(amountErrors.minimum)} aria-describedby={amountErrors.minimum ? minimumErrorId : undefined} onChange={(event) => onChange({ minimum_amount_yuan: event.target.value })} /><FieldError id={minimumErrorId}>{amountErrors.minimum}</FieldError></Field><Field data-invalid={Boolean(amountErrors.maximum)}><FieldLabel htmlFor={`budget-max-${index}`}>最高价（元/{item.unit === "sqm" ? "㎡" : "项"}）</FieldLabel><Input id={`budget-max-${index}`} inputMode="decimal" value={item.maximum_amount_yuan} aria-invalid={Boolean(amountErrors.maximum)} aria-describedby={amountErrors.maximum ? maximumErrorId : undefined} onChange={(event) => onChange({ maximum_amount_yuan: event.target.value })} /><FieldError id={maximumErrorId}>{amountErrors.maximum}</FieldError></Field><Field><FieldLabel htmlFor={`budget-status-${index}`}>项目状态</FieldLabel><FormSelect id={`budget-status-${index}`} value={item.status} options={[{ value: "active", label: "参与计算" }, { value: "inactive", label: "暂不参与" }]} onChange={(value) => onChange({ status: value as BudgetPricingEditorItem["status"] })} /></Field>{item.item_code.startsWith("base.") ? <><CoefficientField id={`budget-property-coefficient-${index}`} label="房屋现状系数（%）" value={item.property_condition_coefficient_bps ?? 10_000} onChange={(value) => onChange({ property_condition_coefficient_bps: value })} /><CoefficientField id={`budget-whole-coefficient-${index}`} label="全屋系数（%）" value={item.whole_house_coefficient_bps ?? 10_000} onChange={(value) => onChange({ whole_house_coefficient_bps: value })} /><CoefficientField id={`budget-partial-coefficient-${index}`} label="局部系数（%）" value={item.partial_coefficient_bps ?? 10_000} onChange={(value) => onChange({ partial_coefficient_bps: value })} /></> : <><Field><FieldLabel htmlFor={`budget-category-${index}`}>预算分类</FieldLabel><FormSelect id={`budget-category-${index}`} value={item.category_code} options={Object.entries(BUDGET_CATEGORY_LABELS).map(([value, label]) => ({ value, label }))} onChange={(value) => onChange({ category_code: value as BudgetPricingEditorItem["category_code"] })} /></Field><Field><FieldLabel htmlFor={`budget-unit-${index}`}>计价单位</FieldLabel><FormSelect id={`budget-unit-${index}`} value={item.unit} options={[{ value: "sqm", label: "按平方米" }, { value: "fixed", label: "按项目" }]} onChange={(value) => onChange({ unit: value as "sqm" | "fixed" })} /></Field><OptionApplicability item={item} index={index} onChange={onChange} /></>}</FieldGroup></section>;
}

export function FactorEditor({ factorPayload, warnings, onChange }: { factorPayload: BudgetPricingFactorPayload; warnings: string[]; onChange: (next: BudgetPricingFactorPayload) => void }) {
  return <section className="flex flex-col gap-4 rounded-md border bg-muted/20 p-4" aria-labelledby="budget-factor-title">
    <div><h3 id="budget-factor-title" className="text-sm font-medium">户型和风格复杂度</h3><p className="mt-1 text-xs text-muted-foreground">按百分比维护版本级系数，只影响基础施工 subtotal，不改变选配项目。</p></div>
    <ValidationSummary id="budget-factor-validation-summary" warnings={warnings} compact />
    <FieldSet><FieldLegend>户型复杂度系数</FieldLegend><FieldGroup className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {Object.entries(BUDGET_LAYOUT_FACTOR_LABELS).map(([code, label]) => <CoefficientField key={code} id={`budget-layout-factor-${code}`} label={`${label}（%）`} value={factorPayload.layout_coefficients_bps[code as keyof BudgetPricingFactorPayload["layout_coefficients_bps"]]} onChange={(value) => onChange({ ...factorPayload, layout_coefficients_bps: { ...factorPayload.layout_coefficients_bps, [code]: value } })} />)}
    </FieldGroup></FieldSet>
    <FieldSet><FieldLegend>风格复杂度系数</FieldLegend><FieldGroup className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {Object.entries(BUDGET_STYLE_FACTOR_LABELS).map(([code, label]) => <CoefficientField key={code} id={`budget-style-factor-${code}`} label={`${label}（%）`} value={factorPayload.style_coefficients_bps[code as keyof BudgetPricingFactorPayload["style_coefficients_bps"]]} onChange={(value) => onChange({ ...factorPayload, style_coefficients_bps: { ...factorPayload.style_coefficients_bps, [code]: value } })} />)}
    </FieldGroup></FieldSet>
  </section>;
}
...
function CoefficientField({ id, label, value, onChange }: { id: string; label: string; value: number; onChange: (value: number) => void }) { const errorId = `${useId()}-error`; const error = getPricingCoefficientFieldError(value); return <Field data-invalid={Boolean(error)}><FieldLabel htmlFor={id}>{label}</FieldLabel><Input id={id} type="number" min="0.01" max="1000" step="0.01" value={bpsToPercentInput(value)} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} onChange={(event) => onChange(Math.round(Number(event.target.value) * 100))} /><FieldError id={errorId}>{error}</FieldError></Field>; }
...
function VersionSummary({ version }: { version: BudgetPricingVersion }) { const display = pricingStatusDisplay(version.status, true); return <div className="flex flex-col gap-2"><div className="flex items-center gap-2"><Badge variant={display.variant}>{display.label}</Badge><span className="font-medium tabular-nums">版本 {version.version_no}</span></div><p className="text-sm text-muted-foreground">{formatDateTime(version.effective_from)} 至 {version.effective_to ? formatDateTime(version.effective_to) : "长期有效"}</p><p className="text-sm">{version.disclaimer}</p></div>; }
function VersionButton({ version, isCurrent, selected, disabled, onSelect }: { version: BudgetPricingVersion; isCurrent: boolean; selected: boolean; disabled: boolean; onSelect: () => void }) { const display = pricingStatusDisplay(version.status, isCurrent); return <button type="button" aria-pressed={selected} disabled={disabled} title={disabled ? "请先保存或撤销当前报价修改" : undefined} className="flex w-full items-center justify-between gap-3 rounded-md border bg-card p-3 text-left transition-colors hover:bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50 aria-pressed:border-primary aria-pressed:bg-secondary" onClick={onSelect}><span className="min-w-0"><span className="block text-sm font-medium tabular-nums">版本 {version.version_no}</span><span className="mt-1 block text-xs text-muted-foreground">{formatDateTime(version.effective_from)} · {version.items.length} 项</span></span><Badge variant={display.variant}>{display.label}</Badge></button>; }
