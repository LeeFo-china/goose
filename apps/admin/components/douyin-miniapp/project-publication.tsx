...
import { ProjectTable, ProjectTableSkeleton } from "./project-publication-table";
import {
...
export function ProjectPublication({
  ...
  const [displayTogglingId, setDisplayTogglingId] = useState<string | null>(null);
  ...
  async function toggleProjectDisplay(row: ProjectPublicationRow, checked: boolean) {
    const draft = projectDisplayToggleDraft(row, checked);
    const toggleState = getProjectDisplayToggleState(row);
    if (!draft || toggleState.disabled) {
      toast.error(toggleState.reason ?? "请先完善公开资料");
      return;
    }
    setDisplayTogglingId(row.id);
    setError(null);
    try {
      const response = await requestBackendJson<unknown>(
        `/tenant/douyin-miniapp/projects/${row.id}/publication`,
        {
          method: "PATCH",
          body: JSON.stringify(draft),
          fallbackMessage: checked ? "开启项目展示失败" : "停止项目展示失败",
        },
      );
      const profile = normalizeSavedProjectProfile(response);
      if (!profile) throw new Error("项目展示状态保存结果无效，请刷新后确认");
      ...
      if (refreshPage !== null) {
        await loadPage(refreshPage, status);
        toast.success(checked ? "项目实景已开启展示" : "项目实景已停止展示");
        return;
      }
      setData((current) => ({
        ...current,
        list: current.list.map((item) =>
          item.id === row.id ? { ...item, public_profile: profile } : item
        ),
      }));
      toast.success(checked ? "项目实景已开启展示" : "项目实景已停止展示");
    } catch (toggleError) {
      toast.error(toggleError instanceof Error
        ? toggleError.message
        : checked ? "开启项目展示失败" : "停止项目展示失败");
    } finally {
      setDisplayTogglingId(null);
    }
  }
      ...
      <Card className="flex min-h-0 flex-1 flex-col overflow-hidden shadow-none">
        ...
        <CardContent className="relative flex min-h-0 flex-1 flex-col p-0">
          <div className="min-h-0 flex-1 overflow-auto">
            ...
            {listView === "empty" || listView === "ready" ? <ProjectTable rows={data.list} loading={loading} togglingId={displayTogglingId} onEdit={setEditing} onToggleDisplay={(row, checked) => void toggleProjectDisplay(row, checked)} /> : null}
          </div>
