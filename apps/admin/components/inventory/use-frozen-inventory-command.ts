...
export function useFrozenInventoryCommand(
  ...
  const storageKey = `${ADMIN_SESSION_STORAGE_PREFIX}${scope}:${options.storageSuffix}`;
  ...
  useEffect(() => {
    ...
    const restored = restoreFrozenInventoryCommand(storage, storageKey, options.parse);
