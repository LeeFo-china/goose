...
import { TransferDraftConflict } from './transfer-draft-conflict';
...
export function TransferWorkspace({
  ...
  const access = transferAccess(permissions);
  if (!access.canRead) {
    return (
      <StatusAlert tone="warning" title="暂无调拨查看权限">
        查看调拨单需要库存查看权限，请联系管理员。
      </StatusAlert>
    );
  }
  if (!session) {
...
function TransferWorkspaceSession({
  access,
  scope,
  initialOrderId,
}: {
  ...
  const [draft, setDraft] = useState<TransferDraftSeed | null>(null);
  ...
  const command = useTransferCommand(scope, (id, outcome) => {
    // Keep the original editor version until the user explicitly discards it.
    if (outcome === 'conflict' && draft?.id === id) {
  ...
  useEffect(() => {
    ...
    setEnabled(null);
    setSettingsError('');
    readTransfer<WarehouseTransferSettings>(
      '/warehouse-transfers/settings',
      controller.signal,
    )
      .then((settings) => {
        if (controller.signal.aborted) return;
        if (typeof settings?.warehouse_transfers_enabled !== 'boolean') {
          setSettingsError('功能配置无效，暂不能写入');
        } else {
          setEnabled(settings.warehouse_transfers_enabled);
        }
      })
      .catch((caught) => {
        if (!controller.signal.aborted) setSettingsError(transferError(caught));
      });
  ...
  const showingList = !draft && !selected;
  const execute = (path: string, body: object, id: string) => {
    if (!blocked && !draftConflict) void command.execute(path, body, id);
  };

  return (
    <div
      data-slot="warehouse-transfer-workspace"
      className={cn(
    ...
    >
      <div className="flex shrink-0 flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold tracking-normal">仓库调拨</h1>
        ...
        <div className="flex gap-2">
          ...
          {access.canManage ? (
            <Button
              disabled={blocked || Boolean(draft)}
              onClick={() => {
                if (draft) return;
                setSelected('');
      ...
      {enabled !== true ? (
        ...
        >
          {settingsError || '历史调拨单仍可查看，当前不能保存、提交、完成或取消。'}
          {settingsError ? (
            <Button
              variant="link"
      ...
      {command.message ? (
        <StatusAlert
          tone={command.message.startsWith('操作已成功') ? 'success' : 'warning'}
        >
          {command.message}
          {!draftConflict && command.message.includes('版本') ? (
      ...
      {!access.canManage && !access.canApprove ? (
        <p className="text-sm text-muted-foreground">
          当前为只读权限；保存、提交和取消需要调拨管理权限，完成需要调拨审批权限。
        </p>
      ) : null}
      {command.pending ? (
        <StatusAlert
          tone="warning"
          title={command.busy ? '正在执行调拨操作' : '上次请求结果尚未确认'}
        >
          <p>
      ...
      {draft ? (
        <TransferDraft
          ...
          seed={draft}
          access={access}
          disabled={blocked || draftConflict}
          onSave={execute}
      ...
      <p className="shrink-0 text-xs text-muted-foreground">
        完成调拨会对两个仓库进行成对库存过账，不生成项目成本、供应商应付或付款记录。
      </p>
    </div>
  );
}
