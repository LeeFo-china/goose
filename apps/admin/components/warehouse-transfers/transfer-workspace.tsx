...
function TransferWorkspaceSession({ access, scope, initialOrderId }: { access: TransferAccess; scope: string; initialOrderId?: string }) {
  ...
  const command = useTransferCommand(scope, (id, outcome) => {
    // Keep the same editor instance and its original version until the user explicitly discards it.
    if (outcome === 'conflict' && draft?.id === id) { setDraftConflict(true); return; }
    setDraftConflict(false); setDraft(null); setSelected(id); setRevision((value) => value + 1);
  });
  useEffect(() => {
  ...
  const execute = (path: string, body: object, id: string) => { if (!blocked && !draftConflict) void command.execute(path, body, id); };
  return <div className="flex min-w-0 flex-col gap-4 pb-6">
    ...
    {command.message && <StatusAlert tone={command.message.startsWith('操作已成功') ? 'success' : 'warning'}>{command.message}{!draftConflict && command.message.includes('版本') && <p>已重新读取最新版本，请核对后再次确认操作。</p>}</StatusAlert>}
    ...
    {draft && draftConflict && <TransferDraftConflict onReload={() => {
      setSelected(draft.id); setDraft(null); setDraftConflict(false); setRevision((value) => value + 1);
    }} />}
    {draft ? <TransferDraft key={draft.id} seed={draft} access={access} disabled={blocked || draftConflict} onSave={execute} onClose={() => setDraft(null)} /> : selected ? <><div><Button variant="ghost" onClick={() => setSelected('')}>返回列表</Button></div><TransferDetail key={selected} id={selected} revision={revision} access={access} disabled={blocked} onDraft={setDraft} onCommand={execute} /></> : <TransferList access={access} revision={revision} onOpen={setSelected} state={listState} onChange={setListState} />}
