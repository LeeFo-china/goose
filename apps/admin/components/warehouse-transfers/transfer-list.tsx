...
export const INITIAL_TRANSFER_LIST_STATE: TransferListState = {
  page: 1, search: '', keyword: '', status: 'all', source: null, destination: null,
};

export function TransferList({ access, revision, onOpen, state, onChange }: {
  access: TransferAccess; revision: number; onOpen: (id: string) => void;
  state: TransferListState; onChange: Dispatch<SetStateAction<TransferListState>>;
}) {
  const { page, search, keyword, status, source, destination } = state;
  const update = (change: Partial<TransferListState>) => onChange((current) => ({ ...current, ...change }));
  ...
  useEffect(() => {
    ...
    readTransfer<TransferPage<WarehouseTransferSummary>>(path, controller.signal).then((next) => {
      ...
      if (page > Math.max(1, next.pagination.totalPages)) { onChange((current) => ({ ...current, page: Math.max(1, next.pagination.totalPages) })); return; }
      setResult(next);
    }).catch((caught) => { if (!controller.signal.aborted) setError(transferError(caught)); });
    return () => controller.abort();
  }, [path, revision, retry, page, onChange]);
  return <section className="flex min-w-0 flex-col gap-3" aria-label="调拨单列表">
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      <form onSubmit={(event) => { event.preventDefault(); update({ keyword: search.trim(), page: 1 }); }}><Field><FieldLabel htmlFor="transfer-search">搜索单号 / 原因</FieldLabel><div className="flex gap-2"><Input id="transfer-search" value={search} maxLength={100} onChange={(event) => update({ search: event.target.value })} /><Button type="submit" variant="outline">搜索</Button></div></Field></form>
      <Field><FieldLabel htmlFor="transfer-status">状态</FieldLabel><FormSelect id="transfer-status" value={status} options={[{ value: 'all', label: '全部状态' }, ...Object.entries(WAREHOUSE_TRANSFER_STATUS_LABELS).map(([value, label]) => ({ value, label }))]} onChange={(value) => update({ status: value, page: 1 })} /></Field>
      {access.canViewWarehouses && <BatchOptionPicker id="transfer-source-filter" label="调出仓库" value={source} load={warehouseLoader} onChange={(value) => update({ source: value, page: 1 })} />}
      {access.canViewWarehouses && <BatchOptionPicker id="transfer-destination-filter" label="调入仓库" value={destination} load={warehouseLoader} onChange={(value) => update({ destination: value, page: 1 })} />}
    </div>
    <div><Button size="sm" variant="ghost" onClick={() => onChange(INITIAL_TRANSFER_LIST_STATE)}>清除筛选</Button></div>
    ...
    <TransferPager label="调拨单分页" pagination={result?.pagination} onPage={(value) => update({ page: value })} />
  </section>;
}
