...
> = {
  confirm_proposal: { label: "方案已确认", from: ["designing"], to: "proposal_confirmed" },
  sign_contract: { label: "项目签约", from: ["proposal_confirmed"], to: "signed" },
  finalize_design: { label: "设计定稿", from: ["signed"], to: "design_finalized" },
  schedule_construction: { label: "排期开工", from: ["design_finalized"], to: "pending_start" },
  start_project: { label: "确认开工", from: ["pending_start"], to: "started" },
  start_construction: { label: "正式进场", from: ["started"], to: "constructing" },
  pause_project: {
    label: "暂停项目",
    from: [
      "designing",
  ...
  resume_project: { label: "恢复项目", from: ["on_hold"], to: "paused_from_status" },
  start_acceptance: { label: "竣工验收", from: ["constructing"], to: "acceptance" },
  mark_invalid: {
    label: "作废项目",
    from: [
      "designing",
