...
import { platformTenantStatusOptions, type Pagination } from "@/components/platform-tenants/platform-tenant-types";

const statusOptions = [
  { value: "__all", label: "全部状态" },
  ...platformTenantStatusOptions,
] as const;

function buildPlatformTenantsHref(input: {
  page?: number;
  status?: string;
  keyword?: string;
}) {
...
export function PlatformTenantFilters({
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    navigate(buildPlatformTenantsHref({
        ...
        {selectedKeyword ? (
          <InputGroupAddon align="inline-end">
            <InputGroupButton
              ...
              onClick={() => {
                setSelectedKeyword("");
                navigate(buildPlatformTenantsHref({ status: selectedStatus }));
              }}
            >
              <X />
            </InputGroupButton>
...
export function PlatformTenantPagination({
  pagination,
  status,
  keyword,
}: {
  ...
  function navigate(page: number) {
    startTransition(() => {
      router.push(buildPlatformTenantsHref({ page, status, keyword }));
      router.refresh();
    });
  }
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      <div className="text-sm text-muted-foreground">
        第 {pagination.page} / {totalPages} 页，共 {pagination.total} 个租户
      </div>
