...
import {
  ...
  typeLabel,
  ValueCell,
} from "@/components/platform-identity-diagnostics/identity-diagnostics-table-shared";

export function IdentityDiagnosticsSummaryCards({ data }: { data: IdentityDiagnosticData }) {
  return (
...
export function IssueList({ issues }: { issues: IdentityDiagnosticIssue[] }) {
  if (issues.length === 0) {
    return (
      <Card>
...
export function AuthUsersTable({ data }: { data: IdentityDiagnosticData }) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
...
export function LegacyWechatTable({ data }: { data: IdentityDiagnosticData }) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
...
export function BusinessProfilesTable({ data }: { data: IdentityDiagnosticData }) {
  const tenantName = new Map(data.tenants.map((tenant) => [tenant.id, tenant.name || tenant.slug || tenant.id]));
  const rows = [
...
export function EventsTable({ events }: { events: IdentityDiagnosticAuthEvent[] }) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
