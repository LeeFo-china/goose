...
import type { AiProviderRecord, AiRouteModelOptionPage, AiRouteModelOptionRecord } from "@/components/platform-ai/ai-config-types";
...
export type RouteModelOptionsState = {
  // Before loading metadata is absent; failed is a client state because API failures are HTTP errors.
  data: Omit<AiRouteModelOptionPage, "discovery"> & {
    discovery?: AiRouteModelOptionPage["discovery"] | { status: "failed" };
  };
  loading: boolean;
  error: string | null;
...
export function AiRouteModelSelector({
  title, target, providers, providerId, keyword, value, state, allowNone = false, disabled = false, sceneModality = "text", manualEnabled = false,
  onProviderChange, onKeywordChange, onSearch, onSelect, onManual,
}: Props) {
  const selected = state.selected;
  const candidates = state.data.list.filter((item) => item.source !== "manual" && item.modality === sceneModality);
  const optionList = selected ? [selected, ...candidates.filter((item) => item.value !== selected.value)] : candidates;
  ...
  const hasFailure = Boolean(state.error) || discovery?.status === "failed";
  const showFailure = hasFailure && !state.loading;
  const manualAction = <Button type="button" variant="outline" className="min-h-11 sm:min-h-9"
    disabled={!providerId || modelDisabled || manualEnabled} onClick={onManual}>手动填写调用名称</Button>;
      ...
      <div className="flex flex-col gap-3 rounded-md border bg-muted/20 p-3">
        <Field>
          ...
          <Select value={providerId} disabled={disabled} onValueChange={(next) => onProviderChange(target, next)}>
            <SelectTrigger id={`ai-route-${target}-provider`} className="min-h-11 sm:min-h-9"><SelectValue placeholder="选择供应商" /></SelectTrigger>
        ...
        <Field>
          ...
          <div className="flex gap-2">
            <Input id={`ai-route-${target}-keyword`} className="min-h-11 min-w-0 sm:min-h-9" value={keyword} disabled={modelDisabled}
              onKeyDown={(event) => { if (event.key === "Enter" && canSearch) { event.preventDefault(); onSearch(target, 1); } }}
              onChange={(event) => onKeywordChange(target, event.target.value)} placeholder="搜索模型名称或调用名称" />
            <Button type="button" variant="outline" className="min-h-11 sm:min-h-9" disabled={!canSearch} onClick={() => onSearch(target, 1)}>
        ...
        <Field>
          ...
          <Select value={value} disabled={!providerId || modelDisabled || state.loading} onValueChange={(next) => {
            ...
            <SelectTrigger id={`ai-route-${target}-model`} className="min-h-11 sm:min-h-9"><SelectValue placeholder={state.loading ? "候选模型加载中" : allowNone ? "无备用模型" : "选择模型"} /></SelectTrigger>
            <SelectContent><SelectGroup>
        ...
        <div>
          ...
          {!state.loading && !hasFailure && discovery?.status === "unsupported" ? <FieldDescription role="status">当前协议不支持自动发现厂商完整目录，列表仅包含已登记模型。</FieldDescription> : null}
          {!state.loading && !hasFailure && discovery?.status === "empty" ? <FieldDescription role="status">OpenRouter 目录暂无符合条件的候选模型。可选择匹配的已登记模型，或手动填写调用名称。</FieldDescription> : null}
          {!state.loading && !hasFailure && discovery?.status === "ready" ? <FieldDescription role="status">OpenRouter 目录已有可选模型，实际调用能力仍需验证。</FieldDescription> : null}
          {!disabled && !state.loading && !hasFailure && providerId && candidates.length === 0 ? <FieldDescription role="status" aria-live="polite">暂无符合场景模态的可用模型，可手动填写调用名称。</FieldDescription> : null}
        ...
        {!showFailure ? manualAction : null}
        {manualEnabled ? <FieldDescription>本次保存使用下方手动填写的调用名称。取消手动填写可恢复所选模型。</FieldDescription> : null}
