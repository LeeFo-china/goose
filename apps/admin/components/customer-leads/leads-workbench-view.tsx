...
export function LeadFiltersToolbar({ filters, assigneeOptions, assigneeKeyword, sourceFilters = false,
  ...
  return <form className="grid gap-2 md:grid-cols-2 xl:grid-cols-4" onSubmit={submit}>
    {sourceFilters ? <><Field><FieldLabel className="sr-only" htmlFor="customer-lead-source">来源</FieldLabel><FormSelect id="customer-lead-source" value={draft.source || "__all"} disabled={disabled} options={[{ value: "__all", label: "全部来源" }, { value: "douyin_miniapp", label: "抖音小程序" }]} onChange={(value) => setDraft({ ...draft, source: value === "douyin_miniapp" ? value : "" })} /></Field><Field><FieldLabel className="sr-only" htmlFor="customer-lead-assignment">分配状态</FieldLabel><FormSelect id="customer-lead-assignment" value={draft.assignment || "all"} disabled={disabled} options={[{ value: "all", label: "全部分配状态" }, { value: "assigned", label: "已分配" }, { value: "unassigned", label: "未分配" }]} onChange={(value) => setDraft({ ...draft, assignment: value as LeadFilters["assignment"], ...(value === "unassigned" ? { assigneeId: "" } : {}) })} /></Field></> : null}
