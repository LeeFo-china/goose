...
export function ServiceHealthPanel() {
  ...
  const businessContainers = useMemo(() => {
    return snapshot?.containers.filter((item) => item.group === "business") || [];
  }, [snapshot]);
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
          ...
          <span>业务容器 {businessContainers.length} 个</span>
        </div>
      </CardContent>
    </Card>
  );
