...
import type { DouyinMaterialNoteTenantDetail } from "@gooes/domain";
import { z } from "zod";
...
async function loadResource<T>(
  ...
  try {
    ...
    if (response.status === 404) {
      return { data: null, error: "资料不存在", status: 404 };
    }
...
export default async function TenantMaterialNoteDetailPage({
  ...
  const detailResult = await loadResource<DouyinMaterialNoteTenantDetail>(
    token,
