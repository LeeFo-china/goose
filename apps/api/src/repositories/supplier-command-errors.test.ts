...
describe("mapSupplierCommandDatabaseError", () => {
  ...
  test("does not treat a longer identifier as a known database token", () => {
    expect(mapSupplierCommandDatabaseError({
      message: "SUPPLIER_PRODUCT_NOT_FOUND_EXTRA",
      details: "untrusted database context",
    })).toBeNull();
    expect(mapSupplierCommandDatabaseError({
      message: "database command failed",
      details: "context: SUPPLIER_PRODUCT_NOT_FOUND (raised by function)",
    })).toMatchObject({
  ...
  test.each([
    ["not_found", "SUPPLIER_PURCHASE_REQUISITION_SELF_REVIEW"],
    ["price_missing", "SUPPLIER_PURCHASE_REQUISITION_PRICE_CHANGED"],
    ["price_changed", "SUPPLIER_PURCHASE_ORDER_PRICE_CHANGED"],
    ["validation_error", "SUPPLIER_PURCHASE_ORDER_AMOUNT_LIMIT_EXCEEDED"],
    ["self_review", "SUPPLIER_PURCHASE_REQUISITION_SELF_REVIEW"],
    ["idempotency_conflict", "SUPPLIER_IDEMPOTENCY_CONFLICT"],
    ["state_conflict", "SUPPLIER_PURCHASE_REQUISITION_UNKNOWN"],
  ])("rejects non-SQL envelope pair %s + %s", (status, code) => {
