...
function validateTrialFacts(row: TrialRow, context: z.RefinementCtx): void {
  ...
  const grantFacts = [row.granted_at, row.granted_by_employee_id, row.starts_at,
    row.trial_ends_at, row.grace_ends_at];
  const durationFacts = [row.starts_at, row.trial_ends_at, row.grace_ends_at];
  const durationFactCount = durationFacts.filter((fact) => fact !== null).length;
  if (durationFactCount !== 0 && durationFactCount !== durationFacts.length) {
    issue('试用时间事实不完整');
  }
  if (['scheduled', 'active', 'grace_period', 'expired'].includes(row.status)
    && grantFacts.some((fact) => fact === null)) issue('已发放状态事实不完整');
  if (row.status === 'pending_review'
    && [...grantFacts, row.reviewed_at].some((fact) => fact !== null)) {
    issue('待审核状态事实冲突');
  }
  if (row.review_decision === null
    ? row.reviewed_at !== null || row.reviewed_by_employee_id !== null || row.review_reason !== null
    : row.reviewed_at === null || row.reviewed_by_employee_id === null || row.review_reason === null) {
    issue('审核事实不完整');
  }
  if (row.status === 'rejected' && row.review_decision !== 'rejected') issue('驳回事实冲突');
  if (row.status === 'withdrawn'
    && (row.withdrawn_at === null || row.withdrawn_by_employee_id === null
      || row.withdraw_reason === null)) issue('撤回事实不完整');
  if (row.status === 'revoked'
    && (row.revoked_at === null || row.revoked_by_employee_id === null
      || row.revoke_reason === null)) issue('撤销事实不完整');
  if (row.status === 'converted'
    && (row.converted_at === null || row.converted_order_id === null)) issue('转化事实不完整');
...
const TenantSummarySchema = z.object({
  id: z.uuid(), name: z.string().nullable(), slug: z.string().nullable(),
}).strict();
