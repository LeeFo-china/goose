...
interface FilterOptionsClient {
  ...
  from(table: "employees"): IncludedEmployeeQuery;
}
interface IncludedEmployeeQuery {
  select(...args: unknown[]): IncludedEmployeeQuery;
  eq(...args: unknown[]): IncludedEmployeeQuery;
  limit(...args: unknown[]): IncludedEmployeeQuery;
...
const IncludedEmployeeSchema = z.strictObject({
  id: z.uuid(), tenant_id: z.uuid(), name: z.string().nullable(),
});
...
export async function listTenantDouyinLeadAssigneeFilterOptions(
  ...
  try {
    ...
    const includeEmployeeId = input.includeEmployeeId;
    if (!includeEmployeeId || list.some((row) => row.id === includeEmployeeId)
      || (visibleIds && !visibleIds.has(includeEmployeeId))) {
      return { rows: list, total };
    }
    const includedResult = await client.from("employees")
      .select("id,tenant_id,name").eq("tenant_id", input.tenantId)
      .eq("id", includeEmployeeId).limit(1).maybeSingle();
    if (includedResult.error) throw Errors.dbError(ERROR_MESSAGE);
    if (includedResult.data === null) return { rows: list, total };
    const included = IncludedEmployeeSchema.safeParse(includedResult.data);
    if (!included.success || included.data.tenant_id !== input.tenantId
      || included.data.id !== includeEmployeeId) {
      throw Errors.dbError(ERROR_MESSAGE);
    }
    return { rows: [{ id: included.data.id, name: included.data.name }, ...list]
      .slice(0, input.pageSize), total };
  } catch {
    throw Errors.dbError(ERROR_MESSAGE);
  }
}
