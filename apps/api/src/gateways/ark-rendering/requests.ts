...
import { normalizeAiProviderBaseUrl } from "@/services/ai-config/endpoint-normalizer";
...
export function arkEndpoint(config: ArkGatewayConfig, path: string): string {
  ...
  const modality = path === "/chat/completions" ? "text"
    : path === "/images/generations" ? "image" : null;
  let baseUrl: string;
  try {
  ...
  if (!modality || new URL(baseUrl).pathname !== "/api/v3"
  ...
  return aiInferenceEndpoint(baseUrl, modality);
}
