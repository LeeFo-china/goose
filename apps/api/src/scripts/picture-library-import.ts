...
async function buildDryRunReport(candidates: ImageCandidate[]) {
  ...
  for (const candidate of candidates) {
    if (await findAssetByChecksum(candidate.checksum)) existingCount += 1;
  }

  const categories = new Set(candidates.map((item) => item.categoryName));
  return {
    mode: "dry-run",
    source_count: candidates.length,
    category_count: categories.size,
    existing_asset_count: existingCount,
    pending_upload_count: candidates.length - existingCount,
