...
export const MATERIAL_NOTE_EXPLAIN_QUERIES = Object.freeze({
  public_list: [
    EXPLAIN,
    "-- public_list",
    "select note.id, note.published_at, count(*) over () as exact_count,",
  ...
  tenant_keyword_list: [
    EXPLAIN,
    "-- tenant_keyword_list",
    "select note.id, note.status, note.published_at, note.updated_at,",
    "  count(*) over () as exact_count,",
    "  latest_version.version_no, latest_version.title, latest_version.category,",
    "  (select count(*)::integer",
    "    from public.douyin_material_note_claims as claim_count",
    "    where claim_count.tenant_id = note.tenant_id",
    "      and claim_count.note_id = note.id) as claim_count",
    "from public.douyin_material_notes as note",
    "left join lateral (",
    "  select version_no, title, category",
    "  from public.douyin_material_note_versions as latest_version_source",
    "  where latest_version_source.tenant_id = note.tenant_id",
    "    and latest_version_source.note_id = note.id",
    "  order by latest_version_source.version_no desc",
    "  limit 1",
    ...
    "      and version.note_id = note.id",
    "      and (version.title ilike $2 escape E'\\\\'",
    "        or version.summary ilike $2 escape E'\\\\'",
    "        or version.category ilike $2 escape E'\\\\')",
    "  )",
  ...
  owned_active_list: [
    EXPLAIN,
    "-- owned_active_list",
    "select claim.id, claim.claimed_at, note.status,",
    "  count(*) over () as exact_count,",
