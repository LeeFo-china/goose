...
describe("supplier foundation migration contract", () => {
  ...
  test("locks idempotent lifecycle state machines and aggregate writes", () => {
    ...
    expectSqlContracts(extractFunction(sql, "set_tenant_supplier_module"), [/p_reason text/, /IF NOT p_module_enabled AND \(p_reason IS NULL OR btrim\(p_reason\) = ''\)/, /reason[\s\S]*p_reason/]);
    expect(sql).toContain("REVOKE ALL ON FUNCTION public.set_tenant_supplier_module(uuid, boolean, boolean, integer, uuid, uuid, text, text) FROM PUBLIC, anon, authenticated;");
    expect(sql).toContain("GRANT EXECUTE ON FUNCTION public.set_tenant_supplier_module(uuid, boolean, boolean, integer, uuid, uuid, text, text) TO service_role;");
    expectSqlContracts(sql, [
