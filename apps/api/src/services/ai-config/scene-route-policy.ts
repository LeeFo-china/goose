...
export class AiSceneRoutePolicy {
  ...
  async prepareCreate(input: AiSceneRoutePayload): Promise<PreparedAiSceneRouteCreate> {
    ...
    if (scene.source !== "system" || !scene.allow_new_configuration) {
    ...
    if (
      scene.source === "system"
      && scene.runtime_status === "not_connected"
      && this.hasBinding(prepared)
    ) {
      throw Errors.business(409, "该 AI 场景运行时尚未接通，不能绑定模型", "AI_SCENE_RUNTIME_NOT_CONNECTED");
    }
    await this.assertRouteModels(prepared);
    return prepared;
  }
  ...
  async prepareUpdate(id: string, input: UpdateAiSceneRoutePayload): Promise<UpdateAiSceneRoutePayload> {
    ...
    if (
      scene.source === "system"
      && scene.runtime_status === "not_connected"
      && this.hasNewBinding(current, input)
    ) {
      throw Errors.business(409, "该 AI 场景运行时尚未接通，不能绑定模型", "AI_SCENE_RUNTIME_NOT_CONNECTED");
    }
    await this.assertRouteModels(merged);
    return input;
  }
  ...
  private hasBinding(input: { primary_model_id?: string | null; fallback_model_id?: string | null }): boolean {
    return Boolean(input.primary_model_id || input.fallback_model_id);
  }

  private hasNewBinding(current: AiSceneRouteRecord, input: UpdateAiSceneRoutePayload): boolean {
    return (["primary_model_id", "fallback_model_id"] as const).some((key) =>
      Object.hasOwn(input, key) && input[key] !== null && input[key] !== current[key]);
  }
