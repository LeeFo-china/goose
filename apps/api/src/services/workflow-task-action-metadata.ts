...
export function buildWorkflowTaskActions(
  ...
  if (input.subjectType === "project") {
    if (isFinalAcceptanceReportNode(input)) {
      return [];
    }
...
function isFinalAcceptanceReportNode(input: BuildWorkflowTaskActionsInput) {
  if (input.nodeKey !== "final_acceptance") return false;
  const snapshot = asRecord(input.currentNodeSnapshot);
  if (!snapshot) return false;
  if (snapshot.node_key !== input.nodeKey) return false;
  return isFinalAcceptanceReportWorkflowNode(snapshot);
}
...
function getCurrentNodeConfig(
  ...
  return getWorkflowNodeConfig(snapshot.config);
}
