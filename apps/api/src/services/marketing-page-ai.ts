...
import type { MarketingPageBlockAiFillInput } from "@/schema/marketing-pages";
...
function normalizePatch(raw: unknown, definitions: Record<string, FieldDefinition>) {
  ...
  for (const [key, definition] of Object.entries(definitions)) {
    const normalized = normalizePatchValue((source as Record<string, unknown>)[key], definition);
    if (normalized) {
