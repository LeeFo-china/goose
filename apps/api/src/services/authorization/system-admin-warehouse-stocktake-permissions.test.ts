...
import { STOCKTAKE_ID } from '@/repositories/warehouse-stocktake-test-fixtures';
...
test('derived system-admin permissions reach stocktake RPC but SQL denial remains authoritative', async () => {
  ...
  const context: EmployeePermissionContextRecord = {
    employee: { id: STOCKTAKE_ID, user_id: STOCKTAKE_ID, tenant_id: STOCKTAKE_ID, status: 'active',
      tenant_department_id: null, post_id: null, name: '管理员', phone: null, avatar: null,
      tenant_department: null, post: null, tenant: { id: STOCKTAKE_ID, name: '租户', slug: 'tenant', status: 'active' } },
    roles: [{ id: STOCKTAKE_ID, tenant_id: STOCKTAKE_ID, code: 'system_admin', name: '系统管理员',
      description: null, status: 'active', created_at: 'now', updated_at: 'now' }],
    rolePermissions: [], overrides: [],
  };
  const auth = buildAuthContext(context, STOCKTAKE_ID);
  ...
  let calls = 0;
  const repository = new WarehouseStocktakesRepository({ rpc: async () => {
    calls += 1; return { data: null, error: { message: 'WAREHOUSE_STOCKTAKE_FORBIDDEN' } };
  } });
