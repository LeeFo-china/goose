...
test("model endpoint redacts nested historical provider values at HTTP boundary", async () => {
  ...
  const model = {
    ...
    provider: {id: "provider", code: "provider", name: "provider", provider_type: "openai_compatible", endpoint_url: null, api_key_setting_key: synthetic, status: "active", sort_order: 0, created_at: "now", updated_at: "now"},
  };
