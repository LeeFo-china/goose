...
describe("DouyinMiniappController", () => {
  ...
  test("validates and dispatches the exact privacy-safe session request", async () => {
    ...
    expect(routes.map(({ method, path }) => `${method} ${path}`)).toEqual([
      ...
      "POST /douyin-mini/renderings/uploads:intent",
      "POST /douyin-mini/renderings/uploads/:id/complete",
      "GET /douyin-mini/renderings/uploads/:id",
      "GET /douyin-mini/renderings/uploads/:id/preview",
      "POST /douyin-mini/renderings/jobs",
      "GET /douyin-mini/renderings/jobs/:id",
